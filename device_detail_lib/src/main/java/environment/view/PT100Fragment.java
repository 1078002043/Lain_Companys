...
import environment.bean.PT100AlertBean;
import environment.bean.PT100HistoryBean;
import environment.bean.PT100RealBean;
import http.MyGson;
...
/**
 * @ClassName: PT100Fragment
 * @Description:
 * @Author: YIN LUO FEI
 * @Date: 2020/3/28 9:39
 */
public class PT100Fragment extends Device8060Fragment<PT100RealBean, PT100AlertBean,
        PT100HistoryBean, PT100RealBean> implements BaseRecyclerViewAdapter.OnItemClickListener, I_DeviceDetailManageLink<Device_Detail_AddBean>, I_HistoryAdapter<PT100HistoryBean>, I_RealAdapterListen<PT100RealBean>, I_UpdateRealView<PT100RealBean> {

    private UltimateRecyclerView waterPressureRealRecycler;

    List<PT100RealBean> waterPressureRealBeans = new ArrayList<>();

    //-------------------------------------------------
    private BaseDeviceDetailManageAdapter<Device_Detail_AddBean, PT100RealBean> manageAdapter;
...
//    public Temperature_Add deviceDetail_AddAdapter;
    ...
    String deleteUrl = "";
    String changeUrl = "";
    NewHistoryAdapter<PT100HistoryBean> newHistoryAdapter;
    List<PT100HistoryBean> lineDataList = new ArrayList<>();
    //设备管理的数据
    ...
    //设备实时数据工具类
    public RealDataUtil<PT100RealBean> realDataUtil;
     ...
     */
    public PT100Fragment(String realLink, String alertLink, String historyLink, String manageLink) {
        super(realLink, alertLink, historyLink, manageLink);
    }

    public PT100Fragment(String realLink, String alertLink, String historyLink, String manageLink, String deleteUrl, String changeUrl) {
        super(realLink, alertLink, historyLink, manageLink);
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        //查询实时数据
        device8060Present.queryRealData(PT100RealBean.class);
//        startRealUpdate();
        //设置历史数据列表
    ...
    public void queryHistoryData() {
        try {
            ...
            //默认查询第一个设备
            PT100RealBean temperatureBean = waterPressureRealBeans.get(0);
            ///获取设备的ID

            int temEhmId = temperatureBean.getEhmpId();
            //结束时间
            String endTime = DateUtil.getInstance().getCurrentDate();
            //开始时间
            String startTime = DateUtil.getInstance().getStartTime();
            //开始查询
            device8060Present.queryHistoryData(PT100HistoryBean.class, temEhmId, startTime, endTime);
    ...
    protected void querySelectDeviceHistory(int ehmID, String startTime, String endTime) {
        device8060Present.queryHistoryData(PT100HistoryBean.class, ehmID, startTime, endTime);
    }
    ...
    public void requestReal() {
        ...
        try {
            ...
            if (alertDeviceBeanList.isEmpty()) {
                //每次查询都保存第一个设备ID
                ehmIDFirst = waterPressureRealBeans.get(0).getEhmpId();
                //更新报警查询的设备
                for (PT100RealBean b :
                        waterPressureRealBeans) {
                    AlertDeviceBean alertB = new AlertDeviceBean();

                    alertB.setName(b.getEhmpDeviceName());
    ...
    public void setCurrentData(int position) {
        try {
            //点击切换上面的圆形进度条的数据
            //设置相关的属性值
            PT100RealBean b = waterPressureRealBeans.get(position);

            if (b.getEhmpTemp() == currentTempValue)
                return;
    ...
    public void requestAlert() {
        ...
        List<PT100AlertBean> air = getAlertData();
        alertBeans.clear();

        for (int i = 0; i < air.size(); i++) {
            PT100AlertBean alertBean = air.get(i);
            AlertBeans alertBeansItem = new AlertBeans();
            alertBeansItem.setEhaInfo(alertBean.getEhpaInfo());
            alertBeansItem.setEhaTime(alertBean.getEhpaTime());
...
//        先清空原有的设备数据
        ...
        List<PT100RealBean> manageData = getManageData();
...
//        适配添加设备的数据
        for (PT100RealBean b :
            ...
            bean.setDeviceName(b.getEhmpDeviceName());

            bean.setEhmId(String.valueOf(b.getEhmpId()));

            //湿度
            bean.setAlertValue(b.getEhmpMinTemp() + " ~ " + b.getEhmpMaxTemp());
            //位置
            bean.setPosition(String.valueOf(b.getEhmpDeviceAddress()));

            bean.setGallery(String.valueOf(b.getGallery()));
            //线长度
            bean.setLineLength(String.valueOf(b.getWireLength()));
...
//        设置 设备管理 中的 添加设备 列表
    ...
    public void queryDeviceManage() {
        //只执行一次
        if (addBeans.size() == 0)
            device8060Present.queryDeviceManage(PT100RealBean.class);
    }

    @Override
    public void querySelectDeviceAlert(int ehmID, String startTime, String endTime) {
        device8060Present.queryAlertData(PT100AlertBean.class, ehmID, startTime, endTime);
    }
    ...
    public void onItemClickListener(View v, int position) {
        ...
        try {
            ...
            if (v.findViewById(com.example.pool.R.id.btn_panel_device) != null) {
                //先保存数据，点击确定后在进行查询
                setEhmID(waterPressureRealBeans.get(position).getEhmpId());
                return;
            }
            //历史数据点击查看 曲线图
            if (v.findViewById(com.example.pool.R.id.new_history_name) != null) {
                ...
                for (PT100HistoryBean temp :
                        lineDataList) {

                    temps.add(temp.getEhphTemp() + "");
                    hums.add(temp.getEhphTime() + "");
                }
                ...
                intent.putExtra("markUnit", waterPressureRealBeans.get(position).getEhmpDeviceName());
                ...
                intent.putExtra("title", waterPressureRealBeans.get(position).getEhmpDeviceName());
            ...
            //设置数据
            //因为不能设置负数，因此需要去除负号
            String temp = String.valueOf(b.getEhmpTemp());
            temp = temp.replace("-", "");
            realDataUtil.setInnerData(temp, "-");
            realDataUtil.setInnerMax(String.valueOf(b.getEhmpMaxTemp()));
            //设置报警值，必须先设置最大值，再设置当前值，才不会出现问题
            realDataUtil.setOuterMax(String.valueOf(b.getEhmpMaxTemp()));
            realDataUtil.setOuterData(String.valueOf(b.getEhmpMaxTemp()));
            //设置文案
            realDataUtil.setInnerTitle("温度 ℃");
     ...
     */
    @Override
    public Class<PT100RealBean> getParseClass() {
        return PT100RealBean.class;
    }

    @Override
    public void bindHistoryData(BaseViewHolder holder, PT100HistoryBean data, int position) {
        ...
        historyTemp.setText("水流量" + data.getEhphTemp());
        historyTime.setText("时间" + data.getEhphTime());
    ...
    public void deleteDevice(int position, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        String ehmId = String.valueOf(waterPressureRealBeans.get(position).getEhmpId());
    ...
    public void withBindDeviceManage(BaseDeviceDetailManageAdapter.SVHolder svHolder, Device_Detail_AddBean data, int position) {
        ...
        TextView deviceGallery = new TextView(getActivity());
        deviceGallery.setText("设备通道：" + data.getGallery());
        textViewList.add(deviceGallery);
        ...
        TextView deviceLine = new TextView(getActivity());
        deviceLine.setText("线长度：" + data.getLineLength());
        textViewList.add(deviceLine);
     ...
     */
    @Override
    public void realItemAdapter(BaseViewHolder holder, PT100RealBean data, int position) {

        TextView item_name = holder.getView(com.example.pool.R.id.dissolved_item_name);
        item_name.setText(data.getEhmpDeviceName());
    ...
    public void changeDevice(String flag, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        ...
        PT100RealBean temp = waterPressureRealBeans.get(position);
        ...
        deviceManageEditBean.setaDeviceName(temp.getEhmpDeviceName());
        deviceManageEditBean.setActionID(temp.getEhmpId());


        deviceManageEditBean.setaDeviceInterval(temp.getIntervalTime());

        deviceManageEditBean.setTempRangeMin(temp.getEhmpMinTemp());
        deviceManageEditBean.setTempRangeMax(temp.getEhmpMaxTemp());
        deviceManageEditBean.setaDeviceLine(temp.getWireLength());
        ...
        //是否允许修改分组
        deviceManageEditBean.setShowDeviceClassify(true);
    ...
    public void addNewDevice(String flag) {
        ...
        deviceManageEditBean.setaDeviceLine(0);
    ...
    public void deterMine(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("ehmpDeviceName", getTextString(keyArray.get("deviceName")));
        formBody.put("ehmpDeviceAddress", getSpinnerString(spinnerMap.get("deviceAddress")));
        formBody.put("gallery", getTextString(keyArray.get("deviceGallery")));
        formBody.put("ehmpMaxTemp", getTextString(keyArray.get("tempMax")));
        formBody.put("ehmpMinTemp", getTextString(keyArray.get("tempMin")));
        //湿度，暂时不用
        formBody.put("ehmpMaxHum", getTextString(keyArray.get("humMax")));
        formBody.put("ehmpMinHum", getTextString(keyArray.get("humMin")));
        formBody.put("ehmpStatus", "2");
        formBody.put("wireLength", getTextString(keyArray.get("deviceLine")));
        ...
        OkHttpUtil.getInstance().sendRowOkHttp("add", LainNewApi.getInstance().getRootPath() + LainNewApi.pt100Insert, MyGson.getInstance().toJson(formBody), this);
    ...
    public void deterChange(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        //将输入的参数添加到 FormBody 中，进行发送给服务器
        formBody.put("ehmpDeviceName", getTextString(keyArray.get("deviceName")));
        formBody.put("ehmpDeviceAddress", getTextString(keyArray.get("deviceAddress")));
        formBody.put("gallery", getTextString(keyArray.get("deviceGallery")));
        formBody.put("ehmpMaxTemp", getTextString(keyArray.get("tempMax")));
        formBody.put("ehmpMinTemp", getTextString(keyArray.get("tempMin")));
        //湿度，暂时不用
        ...
        formBody.put("gId", getTextString(keyArray.get("gId")));
        formBody.put("wireLength", getTextString(keyArray.get("deviceLine")));
        formBody.put("ehmpId", getTextString(keyArray.get("actionID")));

        OkHttpUtil.getInstance().sendPutOkHttp("change", LainNewApi.getInstance().getRootPath() + LainNewApi.pt100Update, MyGson.getInstance().toJson(formBody), this);
    ...
    @Override
    public void updateRealView(TextView textView, PT100RealBean data, int position) {
        //设置列表的数据
        textView.setText(data.getEhmpDeviceName());
    }
}
