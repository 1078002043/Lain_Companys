...
import environment.bean.WindRealBean;
import environment.bean.WindAlertBean;
import environment.bean.WindHistoryBean;
import http.MyGson;
...
/**
 * @ClassName: WindFragment
 * @Description: 风速风向
 * @Author: YIN LUO FEI
 * @Date: 2020/3/28 13:23
 */
public class WindFragment extends Device8060Fragment<WindRealBean, WindAlertBean,
        WindHistoryBean, WindRealBean> implements BaseRecyclerViewAdapter.OnItemClickListener, I_DeviceDetailManageLink<Device_Detail_AddBean>, I_HistoryAdapter<WindHistoryBean>, I_RealAdapterListen<WindRealBean>, I_UpdateRealView<WindRealBean> {

    private UltimateRecyclerView waterPressureRealRecycler;

    List<WindRealBean> waterPressureRealBeans = new ArrayList<>();

    //-------------------------------------------------
    private BaseDeviceDetailManageAdapter<Device_Detail_AddBean, WindRealBean> manageAdapter;
...
//    public Temperature_Add deviceDetail_AddAdapter;
    ...
    String deleteUrl = "";
    String changeUrl = "";
    NewHistoryAdapter<WindHistoryBean> newHistoryAdapter;
    List<WindHistoryBean> lineDataList = new ArrayList<>();
    //设备管理的数据
    ...
    //设备实时数据工具类
    public RealDataUtil<WindRealBean> realDataUtil;
     ...
     */
    public WindFragment(String realLink, String alertLink, String historyLink, String manageLink) {
        super(realLink, alertLink, historyLink, manageLink);
    }

    public WindFragment(String realLink, String alertLink, String historyLink, String manageLink, String deleteUrl, String changeUrl) {
        super(realLink, alertLink, historyLink, manageLink);
...
//        changeUrl = LainNewApi.tempChange;
        ...
        //查询实时数据
        device8060Present.queryRealData(WindRealBean.class);
//        startRealUpdate();
        //设置历史数据列表
    ...
    public void queryHistoryData() {
        try {
            ...
            //默认查询第一个设备
            WindRealBean temperatureBean = waterPressureRealBeans.get(0);
            ///获取设备的ID
            
            int temEhmId = temperatureBean.getWindSpeedId();
            //结束时间
            String endTime = DateUtil.getInstance().getCurrentDate();
            //开始时间
            String startTime = DateUtil.getInstance().getStartTime();
            //开始查询
            device8060Present.queryHistoryData(WindHistoryBean.class, temEhmId, startTime, endTime);
    ...
    protected void querySelectDeviceHistory(int ehmID, String startTime, String endTime) {
        device8060Present.queryHistoryData(WindHistoryBean.class, ehmID, startTime, endTime);
    }
    ...
    public void requestReal() {
        ...
        try {
            ...
            if (alertDeviceBeanList.isEmpty()) {
                //每次查询都保存第一个设备ID
                ehmIDFirst = waterPressureRealBeans.get(0).getWindSpeedId();
                //更新报警查询的设备
                for (WindRealBean b :
                        waterPressureRealBeans) {
                    AlertDeviceBean alertB = new AlertDeviceBean();

                    alertB.setName(b.getWindSpeedName());
    ...
    public void setCurrentData(int position) {
        try {
            //点击切换上面的圆形进度条的数据
            //设置相关的属性值
            WindRealBean b = waterPressureRealBeans.get(position);

            if (b.getWindSpeedData() == currentTempValue)
                return;
    ...
    public void requestAlert() {
        ...
        List<WindAlertBean> air = getAlertData();
        alertBeans.clear();

        for (int i = 0; i < air.size(); i++) {
            WindAlertBean alertBean = air.get(i);
            AlertBeans alertBeansItem = new AlertBeans();
            alertBeansItem.setEhaInfo(alertBean.getInfo());
            alertBeansItem.setEhaTime(alertBean.getWindSpeedAlarmTime());
...
//        先清空原有的设备数据
        ...
        List<WindRealBean> manageData = getManageData();
...
//        适配添加设备的数据
        for (WindRealBean b :
            ...
            bean.setDeviceName(b.getWindSpeedName());

            bean.setEhmId(String.valueOf(b.getWindSpeedId()));
            //湿度
            bean.setAlertValue(b.getWindSpeedMinData() + " ~ " + b.getWindSpeedMaxData());

            //位置
            bean.setPosition(String.valueOf(b.getWindSpeedAddress()));
            //更新时间
...
//        设置 设备管理 中的 添加设备 列表
    ...
    public void queryDeviceManage() {
        //只执行一次
        if (addBeans.size() == 0)
            device8060Present.queryDeviceManage(WindRealBean.class);
    }

    @Override
    public void querySelectDeviceAlert(int ehmID, String startTime, String endTime) {
        device8060Present.queryAlertData(WindAlertBean.class, ehmID, startTime, endTime);
    }
    @Override
    public void onItemClickListener(View v, int position) {
        ...
        try {
            ...
            if (v.findViewById(com.example.pool.R.id.btn_panel_device) != null) {
                //先保存数据，点击确定后在进行查询
                setEhmID(waterPressureRealBeans.get(position).getWindSpeedId());
                return;
            }
            //历史数据点击查看 曲线图
            if (v.findViewById(com.example.pool.R.id.new_history_name) != null) {
                ...
                for (WindHistoryBean temp :
                        lineDataList) {

                    temps.add(temp.getWindSpeedData() + "");
                    hums.add(temp.getWindSpeedHistoryTime() + "");
                }
                ...
                intent.putExtra("markUnit",  waterPressureRealBeans.get(position).getWindSpeedName());
                ...
                intent.putExtra("title", waterPressureRealBeans.get(position).getWindSpeedName());
            ...
            WindRealBean b = waterPressureRealBeans.get(position);
            //设置数据
            realDataUtil.setInnerMax(String.valueOf(b.getWindSpeedData()));
            realDataUtil.setInnerData(String.valueOf(b.getWindSpeedMaxData()));
            //设置报警值，必须先设置最大值，再设置当前值，才不会出现问题
            realDataUtil.setOuterMax(String.valueOf(b.getWindSpeedMaxData()));
            realDataUtil.setOuterData(String.valueOf(b.getWindSpeedMaxData()));
            //设置文案
            realDataUtil.setInnerTitle("风速 m/s");
     ...
     */
    @Override
    public Class<WindRealBean> getParseClass() {
        return WindRealBean.class;
    }
    @Override
    public void bindHistoryData(BaseViewHolder holder, WindHistoryBean data, int position) {
        ...
        historyTemp.setText("水流量" + data.getWindSpeedData());
        historyTime.setText("时间" + data.getWindSpeedHistoryTime());
    ...
    public void deleteDevice(int position, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        String ehmId = String.valueOf(waterPressureRealBeans.get(position).getWindSpeedId());
     ...
     */
    @Override
    public void realItemAdapter(BaseViewHolder holder, WindRealBean data, int position) {

        TextView item_name = holder.getView(com.example.pool.R.id.dissolved_item_name);
        item_name.setText(data.getWindSpeedName());
    ...
    public void changeDevice(String flag, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        ...
        WindRealBean temp = waterPressureRealBeans.get(position);
        ...
        deviceManageEditBean.setaDeviceName(temp.getWindSpeedName());
        deviceManageEditBean.setActionID(temp.getWindSpeedId());


        deviceManageEditBean.setaDeviceInterval(temp.getIntervalTime());
//
        deviceManageEditBean.setRangeMin(temp.getWindSpeedMinData());
        deviceManageEditBean.setRangeMax(temp.getWindSpeedMaxData());
    ...
    public void deterMine(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("windSpeedName", getTextString(keyArray.get("deviceName")));
        formBody.put("windSpeedAddress", getSpinnerString(spinnerMap.get("deviceAddress")));
        formBody.put("windSpeedMaxData", getTextString(keyArray.get("rangeMax")));
        formBody.put("windSpeedMinData", getTextString(keyArray.get("rangeMin")));
        ...
        OkHttpUtil.getInstance().sendRowOkHttp("add", LainNewApi.getInstance().getRootPath() + LainNewApi.fiberInsert, MyGson.getInstance().toJson(formBody), this);
    ...
    public void deterChange(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("windSpeedMinData", getTextString(keyArray.get("rangeMin")));
        formBody.put("intervalTime", getTextString(keyArray.get("interval")));
        formBody.put("windSpeedId", getTextString(keyArray.get("actionID")));
    ...
    @Override
    public void updateRealView(TextView textView, WindRealBean data, int position) {
        //设置列表的数据
        textView.setText(data.getWindSpeedName());
    }
}
