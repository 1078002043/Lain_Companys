...
import environment.bean.OutdoorTempAlertBean;
import environment.bean.OutdoorTempHistoryBean;
import environment.bean.OutdoorTempRealBean;
import http.MyGson;
...
/**
 * @ClassName: OutdoorTempFragment
 * @Description:
 * @Author: YIN LUO FEI
 * @Date: 2020/3/28 13:59
 */
public class OutdoorTempFragment extends Device8060Fragment<OutdoorTempRealBean, OutdoorTempAlertBean,
        OutdoorTempHistoryBean, OutdoorTempRealBean> implements BaseRecyclerViewAdapter.OnItemClickListener, I_DeviceDetailManageLink<Device_Detail_AddBean>, I_HistoryAdapter<OutdoorTempHistoryBean>, I_RealAdapterListen<OutdoorTempRealBean>, I_UpdateRealView<OutdoorTempRealBean> {

    private UltimateRecyclerView waterPressureRealRecycler;

    List<OutdoorTempRealBean> waterPressureRealBeans = new ArrayList<>();

    //-------------------------------------------------
    private BaseDeviceDetailManageAdapter<Device_Detail_AddBean, OutdoorTempRealBean> manageAdapter;
...
//    public Temperature_Add deviceDetail_AddAdapter;
    ...
    String deleteUrl = "";
    String changeUrl = "";
    NewHistoryAdapter<OutdoorTempHistoryBean> newHistoryAdapter;
    List<OutdoorTempHistoryBean> lineDataList = new ArrayList<>();
    //设备管理的数据
    ...
    //设备实时数据工具类
    public RealDataUtil<OutdoorTempRealBean> realDataUtil;
     ...
     */
    public OutdoorTempFragment(String realLink, String alertLink, String historyLink, String manageLink) {
        super(realLink, alertLink, historyLink, manageLink);
    }

    public OutdoorTempFragment(String realLink, String alertLink, String historyLink, String manageLink, String deleteUrl, String changeUrl) {
        super(realLink, alertLink, historyLink, manageLink);
...
//        changeUrl = LainNewApi.tempChange;
        ...
        //查询实时数据
        device8060Present.queryRealData(OutdoorTempRealBean.class);
//        startRealUpdate();
        //设置历史数据列表
    ...
    public void queryHistoryData() {
        try {
            ...
            //默认查询第一个设备
            OutdoorTempRealBean temperatureBean = waterPressureRealBeans.get(0);
            ///获取设备的ID

            int temEhmId = temperatureBean.getOutdoorHumitureId();
            //结束时间
            String endTime = DateUtil.getInstance().getCurrentDate();
            //开始时间
            String startTime = DateUtil.getInstance().getStartTime();
            //开始查询
            device8060Present.queryHistoryData(OutdoorTempHistoryBean.class, temEhmId, startTime, endTime);
    ...
    protected void querySelectDeviceHistory(int ehmID, String startTime, String endTime) {
        device8060Present.queryHistoryData(OutdoorTempHistoryBean.class, ehmID, startTime, endTime);
    }
    ...
    public void requestReal() {
        ...
        try {
            ...
            if (alertDeviceBeanList.isEmpty()) {
                //每次查询都保存第一个设备ID
                ehmIDFirst = waterPressureRealBeans.get(0).getOutdoorHumitureId();
                //更新报警查询的设备
                for (OutdoorTempRealBean b :
                        waterPressureRealBeans) {
                    AlertDeviceBean alertB = new AlertDeviceBean();

                    alertB.setName(b.getOutdoorHumitureDeviceName());
    ...
    public void setCurrentData(int position) {
        try {
            //点击切换上面的圆形进度条的数据
            //设置相关的属性值
            OutdoorTempRealBean b = waterPressureRealBeans.get(position);

            if (b.getOutdoorHumitureHum() == currentTempValue && b.getOutdoorHumitureTem() == currentHumValue)
                return;
    ...
    public void requestAlert() {
        ...
        List<OutdoorTempAlertBean> air = getAlertData();
        alertBeans.clear();

        for (int i = 0; i < air.size(); i++) {
            OutdoorTempAlertBean alertBean = air.get(i);
            AlertBeans alertBeansItem = new AlertBeans();
            alertBeansItem.setEhaInfo(alertBean.getOutdoorHumitureAlarmInfo());
            alertBeansItem.setEhaTime(alertBean.getOutdoorHumitureAlarmTime());
            alertBeansItem.setEcmDeviceName(alertBean.getOutdoorHumitureName());
...
//        先清空原有的设备数据
        ...
        List<OutdoorTempRealBean> manageData = getManageData();
...
//        适配添加设备的数据
        for (OutdoorTempRealBean b :
            ...
            bean.setDeviceName(b.getOutdoorHumitureDeviceName());

            bean.setEhmId(String.valueOf(b.getOutdoorHumitureId()));
            //湿度
            bean.setHumidityMin(String.valueOf(b.getOutdoorHumitureMinHum()));
            bean.setHumidityMax(String.valueOf(b.getOutdoorHumitureMaxHum()));
            //温度
            bean.setTemperatureMax(String.valueOf(b.getOutdoorHumitureMaxTem()));
            bean.setTemperatureMin(String.valueOf(b.getOutdoorHumitureMinTem()));
            //位置
            bean.setPosition(String.valueOf(b.getOutdoorHumitureDeviceAddress()));
            //更新时间
...
//        设置 设备管理 中的 添加设备 列表
    ...
    public void queryDeviceManage() {
        //只执行一次
        if (addBeans.size() == 0)
            device8060Present.queryDeviceManage(OutdoorTempRealBean.class);
    }

    @Override
    public void querySelectDeviceAlert(int ehmID, String startTime, String endTime) {
        device8060Present.queryAlertData(OutdoorTempAlertBean.class, ehmID, startTime, endTime);
    }
    ...
    public void onItemClickListener(View v, int position) {
        ...
        try {
            ...
            if (v.findViewById(com.example.pool.R.id.btn_panel_device) != null) {
                //先保存数据，点击确定后在进行查询
                setEhmID(waterPressureRealBeans.get(position).getOutdoorHumitureId());
                return;
            }
            //历史数据点击查看 曲线图
            if (v.findViewById(com.example.pool.R.id.new_history_name) != null) {
                ...
                for (OutdoorTempHistoryBean temp :
                        lineDataList) {
                    temps.add(temp.getOutdoorHumitureHum() + "");
                    hums.add(temp.getOutdoorHumitureTem() + "");
                    times.add(temp.getOutdoorHumitureHistoryTime() + "");
                }
                ...
                intent.putExtra("markUnit", waterPressureRealBeans.get(position).getOutdoorHumitureDeviceName());
                ...
                intent.putExtra("title", waterPressureRealBeans.get(position).getOutdoorHumitureDeviceName());
            ...
            OutdoorTempRealBean b = waterPressureRealBeans.get(position);
            //设置数据
            realDataUtil.setInnerMax(String.valueOf(b.getOutdoorHumitureMaxHum()));
            realDataUtil.setInnerData(String.valueOf(b.getOutdoorHumitureHum()));
            //设置报警值，必须先设置最大值，再设置当前值，才不会出现问题
            realDataUtil.setOuterMax(String.valueOf(b.getOutdoorHumitureMaxTem()));
            realDataUtil.setOuterData(String.valueOf(b.getOutdoorHumitureTem()));
            //设置文案
            realDataUtil.setInnerTitle("湿度 %RH");
     ...
     */
    @Override
    public Class<OutdoorTempRealBean> getParseClass() {
        return OutdoorTempRealBean.class;
    }

    @Override
    public void bindHistoryData(BaseViewHolder holder, OutdoorTempHistoryBean data, int position) {
        ...
        historyTemp.setText("水流量" + data.getOutdoorHumitureHum());
        historyTime.setText("时间" + data.getOutdoorHumitureHistoryTime());
    ...
    public void deleteDevice(int position, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        String ehmId = String.valueOf(waterPressureRealBeans.get(position).getOutdoorHumitureId());
     ...
     */
    @Override
    public void realItemAdapter(BaseViewHolder holder, OutdoorTempRealBean data, int position) {

        TextView item_name = holder.getView(com.example.pool.R.id.dissolved_item_name);
        item_name.setText(data.getOutdoorHumitureDeviceName());
    ...
    public void changeDevice(String flag, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        ...
        OutdoorTempRealBean temp = waterPressureRealBeans.get(position);
        ...
        deviceManageEditBean.setaDeviceName(temp.getOutdoorHumitureDeviceName());
        deviceManageEditBean.setActionID(temp.getOutdoorHumitureId());


        deviceManageEditBean.setaDeviceInterval(temp.getIntervalTime());
//
        deviceManageEditBean.setTempRangeMax(temp.getOutdoorHumitureMaxTem());
        deviceManageEditBean.setTempRangeMin(temp.getOutdoorHumitureMinTem());

        deviceManageEditBean.setHumRangeMax(temp.getOutdoorHumitureMaxHum());
        deviceManageEditBean.setHumRangeMin(temp.getOutdoorHumitureMinHum());
    ...
    public void deterMine(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("outdoorHumitureDeviceAddress", getSpinnerString(spinnerMap.get("deviceAddress")));
        formBody.put("outdoorHumitureDeviceName", getTextString(keyArray.get("deviceName")));
        formBody.put("outdoorHumitureMaxTem", getTextString(keyArray.get("tempMax")));
        formBody.put("outdoorHumitureMinTem", getTextString(keyArray.get("tempMin")));
        formBody.put("outdoorHumitureMaxHum", getTextString(keyArray.get("humMax")));
        formBody.put("outdoorHumitureMinHum", getTextString(keyArray.get("humMin")));
        formBody.put("intervalTime", getTextString(keyArray.get("interval")));
        formBody.put("diId", getTextString(keyArray.get("diId")));

        OkHttpUtil.getInstance().sendRowOkHttp("add", LainNewApi.getInstance().getRootPath() + LainNewApi.outdoorTempInsert, MyGson.getInstance().toJson(formBody), this);
    ...
    public void deterChange(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("outdoorHumitureId", getTextString(keyArray.get("actionID")));
    ...
    @Override
    public void updateRealView(TextView textView, OutdoorTempRealBean data, int position) {
        //设置列表的数据
        textView.setText(data.getOutdoorHumitureDeviceName());
    }
}
