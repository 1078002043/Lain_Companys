...
import environment.bean.WaterproofRealBean;
import environment.bean.WaterproofAlertBean;
import environment.bean.WaterproofHistoryBean;
import http.MyGson;
...
/**
 * @ClassName: WaterproofFragment
 * @Description: 室外防水温湿
 * @Author: YIN LUO FEI
 * @Date: 2020/3/28 13:58
 */
public class WaterproofFragment extends Device8060Fragment<WaterproofRealBean, WaterproofAlertBean,
        WaterproofHistoryBean, WaterproofRealBean> implements BaseRecyclerViewAdapter.OnItemClickListener, I_DeviceDetailManageLink<Device_Detail_AddBean>, I_HistoryAdapter<WaterproofHistoryBean>, I_RealAdapterListen<WaterproofRealBean>, I_UpdateRealView<WaterproofRealBean> {

    private UltimateRecyclerView waterPressureRealRecycler;

    List<WaterproofRealBean> waterPressureRealBeans = new ArrayList<>();

    //-------------------------------------------------
    private BaseDeviceDetailManageAdapter<Device_Detail_AddBean, WaterproofRealBean> manageAdapter;
...
//    public Temperature_Add deviceDetail_AddAdapter;
    ...
    String deleteUrl = "";
    String changeUrl = "";
    NewHistoryAdapter<WaterproofHistoryBean> newHistoryAdapter;
    List<WaterproofHistoryBean> lineDataList = new ArrayList<>();
    //设备管理的数据
    ...
    //设备实时数据工具类
    public RealDataUtil<WaterproofRealBean> realDataUtil;
     ...
     */
    public WaterproofFragment(String realLink, String alertLink, String historyLink, String manageLink) {
        super(realLink, alertLink, historyLink, manageLink);
    }

    public WaterproofFragment(String realLink, String alertLink, String historyLink, String manageLink, String deleteUrl, String changeUrl) {
        super(realLink, alertLink, historyLink, manageLink);
...
//        changeUrl = LainNewApi.tempChange;
        ...
        //查询实时数据
        device8060Present.queryRealData(WaterproofRealBean.class);
//        startRealUpdate();
        //设置历史数据列表
    ...
    public void queryHistoryData() {
        try {
            ...
            //默认查询第一个设备
            WaterproofRealBean temperatureBean = waterPressureRealBeans.get(0);
            ///获取设备的ID
            ...
            //开始查询
            device8060Present.queryHistoryData(WaterproofHistoryBean.class, temEhmId, startTime, endTime);
    ...
    protected void querySelectDeviceHistory(int ehmID, String startTime, String endTime) {
        device8060Present.queryHistoryData(WaterproofHistoryBean.class, ehmID, startTime, endTime);
    }
    ...
    public void setCurrentData(int position) {
        try {
            //点击切换上面的圆形进度条的数据
            //设置相关的属性值
            WaterproofRealBean b = waterPressureRealBeans.get(position);
    ...
    public void requestAlert() {
        ...
        List<WaterproofAlertBean> air = getAlertData();
        alertBeans.clear();

        for (int i = 0; i < air.size(); i++) {
            WaterproofAlertBean alertBean = air.get(i);
...
//        先清空原有的设备数据
        ...
        List<WaterproofRealBean> manageData = getManageData();
...
//        设置 设备管理 中的 添加设备 列表
    ...
    public void queryDeviceManage() {
        //只执行一次
        if (addBeans.size() == 0)
            device8060Present.queryDeviceManage(WaterproofRealBean.class);
    }

    @Override
    public void querySelectDeviceAlert(int ehmID, String startTime, String endTime) {
        device8060Present.queryAlertData(WaterproofAlertBean.class, ehmID, startTime, endTime);
    }
    ...
    public void onItemClickListener(View v, int position) {
        ...
        try {
            ...
            if (v.findViewById(com.example.pool.R.id.new_history_name) != null) {
                ...
                for (WaterproofHistoryBean temp :
                        lineDataList) {
     ...
     */
    @Override
    public Class<WaterproofRealBean> getParseClass() {
        return WaterproofRealBean.class;
    }

    @Override
    public void bindHistoryData(BaseViewHolder holder, WaterproofHistoryBean data, int position) {
     ...
     */
    @Override
    public void realItemAdapter(BaseViewHolder holder, WaterproofRealBean data, int position) {
    ...
    public void changeDevice(String flag, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        ...
        WaterproofRealBean temp = waterPressureRealBeans.get(position);
...
//
    ...
    @Override
    public void updateRealView(TextView textView, WaterproofRealBean data, int position) {
        //设置列表的数据
