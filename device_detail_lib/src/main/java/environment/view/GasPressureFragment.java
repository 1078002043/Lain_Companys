...
import environment.bean.GasPressureAlertBean;
import environment.bean.GasPressureHistoryBean;
import environment.bean.GasPressureRealBean;
import http.MyGson;
...
/**
 * @ClassName: GasPressureFragment
 * @Description: 气压强度
 * @Author: YIN LUO FEI
 * @Date: 2020/3/28 10:33
 */
public class GasPressureFragment extends Device8060Fragment<GasPressureRealBean, GasPressureAlertBean,
        GasPressureHistoryBean, GasPressureRealBean> implements BaseRecyclerViewAdapter.OnItemClickListener, I_DeviceDetailManageLink<Device_Detail_AddBean>, I_HistoryAdapter<GasPressureHistoryBean>, I_RealAdapterListen<GasPressureRealBean>,
        I_UpdateRealView<GasPressureRealBean> {

    private UltimateRecyclerView waterPressureRealRecycler;

    List<GasPressureRealBean> waterPressureRealBeans = new ArrayList<>();

    //-------------------------------------------------
    private BaseDeviceDetailManageAdapter<Device_Detail_AddBean, GasPressureRealBean> manageAdapter;
...
//    public Temperature_Add deviceDetail_AddAdapter;
    ...
    String deleteUrl = "";
    String changeUrl = "";
    NewHistoryAdapter<GasPressureHistoryBean> newHistoryAdapter;
    List<GasPressureHistoryBean> lineDataList = new ArrayList<>();
    //设备管理的数据
    ...
    //设备实时数据工具类
    public RealDataUtil<GasPressureRealBean> realDataUtil;
     ...
     */
    public GasPressureFragment(String realLink, String alertLink, String historyLink, String manageLink) {
        super(realLink, alertLink, historyLink, manageLink);
    }

    public GasPressureFragment(String realLink, String alertLink, String historyLink, String manageLink, String deleteUrl, String changeUrl) {
        super(realLink, alertLink, historyLink, manageLink);
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        //查询实时数据
        device8060Present.queryRealData(GasPressureRealBean.class);
//        startRealUpdate();
        //设置历史数据列表
    ...
    public void queryHistoryData() {
        try {
            ...
            //默认查询第一个设备
            GasPressureRealBean temperatureBean = waterPressureRealBeans.get(0);
            ///获取设备的ID
            
            int temEhmId = temperatureBean.getGasPressureId();
            //结束时间
            String endTime = DateUtil.getInstance().getCurrentDate();
            //开始时间
            String startTime = DateUtil.getInstance().getStartTime();
            //开始查询
            device8060Present.queryHistoryData(GasPressureHistoryBean.class, temEhmId, startTime, endTime);
    ...
    protected void querySelectDeviceHistory(int ehmID, String startTime, String endTime) {
        device8060Present.queryHistoryData(GasPressureHistoryBean.class, ehmID, startTime, endTime);
    }
    ...
    public void requestReal() {
        ...
        try {
            ...
            if (alertDeviceBeanList.isEmpty()) {
                //每次查询都保存第一个设备ID
                ehmIDFirst = waterPressureRealBeans.get(0).getGasPressureId();
                //更新报警查询的设备
                for (GasPressureRealBean b :
                        waterPressureRealBeans) {
                    AlertDeviceBean alertB = new AlertDeviceBean();

                    alertB.setName(b.getGasPressureName());
    ...
    public void setCurrentData(int position) {
        try {
            //点击切换上面的圆形进度条的数据
            //设置相关的属性值
            GasPressureRealBean b = waterPressureRealBeans.get(position);

            if (b.getGasPressureData() == currentTempValue)
                return;
    ...
    public void requestAlert() {
        ...
        List<GasPressureAlertBean> air = getAlertData();
        alertBeans.clear();

        for (int i = 0; i < air.size(); i++) {
            GasPressureAlertBean alertBean = air.get(i);
            AlertBeans alertBeansItem = new AlertBeans();
            alertBeansItem.setEhaInfo(alertBean.getInfo());
            alertBeansItem.setEhaTime(alertBean.getGasPressureAlarmTime());
...
//        先清空原有的设备数据
        ...
        List<GasPressureRealBean> manageData = getManageData();
...
//        适配添加设备的数据
        for (GasPressureRealBean b :
            ...
            bean.setDeviceName(b.getGasPressureName());

            bean.setEhmId(String.valueOf(b.getGasPressureId()));
            //湿度
            bean.setAlertValue(b.getGasPressureMinData() + " ~ " + b.getGasPressureMaxData());

            //位置
            bean.setPosition(String.valueOf(b.getGasPressureAddress()));
            //更新时间
...
//        设置 设备管理 中的 添加设备 列表
    ...
    public void queryDeviceManage() {
        //只执行一次
        if (addBeans.size() == 0)
            device8060Present.queryDeviceManage(GasPressureRealBean.class);
    }

    @Override
    public void querySelectDeviceAlert(int ehmID, String startTime, String endTime) {
        device8060Present.queryAlertData(GasPressureAlertBean.class, ehmID, startTime, endTime);
    }
    @Override
    public void onItemClickListener(View v, int position) {
        ...
        try {
            ...
            if (v.findViewById(com.example.pool.R.id.btn_panel_device) != null) {
                //先保存数据，点击确定后在进行查询
                setEhmID(waterPressureRealBeans.get(position).getGasPressureId());
                return;
            }
            //历史数据点击查看 曲线图
            if (v.findViewById(com.example.pool.R.id.new_history_name) != null) {
                ...
                for (GasPressureHistoryBean temp :
                        lineDataList) {

                    temps.add(temp.getGasPressureData() + "");
                    hums.add(temp.getGasPressureHistoryTime() + "");
                }
                ...
                intent.putExtra("markUnit",  waterPressureRealBeans.get(position).getGasPressureName());
                ...
                intent.putExtra("title", waterPressureRealBeans.get(position).getGasPressureName());
            ...
            GasPressureRealBean b = waterPressureRealBeans.get(position);
            //设置数据
            realDataUtil.setInnerData(String.valueOf(b.getGasPressureData()));
            realDataUtil.setInnerMax(String.valueOf(b.getGasPressureMaxData()));
            //设置报警值，必须先设置最大值，再设置当前值，才不会出现问题
            realDataUtil.setOuterMax(String.valueOf(b.getGasPressureMaxData()));
            realDataUtil.setOuterData(String.valueOf(b.getGasPressureMaxData()));
            //设置文案
            realDataUtil.setInnerTitle("压强 Pa");
     ...
     */
    @Override
    public Class<GasPressureRealBean> getParseClass() {
        return GasPressureRealBean.class;
    }
    @Override
    public void bindHistoryData(BaseViewHolder holder, GasPressureHistoryBean data, int position) {
        ...
        historyTemp.setText("水流量" + data.getGasPressureData());
        historyTime.setText("时间" + data.getGasPressureHistoryTime());
    ...
    public void deleteDevice(int position, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        String ehmId = String.valueOf(waterPressureRealBeans.get(position).getGasPressureId());
     ...
     */
    @Override
    public void realItemAdapter(BaseViewHolder holder, GasPressureRealBean data, int position) {

        TextView item_name = holder.getView(com.example.pool.R.id.dissolved_item_name);
        item_name.setText(data.getGasPressureName());
    ...
    public void changeDevice(String flag, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        ...
        GasPressureRealBean temp = waterPressureRealBeans.get(position);
        ...
        deviceManageEditBean.setaDeviceName(temp.getGasPressureName());
        deviceManageEditBean.setActionID(temp.getGasPressureId());


        deviceManageEditBean.setaDeviceInterval(temp.getIntervalTime());
//
        deviceManageEditBean.setRangeMin(temp.getGasPressureMinData());
        deviceManageEditBean.setRangeMax(temp.getGasPressureMaxData());
    ...
    public void deterMine(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("gasPressureName", getTextString(keyArray.get("deviceName")));
        formBody.put("gasPressureAddress", getSpinnerString(spinnerMap.get("deviceAddress")));
        formBody.put("gasPressureMaxData", getTextString(keyArray.get("rangeMax")));
        formBody.put("gasPressureMinData", getTextString(keyArray.get("rangeMin")));
        ...
        OkHttpUtil.getInstance().sendRowOkHttp("add", LainNewApi.getInstance().getRootPath() + LainNewApi.gasPressureInsert, MyGson.getInstance().toJson(formBody), this);
    ...
    public void deterChange(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("gasPressureMinData", getTextString(keyArray.get("rangeMin")));
        formBody.put("intervalTime", getTextString(keyArray.get("interval")));
        formBody.put("gasPressureId", getTextString(keyArray.get("actionID")));

        OkHttpUtil.getInstance().sendPutOkHttp("change", LainNewApi.getInstance().getRootPath() + LainNewApi.gasPressureUpdate, MyGson.getInstance().toJson(formBody), this);
    ...
    @Override
    public void updateRealView(TextView textView, GasPressureRealBean data, int position) {
        //设置列表的数据
        textView.setText(data.getGasPressureName());
    }
}
