...
import environment.bean.ElectricalFireAlertBean;
import environment.bean.ElectricalFireHistoryBean;
import environment.bean.ElectricalFireRealBean;
import http.MyGson;
...
/**
 * @ClassName: ElectricalFireFragment
 * @Description: 剩余电流
 * @Author: YIN LUO FEI
 * @Date: 2020/3/28 13:10
 */
public class ElectricalFireFragment extends Device8060Fragment<ElectricalFireRealBean, ElectricalFireAlertBean,
        ElectricalFireHistoryBean, ElectricalFireRealBean> implements BaseRecyclerViewAdapter.OnItemClickListener, I_DeviceDetailManageLink<Device_Detail_AddBean>, I_HistoryAdapter<ElectricalFireHistoryBean>, I_RealAdapterListen<ElectricalFireRealBean>, I_UpdateRealView<ElectricalFireRealBean> {

    private UltimateRecyclerView waterPressureRealRecycler;

    List<ElectricalFireRealBean> waterPressureRealBeans = new ArrayList<>();

    //-------------------------------------------------
    private BaseDeviceDetailManageAdapter<Device_Detail_AddBean, ElectricalFireRealBean> manageAdapter;
...
//    public Temperature_Add deviceDetail_AddAdapter;
    ...
    String deleteUrl = "";
    String changeUrl = "";
    NewHistoryAdapter<ElectricalFireHistoryBean> newHistoryAdapter;
    List<ElectricalFireHistoryBean> lineDataList = new ArrayList<>();
    //设备管理的数据
    ...
    //设备实时数据工具类
    public RealDataUtil<ElectricalFireRealBean> realDataUtil;
     ...
     */
    public ElectricalFireFragment(String realLink, String alertLink, String historyLink, String manageLink) {
        super(realLink, alertLink, historyLink, manageLink);
    }

    public ElectricalFireFragment(String realLink, String alertLink, String historyLink, String manageLink, String deleteUrl, String changeUrl) {
        super(realLink, alertLink, historyLink, manageLink);
...
//        changeUrl = LainNewApi.tempChange;
        ...
        //查询实时数据
        device8060Present.queryRealData(ElectricalFireRealBean.class);
//        startRealUpdate();
        //设置历史数据列表
    ...
    public void queryHistoryData() {
        try {
            ...
            //默认查询第一个设备
            ElectricalFireRealBean temperatureBean = waterPressureRealBeans.get(0);
            ///获取设备的ID

            int temEhmId = temperatureBean.getElectricalFireId();
            //结束时间
            String endTime = DateUtil.getInstance().getCurrentDate();
            //开始时间
            String startTime = DateUtil.getInstance().getStartTime();
            //开始查询
            device8060Present.queryHistoryData(ElectricalFireHistoryBean.class, temEhmId, startTime, endTime);
    ...
    protected void querySelectDeviceHistory(int ehmID, String startTime, String endTime) {
        device8060Present.queryHistoryData(ElectricalFireHistoryBean.class, ehmID, startTime, endTime);
    }
    ...
    public void requestReal() {
        ...
        try {
            ...
            if (alertDeviceBeanList.isEmpty()) {
                //每次查询都保存第一个设备ID
                ehmIDFirst = waterPressureRealBeans.get(0).getElectricalFireId();
                //更新报警查询的设备
                for (ElectricalFireRealBean b :
                        waterPressureRealBeans) {
                    AlertDeviceBean alertB = new AlertDeviceBean();

                    alertB.setName(b.getElectricalFireName());
    ...
    public void setCurrentData(int position) {
        try {
            //点击切换上面的圆形进度条的数据
            //设置相关的属性值
            ElectricalFireRealBean b = waterPressureRealBeans.get(position);

            if (b.getCurrent() == currentTempValue && b.getTemp() == currentHumValue)
                return;
    ...
    public void requestAlert() {
        ...
        List<ElectricalFireAlertBean> air = getAlertData();
        alertBeans.clear();

        for (int i = 0; i < air.size(); i++) {
            ElectricalFireAlertBean alertBean = air.get(i);
            AlertBeans alertBeansItem = new AlertBeans();
            alertBeansItem.setEhaInfo(alertBean.getInfo());
            alertBeansItem.setEhaTime(alertBean.getElectricalFireAlarmTime());
            alertBeansItem.setEcmDeviceName(alertBean.getElectricalFireName());
...
//        先清空原有的设备数据
        ...
        List<ElectricalFireRealBean> manageData = getManageData();
...
//        适配添加设备的数据
        for (ElectricalFireRealBean b :
            ...
            bean.setDeviceName(b.getElectricalFireName());

            bean.setEhmId(String.valueOf(b.getElectricalFireId()));
            //湿度
            bean.setAlertValue(b.getMinTemp() + " ~ " + b.getMaxTemp());

            //位置
            bean.setPosition(String.valueOf(b.getElectricalFireAddress()));
            //更新时间
...
//        设置 设备管理 中的 添加设备 列表
    ...
    public void queryDeviceManage() {
        //只执行一次
        if (addBeans.size() == 0)
            device8060Present.queryDeviceManage(ElectricalFireRealBean.class);
    }

    @Override
    public void querySelectDeviceAlert(int ehmID, String startTime, String endTime) {
        device8060Present.queryAlertData(ElectricalFireAlertBean.class, ehmID, startTime, endTime);
    }
    ...
    public void onItemClickListener(View v, int position) {
        ...
        try {
            ...
            if (v.findViewById(com.example.pool.R.id.btn_panel_device) != null) {
                //先保存数据，点击确定后在进行查询
                setEhmID(waterPressureRealBeans.get(position).getElectricalFireId());
                return;
            }
            //历史数据点击查看 曲线图
            if (v.findViewById(com.example.pool.R.id.new_history_name) != null) {
                ...
                for (ElectricalFireHistoryBean temp :
                        lineDataList) {
                    temps.add(temp.getCurrent() + "");
                    hums.add(temp.getTemp() + "");
                    times.add(temp.getElectricalFireHistoryTime() + "");
                }
                ...
                intent.putExtra("markUnit", waterPressureRealBeans.get(position).getElectricalFireName());
                ...
                intent.putExtra("title", waterPressureRealBeans.get(position).getElectricalFireName());
            ...
            ElectricalFireRealBean b = waterPressureRealBeans.get(position);
            //设置数据
            realDataUtil.setInnerMax(String.valueOf(b.getMaxCurrent()));
            realDataUtil.setInnerData(String.valueOf(b.getCurrent()));
            ...
            //设置文案
            realDataUtil.setInnerTitle("电流 mA");
     ...
     */
    @Override
    public Class<ElectricalFireRealBean> getParseClass() {
        return ElectricalFireRealBean.class;
    }

    @Override
    public void bindHistoryData(BaseViewHolder holder, ElectricalFireHistoryBean data, int position) {
        ...
        historyTemp.setText("水流量" + data.getCurrent());
        historyTime.setText("时间" + data.getElectricalFireHistoryTime());
    ...
    public void deleteDevice(int position, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        String ehmId = String.valueOf(waterPressureRealBeans.get(position).getElectricalFireId());
     ...
     */
    @Override
    public void realItemAdapter(BaseViewHolder holder, ElectricalFireRealBean data, int position) {

        TextView item_name = holder.getView(com.example.pool.R.id.dissolved_item_name);
        item_name.setText(data.getElectricalFireName());
    ...
    public void changeDevice(String flag, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        ...
        ElectricalFireRealBean temp = waterPressureRealBeans.get(position);
        ...
        deviceManageEditBean.setaDeviceName(temp.getElectricalFireName());
        deviceManageEditBean.setActionID(temp.getElectricalFireId());
...
//
        ...
        //剩于电流
        deviceManageEditBean.setElectricMax(temp.getMaxCurrent());
        deviceManageEditBean.setElectricMin(temp.getMinCurrent());
    ...
    public void addNewDevice(String flag) {
        ...
        //剩于电流
        deviceManageEditBean.setElectricMax(0);
    ...
    public void deterMine(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("electricalFireName", getTextString(keyArray.get("deviceName")));
        formBody.put("electricalFireAddress", getSpinnerString(spinnerMap.get("deviceAddress")));
        formBody.put("maxTemp", getTextString(keyArray.get("tempMax")));
        formBody.put("minTemp", getTextString(keyArray.get("tempMin")));
        //剩下电流
        formBody.put("minCurrent", getTextString(keyArray.get("electricMin")));
        formBody.put("maxCurrent", getTextString(keyArray.get("electricMax")));
        ...
        OkHttpUtil.getInstance().sendRowOkHttp("add", LainNewApi.getInstance().getRootPath() + LainNewApi.electricalInsert, MyGson.getInstance().toJson(formBody), this);
    ...
    public void deterChange(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("intervalTime", getTextString(keyArray.get("interval")));
        formBody.put("electricalFireId", getTextString(keyArray.get("actionID")));
    ...
    @Override
    public void updateRealView(TextView textView, ElectricalFireRealBean data, int position) {
        //设置列表的数据
        textView.setText(data.getElectricalFireName());
    }
}
