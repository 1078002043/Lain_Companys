...
import environment.bean.PatchTempAlertBean;
import environment.bean.PatchTempHistoryBean;
import environment.bean.PatchTempRealBean;
import http.MyGson;
...
/**
 * @ClassName: PatchTempFragment
 * @Description: 贴片式温度
 * @Author: YIN LUO FEI
 * @Date: 2020/3/28 11:08
 */
public class PatchTempFragment extends Device8060Fragment<PatchTempRealBean, PatchTempAlertBean,
        PatchTempHistoryBean, PatchTempRealBean> implements BaseRecyclerViewAdapter.OnItemClickListener, I_DeviceDetailManageLink<Device_Detail_AddBean>, I_HistoryAdapter<PatchTempHistoryBean>, I_RealAdapterListen<PatchTempRealBean>,
        I_UpdateRealView<PatchTempRealBean> {

    private UltimateRecyclerView waterPressureRealRecycler;

    List<PatchTempRealBean> waterPressureRealBeans = new ArrayList<>();

    //-------------------------------------------------
    private BaseDeviceDetailManageAdapter<Device_Detail_AddBean, PatchTempRealBean> manageAdapter;
...
//    public Temperature_Add deviceDetail_AddAdapter;
    ...
    //设备实时数据工具类
    public RealDataUtil<PatchTempRealBean> realDataUtil;

    //删除URL
    String deleteUrl = "";
    String changeUrl = "";
    NewHistoryAdapter<PatchTempHistoryBean> newHistoryAdapter;
    List<PatchTempHistoryBean> lineDataList = new ArrayList<>();
    //设备管理的数据
     ...
     */
    public PatchTempFragment(String realLink, String alertLink, String historyLink, String manageLink) {
        super(realLink, alertLink, historyLink, manageLink);
    }

    public PatchTempFragment(String realLink, String alertLink, String historyLink, String manageLink, String deleteUrl, String changeUrl) {
        super(realLink, alertLink, historyLink, manageLink);
...
//        changeUrl = LainNewApi.tempChange;
        ...
        //查询实时数据
        device8060Present.queryRealData(PatchTempRealBean.class);
//        startRealUpdate();
        //设置历史数据列表
    ...
    public void queryHistoryData() {
        try {
            ...
            //默认查询第一个设备
            PatchTempRealBean temperatureBean = waterPressureRealBeans.get(0);
            ///获取设备的ID
            
            int temEhmId = temperatureBean.getPatchId();
            //结束时间
            String endTime = DateUtil.getInstance().getCurrentDate();
            //开始时间
            String startTime = DateUtil.getInstance().getStartTime();
            //开始查询
            device8060Present.queryHistoryData(PatchTempHistoryBean.class, temEhmId, startTime, endTime);
    ...
    protected void querySelectDeviceHistory(int ehmID, String startTime, String endTime) {
        device8060Present.queryHistoryData(PatchTempHistoryBean.class, ehmID, startTime, endTime);
    }
    ...
    public void requestReal() {
        ...
        try {
            ...
            if (alertDeviceBeanList.isEmpty()) {
                //每次查询都保存第一个设备ID
                ehmIDFirst = waterPressureRealBeans.get(0).getPatchId();
                //更新报警查询的设备
                for (PatchTempRealBean b :
                        waterPressureRealBeans) {
                    AlertDeviceBean alertB = new AlertDeviceBean();

                    alertB.setName(b.getPatchName());
    ...
    public void setCurrentData(int position) {
        try {
            //点击切换上面的圆形进度条的数据
            //设置相关的属性值
            PatchTempRealBean b = waterPressureRealBeans.get(position);

            if (b.getPatchData() == currentTempValue)
                return;
    ...
    public void requestAlert() {
        ...
        List<PatchTempAlertBean> air = getAlertData();
        alertBeans.clear();

        for (int i = 0; i < air.size(); i++) {
            PatchTempAlertBean alertBean = air.get(i);
            AlertBeans alertBeansItem = new AlertBeans();
            alertBeansItem.setEhaInfo(alertBean.getInfo());
            alertBeansItem.setEhaTime(alertBean.getPatchAlarmTime());
...
//        先清空原有的设备数据
        ...
        List<PatchTempRealBean> manageData = getManageData();
...
//        适配添加设备的数据
        for (PatchTempRealBean b :
            ...
            bean.setDeviceName(b.getPatchName());

            bean.setEhmId(String.valueOf(b.getPatchId()));
            //湿度
            bean.setAlertValue(b.getPatchMinData() + " ~ " + b.getPatchMaxData());

            //位置
            bean.setPosition(String.valueOf(b.getPatchAddress()));
            //更新时间
...
//        设置 设备管理 中的 添加设备 列表
    ...
    public void queryDeviceManage() {
        //只执行一次
        if (addBeans.size() == 0)
            device8060Present.queryDeviceManage(PatchTempRealBean.class);
    }

    @Override
    public void querySelectDeviceAlert(int ehmID, String startTime, String endTime) {
        device8060Present.queryAlertData(PatchTempAlertBean.class, ehmID, startTime, endTime);
    }
    @Override
    public void onItemClickListener(View v, int position) {
        ...
        try {
            ...
            if (v.findViewById(com.example.pool.R.id.btn_panel_device) != null) {
                //先保存数据，点击确定后在进行查询
                setEhmID(waterPressureRealBeans.get(position).getPatchId());
                return;
            }
            //历史数据点击查看 曲线图
            if (v.findViewById(com.example.pool.R.id.new_history_name) != null) {
                ...
                for (PatchTempHistoryBean temp :
                        lineDataList) {

                    temps.add(temp.getPatchData() + "");
                    hums.add(temp.getPatchHistoryTime() + "");
                }
                ...
                intent.putExtra("markUnit",  waterPressureRealBeans.get(position).getPatchName());
                ...
                intent.putExtra("title", waterPressureRealBeans.get(position).getPatchName());
            ...
            //设置数据
            realDataUtil.setInnerData(String.valueOf(b.getPatchData()));
            realDataUtil.setInnerMax(String.valueOf(b.getPatchMaxData()));
            //设置报警值，必须先设置最大值，再设置当前值，才不会出现问题
            realDataUtil.setOuterMax(String.valueOf(b.getPatchMaxData()));
            realDataUtil.setOuterData(String.valueOf(b.getPatchMaxData()));
            //设置文案
     ...
     */
    @Override
    public Class<PatchTempRealBean> getParseClass() {
        return PatchTempRealBean.class;
    }
    @Override
    public void bindHistoryData(BaseViewHolder holder, PatchTempHistoryBean data, int position) {
        ...
        historyTemp.setText("水流量" + data.getPatchData());
        historyTime.setText("时间" + data.getPatchHistoryTime());
    ...
    public void deleteDevice(int position, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        String ehmId = String.valueOf(waterPressureRealBeans.get(position).getPatchId());
     ...
     */
    @Override
    public void realItemAdapter(BaseViewHolder holder, PatchTempRealBean data, int position) {

        TextView item_name = holder.getView(com.example.pool.R.id.dissolved_item_name);
        item_name.setText(data.getPatchName());
    ...
    public void changeDevice(String flag, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        ...
        PatchTempRealBean temp = waterPressureRealBeans.get(position);
        ...
        deviceManageEditBean.setaDeviceName(temp.getPatchName());
        deviceManageEditBean.setActionID(temp.getPatchId());


        deviceManageEditBean.setaDeviceInterval(temp.getIntervalTime());
//
        deviceManageEditBean.setRangeMin(temp.getPatchMinData());
        deviceManageEditBean.setRangeMax(temp.getPatchMaxData());
    ...
    public void deterMine(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("patchName", getTextString(keyArray.get("deviceName")));
        formBody.put("patchAddress", getSpinnerString(spinnerMap.get("deviceAddress")));
        formBody.put("patchMaxData", getTextString(keyArray.get("rangeMax")));
        formBody.put("patchMinData", getTextString(keyArray.get("rangeMin")));
        ...
        OkHttpUtil.getInstance().sendRowOkHttp("add", LainNewApi.getInstance().getRootPath() + LainNewApi.patchTempInsert, MyGson.getInstance().toJson(formBody), this);
    ...
    public void deterChange(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("patchMinData", getTextString(keyArray.get("rangeMin")));
        formBody.put("intervalTime", getTextString(keyArray.get("interval")));
        formBody.put("patchId", getTextString(keyArray.get("actionID")));

        OkHttpUtil.getInstance().sendPutOkHttp("change", LainNewApi.getInstance().getRootPath() + LainNewApi.patchTempUpdate, MyGson.getInstance().toJson(formBody), this);
    ...
    @Override
    public void updateRealView(TextView textView, PatchTempRealBean data, int position) {
        //设置列表的数据
        textView.setText(data.getPatchName());
    }
}
