...
import environment.bean.DS18B20AlertBean;
import environment.bean.DS18B20HistoryBean;
import environment.bean.DS18B20RealBean;
import http.MyGson;
...
/**
 * @ClassName: DS18B20Fragment
 * @Description:
 * @Author: YIN LUO FEI
 * @Date: 2020/3/28 10:05
 */
public class DS18B20Fragment extends Device8060Fragment<DS18B20RealBean, DS18B20AlertBean,
        DS18B20HistoryBean, DS18B20RealBean> implements BaseRecyclerViewAdapter.OnItemClickListener, I_DeviceDetailManageLink<Device_Detail_AddBean>, I_HistoryAdapter<DS18B20HistoryBean>, I_RealAdapterListen<DS18B20RealBean>, I_UpdateRealView<DS18B20RealBean> {

    private UltimateRecyclerView waterPressureRealRecycler;

    List<DS18B20RealBean> waterPressureRealBeans = new ArrayList<>();

    //-------------------------------------------------
    private BaseDeviceDetailManageAdapter<Device_Detail_AddBean, DS18B20RealBean> manageAdapter;
...
//    public Temperature_Add deviceDetail_AddAdapter;
    ...
    String deleteUrl = "";
    String changeUrl = "";
    NewHistoryAdapter<DS18B20HistoryBean> newHistoryAdapter;
    List<DS18B20HistoryBean> lineDataList = new ArrayList<>();
    //设备管理的数据
    ...
    //设备实时数据工具类
    public RealDataUtil<DS18B20RealBean> realDataUtil;
     ...
     */
    public DS18B20Fragment(String realLink, String alertLink, String historyLink, String manageLink) {
        super(realLink, alertLink, historyLink, manageLink);
    }

    public DS18B20Fragment(String realLink, String alertLink, String historyLink, String manageLink, String deleteUrl, String changeUrl) {
        super(realLink, alertLink, historyLink, manageLink);
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        //查询实时数据
        device8060Present.queryRealData(DS18B20RealBean.class);
//        startRealUpdate();
        //设置历史数据列表
    ...
    public void queryHistoryData() {
        try {
            ...
            //默认查询第一个设备
            DS18B20RealBean temperatureBean = waterPressureRealBeans.get(0);
            ///获取设备的ID
            
            int temEhmId = temperatureBean.getEhbmId();
            //结束时间
            String endTime = DateUtil.getInstance().getCurrentDate();
            //开始时间
            String startTime = DateUtil.getInstance().getStartTime();
            //开始查询
            device8060Present.queryHistoryData(DS18B20HistoryBean.class, temEhmId, startTime, endTime);
    ...
    protected void querySelectDeviceHistory(int ehmID, String startTime, String endTime) {
        device8060Present.queryHistoryData(DS18B20HistoryBean.class, ehmID, startTime, endTime);
    }
    ...
    public void requestReal() {
        ...
        try {
            ...
            if (alertDeviceBeanList.isEmpty()) {
                //每次查询都保存第一个设备ID
                ehmIDFirst = waterPressureRealBeans.get(0).getEhbmId();
                //更新报警查询的设备
                for (DS18B20RealBean b :
                        waterPressureRealBeans) {
                    AlertDeviceBean alertB = new AlertDeviceBean();

                    alertB.setName(b.getEhbmDeviceName());
    ...
    public void setCurrentData(int position) {
        try {
            //点击切换上面的圆形进度条的数据
            //设置相关的属性值
            DS18B20RealBean b = waterPressureRealBeans.get(position);

            if (b.getEhbmTemp() == currentTempValue)
                return;
    ...
    public void requestAlert() {
        ...
        List<DS18B20AlertBean> air = getAlertData();
        alertBeans.clear();

        for (int i = 0; i < air.size(); i++) {
            DS18B20AlertBean alertBean = air.get(i);
            AlertBeans alertBeansItem = new AlertBeans();
            alertBeansItem.setEhaInfo(alertBean.getEhba_info());
            alertBeansItem.setEhaTime(alertBean.getEhba_time());
...
//        先清空原有的设备数据
        ...
        List<DS18B20RealBean> manageData = getManageData();
...
//        适配添加设备的数据
        for (DS18B20RealBean b :
            ...
            bean.setDeviceName(b.getEhbmDeviceName());

            bean.setEhmId(String.valueOf(b.getEhbmId()));
            //湿度
            bean.setAlertValue(b.getEhbmMinTemp() +" ~ " + b.getEhbmMaxTemp());
            //位置
            bean.setPosition(String.valueOf(b.getEhbmDeviceAddress()));
...
//        设置 设备管理 中的 添加设备 列表
    ...
    public void queryDeviceManage() {
        //只执行一次
        if (addBeans.size() == 0)
            device8060Present.queryDeviceManage(DS18B20RealBean.class);
    }

    @Override
    public void querySelectDeviceAlert(int ehmID, String startTime, String endTime) {
        device8060Present.queryAlertData(DS18B20AlertBean.class, ehmID, startTime, endTime);
    }
    @Override
    public void onItemClickListener(View v, int position) {
        ...
        try {
            ...
            if (v.findViewById(com.example.pool.R.id.btn_panel_device) != null) {
                //先保存数据，点击确定后在进行查询
                setEhmID(waterPressureRealBeans.get(position).getEhbmId());
                return;
            }
            //历史数据点击查看 曲线图
            if (v.findViewById(com.example.pool.R.id.new_history_name) != null) {
                ...
                for (DS18B20HistoryBean temp :
                        lineDataList) {

                    temps.add(temp.getEhbhTemp() + "");
                    hums.add(temp.getEhbhTime() + "");
                }
                ...
                intent.putExtra("markUnit",  waterPressureRealBeans.get(position).getEhbmDeviceName());
                ...
                intent.putExtra("title", waterPressureRealBeans.get(position).getEhbmDeviceName());
            ...
            DS18B20RealBean b = waterPressureRealBeans.get(position);
            //设置数据
            realDataUtil.setInnerData(String.valueOf(b.getEhbmTemp()));
            realDataUtil.setInnerMax(String.valueOf(b.getEhbmMaxTemp()));
            //设置报警值，必须先设置最大值，再设置当前值，才不会出现问题
            realDataUtil.setOuterMax(String.valueOf(b.getEhbmMaxTemp()));
            realDataUtil.setOuterData(String.valueOf(b.getEhbmMaxTemp()));
            //设置文案
     ...
     */
    @Override
    public Class<DS18B20RealBean> getParseClass() {
        return DS18B20RealBean.class;
    }
    @Override
    public void bindHistoryData(BaseViewHolder holder, DS18B20HistoryBean data, int position) {
        ...
        historyTemp.setText("水流量" + data.getEhbhTemp());
        historyTime.setText("时间" + data.getEhbhTime());
    ...
    public void deleteDevice(int position, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        String ehmId = String.valueOf(waterPressureRealBeans.get(position).getEhbmId());
     ...
     */
    @Override
    public void realItemAdapter(BaseViewHolder holder, DS18B20RealBean data, int position) {

        TextView item_name = holder.getView(com.example.pool.R.id.dissolved_item_name);
        item_name.setText(data.getEhbmDeviceName());
    ...
    public void changeDevice(String flag, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        ...
        DS18B20RealBean temp = waterPressureRealBeans.get(position);
        ...
        deviceManageEditBean.setaDeviceName(temp.getEhbmDeviceName());
        deviceManageEditBean.setActionID(temp.getEhbmId());


        deviceManageEditBean.setaDeviceInterval(temp.getIntervalTime());

        deviceManageEditBean.setTempRangeMin(temp.getEhbmMinTemp());
        deviceManageEditBean.setTempRangeMax(temp.getEhbmMaxTemp());
    ...
    public void deterMine(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("ehbmDeviceName", getTextString(keyArray.get("deviceName")));
        formBody.put("ehbmDeviceAddress", getSpinnerString(spinnerMap.get("deviceAddress")));
        formBody.put("gallery", getTextString(keyArray.get("deviceGallery")));
        formBody.put("ehbmMaxTemp", getTextString(keyArray.get("tempMax")));
        formBody.put("ehbmMinTemp", getTextString(keyArray.get("tempMin")));
        //湿度，暂时不用
        ...
        OkHttpUtil.getInstance().sendRowOkHttp("add", LainNewApi.getInstance().getRootPath() + LainNewApi.DS18B20Insert, MyGson.getInstance().toJson(formBody), this);
    ...
    public void deterChange(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        //将输入的参数添加到 FormBody 中，进行发送给服务器
        formBody.put("ehbmDeviceName", getTextString(keyArray.get("deviceName")));
        formBody.put("ehbmDeviceAddress", getTextString(keyArray.get("deviceAddress")));
        formBody.put("gallery", getTextString(keyArray.get("deviceGallery")));
        formBody.put("ehbmMaxTemp", getTextString(keyArray.get("tempMax")));
        formBody.put("ehbmMinTemp", getTextString(keyArray.get("tempMin")));
        formBody.put("intervalTime", getTextString(keyArray.get("interval")));
        //湿度，暂时不用
        formBody.put("ehbmMaxHum", getTextString(keyArray.get("humMax")));
        formBody.put("ehbmMinHum", getTextString(keyArray.get("humMin")));

        formBody.put("gId", getTextString(keyArray.get("gId")));
        formBody.put("ehbmId", getTextString(keyArray.get("actionID")));

        OkHttpUtil.getInstance().sendPutOkHttp("change", LainNewApi.getInstance().getRootPath() + LainNewApi.DS18B20Update, MyGson.getInstance().toJson(formBody), this);
    ...
    @Override
    public void updateRealView(TextView textView, DS18B20RealBean data, int position) {
        //设置列表的数据
        textView.setText(data.getEhbmDeviceName());
    }
}
