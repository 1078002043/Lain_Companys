...
import environment.bean.FiberAlertBean;
import environment.bean.FiberHistoryBean;
import environment.bean.FiberRealBean;
import http.MyGson;
...
/**
 * @ClassName: FiberFragment
 * @Description: 光纤电缆测温
 * @Author: YIN LUO FEI
 * @Date: 2020/3/28 11:26
 */
public class FiberFragment extends Device8060Fragment<FiberRealBean, FiberAlertBean,
        FiberHistoryBean, FiberRealBean> implements BaseRecyclerViewAdapter.OnItemClickListener, I_DeviceDetailManageLink<Device_Detail_AddBean>, I_HistoryAdapter<FiberHistoryBean>, I_RealAdapterListen<FiberRealBean> , I_UpdateRealView<FiberRealBean> {

    private UltimateRecyclerView waterPressureRealRecycler;

    List<FiberRealBean> waterPressureRealBeans = new ArrayList<>();

    //-------------------------------------------------
    private BaseDeviceDetailManageAdapter<Device_Detail_AddBean, FiberRealBean> manageAdapter;
...
//    public Temperature_Add deviceDetail_AddAdapter;
    ...
    String deleteUrl = "";
    String changeUrl = "";
    NewHistoryAdapter<FiberHistoryBean> newHistoryAdapter;
    List<FiberHistoryBean> lineDataList = new ArrayList<>();
    //设备管理的数据
    ...
    //设备实时数据工具类
    public RealDataUtil<FiberRealBean> realDataUtil;
     ...
     */
    public FiberFragment(String realLink, String alertLink, String historyLink, String manageLink) {
        super(realLink, alertLink, historyLink, manageLink);
    }

    public FiberFragment(String realLink, String alertLink, String historyLink, String manageLink, String deleteUrl, String changeUrl) {
        super(realLink, alertLink, historyLink, manageLink);
...
//        changeUrl = LainNewApi.tempChange;
        ...
        //查询实时数据
        device8060Present.queryRealData(FiberRealBean.class);
//        startRealUpdate();
        //设置历史数据列表
    ...
    public void queryHistoryData() {
        try {
            ...
            //默认查询第一个设备
            FiberRealBean temperatureBean = waterPressureRealBeans.get(0);
            ///获取设备的ID

            int temEhmId = temperatureBean.getOpticalId();
            //结束时间
            String endTime = DateUtil.getInstance().getCurrentDate();
            //开始时间
            String startTime = DateUtil.getInstance().getStartTime();
            //开始查询
            device8060Present.queryHistoryData(FiberHistoryBean.class, temEhmId, startTime, endTime);
    ...
    protected void querySelectDeviceHistory(int ehmID, String startTime, String endTime) {
        device8060Present.queryHistoryData(FiberHistoryBean.class, ehmID, startTime, endTime);
    }
    ...
    public void requestReal() {
        ...
        try {
            ...
            if (alertDeviceBeanList.isEmpty()) {
                //每次查询都保存第一个设备ID
                ehmIDFirst = waterPressureRealBeans.get(0).getOpticalId();
                //更新报警查询的设备
                for (FiberRealBean b :
                        waterPressureRealBeans) {
                    AlertDeviceBean alertB = new AlertDeviceBean();

                    alertB.setName(b.getOpticalName());
    ...
    public void setCurrentData(int position) {
        try {
            //点击切换上面的圆形进度条的数据
            //设置相关的属性值
            FiberRealBean b = waterPressureRealBeans.get(position);

            if (b.getOpticalData() == currentTempValue)
                return;
    ...
    public void requestAlert() {
        ...
        List<FiberAlertBean> air = getAlertData();
        alertBeans.clear();

        for (int i = 0; i < air.size(); i++) {
            FiberAlertBean alertBean = air.get(i);
            AlertBeans alertBeansItem = new AlertBeans();
            alertBeansItem.setEhaInfo(alertBean.getInfo());
            alertBeansItem.setEhaTime(alertBean.getOpticalAlarmTime());
...
//        先清空原有的设备数据
        ...
        List<FiberRealBean> manageData = getManageData();
...
//        适配添加设备的数据
        for (FiberRealBean b :
            ...
            bean.setDeviceName(b.getOpticalName());

            bean.setEhmId(String.valueOf(b.getOpticalId()));
            //湿度
            bean.setAlertValue(b.getOpticalMinData() + " ~ " + b.getOpticalMaxData());

            //位置
            bean.setPosition(String.valueOf(b.getOpticalAddress()));
            //更新时间
...
//        设置 设备管理 中的 添加设备 列表
    ...
    public void queryDeviceManage() {
        //只执行一次
        if (addBeans.size() == 0)
            device8060Present.queryDeviceManage(FiberRealBean.class);
    }

    @Override
    public void querySelectDeviceAlert(int ehmID, String startTime, String endTime) {
        device8060Present.queryAlertData(FiberAlertBean.class, ehmID, startTime, endTime);
    }
    ...
    public void onItemClickListener(View v, int position) {
        ...
        try {
            ...
            if (v.findViewById(com.example.pool.R.id.btn_panel_device) != null) {
                //先保存数据，点击确定后在进行查询
                setEhmID(waterPressureRealBeans.get(position).getOpticalId());
                return;
            }
            //历史数据点击查看 曲线图
            if (v.findViewById(com.example.pool.R.id.new_history_name) != null) {
                ...
                for (FiberHistoryBean temp :
                        lineDataList) {

                    temps.add(temp.getOpticalData() + "");
                    hums.add(temp.getOpticalHistoryTime() + "");
                }
                ...
                intent.putExtra("markUnit", waterPressureRealBeans.get(position).getOpticalName());
                ...
                intent.putExtra("title", waterPressureRealBeans.get(position).getOpticalName());
            ...
            FiberRealBean b = waterPressureRealBeans.get(position);
            //设置数据
            realDataUtil.setInnerData(String.valueOf(b.getOpticalData()));
            realDataUtil.setInnerMax(String.valueOf(b.getOpticalMaxData()));
            //设置报警值，必须先设置最大值，再设置当前值，才不会出现问题
            realDataUtil.setOuterMax(String.valueOf(b.getOpticalMaxData()));
            realDataUtil.setOuterData(String.valueOf(b.getOpticalMaxData()));
            //设置文案
     ...
     */
    @Override
    public Class<FiberRealBean> getParseClass() {
        return FiberRealBean.class;
    }

    @Override
    public void bindHistoryData(BaseViewHolder holder, FiberHistoryBean data, int position) {
        ...
        historyTemp.setText("水流量" + data.getOpticalData());
        historyTime.setText("时间" + data.getOpticalHistoryTime());
    ...
    public void deleteDevice(int position, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        String ehmId = String.valueOf(waterPressureRealBeans.get(position).getOpticalId());
     ...
     */
    @Override
    public void realItemAdapter(BaseViewHolder holder, FiberRealBean data, int position) {

        TextView item_name = holder.getView(com.example.pool.R.id.dissolved_item_name);
        item_name.setText(data.getOpticalName());
    ...
    public void changeDevice(String flag, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        ...
        FiberRealBean temp = waterPressureRealBeans.get(position);
        ...
        deviceManageEditBean.setaDeviceName(temp.getOpticalName());
        deviceManageEditBean.setActionID(temp.getOpticalId());


        deviceManageEditBean.setaDeviceInterval(temp.getIntervalTime());
//
        deviceManageEditBean.setRangeMin(temp.getOpticalMinData());
        deviceManageEditBean.setRangeMax(temp.getOpticalMaxData());
    ...
    public void deterMine(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("opticalName", getTextString(keyArray.get("deviceName")));
        formBody.put("opticalAddress", getSpinnerString(spinnerMap.get("deviceAddress")));
        formBody.put("opticalMaxData", getTextString(keyArray.get("rangeMax")));
        formBody.put("opticalMinData", getTextString(keyArray.get("rangeMin")));
    ...
    public void deterChange(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("opticalMinData", getTextString(keyArray.get("rangeMin")));
        formBody.put("intervalTime", getTextString(keyArray.get("interval")));
        formBody.put("opticalId", getTextString(keyArray.get("actionID")));
    ...
    @Override
    public void updateRealView(TextView textView, FiberRealBean data, int position) {
        //设置列表的数据
        textView.setText(data.getOpticalName());
    }
}
