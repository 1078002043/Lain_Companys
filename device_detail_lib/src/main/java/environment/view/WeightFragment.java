...
import environment.bean.WeightAlertBean;
import environment.bean.WeightHistoryBean;
import environment.bean.WeightRealBean;
import http.MyGson;
...
/**
 * @ClassName: WeightFragment
 * @Description:
 * @Author: YIN LUO FEI
 * @Date: 2020/3/28 11:45
 */
public class WeightFragment extends Device8060Fragment<WeightRealBean, WeightAlertBean,
        WeightHistoryBean, WeightRealBean> implements BaseRecyclerViewAdapter.OnItemClickListener, I_DeviceDetailManageLink<Device_Detail_AddBean>, I_HistoryAdapter<WeightHistoryBean>, I_RealAdapterListen<WeightRealBean>, I_UpdateRealView<WeightRealBean> {

    private UltimateRecyclerView waterPressureRealRecycler;

    List<WeightRealBean> waterPressureRealBeans = new ArrayList<>();

    //-------------------------------------------------
    private BaseDeviceDetailManageAdapter<Device_Detail_AddBean, WeightRealBean> manageAdapter;
...
//    public Temperature_Add deviceDetail_AddAdapter;
    ...
    String deleteUrl = "";
    String changeUrl = "";
    NewHistoryAdapter<WeightHistoryBean> newHistoryAdapter;
    List<WeightHistoryBean> lineDataList = new ArrayList<>();
    //设备管理的数据
    ...
    //设备实时数据工具类
    public RealDataUtil<WeightRealBean> realDataUtil;
     ...
     */
    public WeightFragment(String realLink, String alertLink, String historyLink, String manageLink) {
        super(realLink, alertLink, historyLink, manageLink);
    }

    public WeightFragment(String realLink, String alertLink, String historyLink, String manageLink, String deleteUrl, String changeUrl) {
        super(realLink, alertLink, historyLink, manageLink);
...
//        changeUrl = LainNewApi.tempChange;
        ...
        //查询实时数据
        device8060Present.queryRealData(WeightRealBean.class);
//        startRealUpdate();
        //设置历史数据列表
    ...
    public void queryHistoryData() {
        try {
            ...
            //默认查询第一个设备
            WeightRealBean temperatureBean = waterPressureRealBeans.get(0);
            ///获取设备的ID

            int temEhmId = temperatureBean.getWeighId();
            //结束时间
            String endTime = DateUtil.getInstance().getCurrentDate();
            //开始时间
            String startTime = DateUtil.getInstance().getStartTime();
            //开始查询
            device8060Present.queryHistoryData(WeightHistoryBean.class, temEhmId, startTime, endTime);
    ...
    protected void querySelectDeviceHistory(int ehmID, String startTime, String endTime) {
        device8060Present.queryHistoryData(WeightHistoryBean.class, ehmID, startTime, endTime);
    }
    ...
    public void requestReal() {
        ...
        try {
            ...
            if (alertDeviceBeanList.isEmpty()) {
                //每次查询都保存第一个设备ID
                ehmIDFirst = waterPressureRealBeans.get(0).getWeighId();
                //更新报警查询的设备
                for (WeightRealBean b :
                        waterPressureRealBeans) {
                    AlertDeviceBean alertB = new AlertDeviceBean();

                    alertB.setName(b.getWeighName());
    ...
    public void setCurrentData(int position) {
        try {
            //点击切换上面的圆形进度条的数据
            //设置相关的属性值
            WeightRealBean b = waterPressureRealBeans.get(position);

            if (b.getWeighData() == currentTempValue)
                return;
    ...
    public void requestAlert() {
        ...
        List<WeightAlertBean> air = getAlertData();
        alertBeans.clear();

        for (int i = 0; i < air.size(); i++) {
            WeightAlertBean alertBean = air.get(i);
            AlertBeans alertBeansItem = new AlertBeans();
            alertBeansItem.setEhaInfo(alertBean.getInfo());
            alertBeansItem.setEhaTime(alertBean.getWeighAlarmTime());
...
//        先清空原有的设备数据
        ...
        List<WeightRealBean> manageData = getManageData();
...
//        适配添加设备的数据
        for (WeightRealBean b :
            ...
            bean.setDeviceName(b.getWeighName());

            bean.setEhmId(String.valueOf(b.getWeighId()));
            //湿度
            bean.setAlertValue(b.getWeighMinData() + " ~ " + b.getWeighMaxData());

            //位置
            bean.setPosition(String.valueOf(b.getWeighAddress()));
            //更新时间
...
//        设置 设备管理 中的 添加设备 列表
    ...
    public void queryDeviceManage() {
        //只执行一次
        if (addBeans.size() == 0)
            device8060Present.queryDeviceManage(WeightRealBean.class);
    }

    @Override
    public void querySelectDeviceAlert(int ehmID, String startTime, String endTime) {
        device8060Present.queryAlertData(WeightAlertBean.class, ehmID, startTime, endTime);
    }
    ...
    public void onItemClickListener(View v, int position) {
        ...
        try {
            ...
            if (v.findViewById(com.example.pool.R.id.btn_panel_device) != null) {
                //先保存数据，点击确定后在进行查询
                setEhmID(waterPressureRealBeans.get(position).getWeighId());
                return;
            }
            //历史数据点击查看 曲线图
            if (v.findViewById(com.example.pool.R.id.new_history_name) != null) {
                ...
                for (WeightHistoryBean temp :
                        lineDataList) {

                    temps.add(temp.getWeighData() + "");
                    hums.add(temp.getWeighHistoryTime() + "");
                }
                ...
                intent.putExtra("markUnit", waterPressureRealBeans.get(position).getWeighName());
                ...
                intent.putExtra("title", waterPressureRealBeans.get(position).getWeighName());
            ...
            //设置数据
            realDataUtil.setInnerData(String.valueOf(b.getWeighData()));
            realDataUtil.setInnerMax(String.valueOf(b.getWeighMaxData()));
            //设置报警值，必须先设置最大值，再设置当前值，才不会出现问题
            realDataUtil.setOuterMax(String.valueOf(b.getWeighMaxData()));
            realDataUtil.setOuterData(String.valueOf(b.getWeighMaxData()));
            //设置文案
            realDataUtil.setInnerTitle("千克 KG");
     ...
     */
    @Override
    public Class<WeightRealBean> getParseClass() {
        return WeightRealBean.class;
    }

    @Override
    public void bindHistoryData(BaseViewHolder holder, WeightHistoryBean data, int position) {
        ...
        historyTemp.setText("水流量" + data.getWeighData());
        historyTime.setText("时间" + data.getWeighHistoryTime());
    ...
    public void deleteDevice(int position, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        String ehmId = String.valueOf(waterPressureRealBeans.get(position).getWeighId());
     ...
     */
    @Override
    public void realItemAdapter(BaseViewHolder holder, WeightRealBean data, int position) {

        TextView item_name = holder.getView(com.example.pool.R.id.dissolved_item_name);
        item_name.setText(data.getWeighName());
    ...
    public void changeDevice(String flag, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        ...
        WeightRealBean temp = waterPressureRealBeans.get(position);
        ...
        deviceManageEditBean.setaDeviceName(temp.getWeighName());
        deviceManageEditBean.setActionID(temp.getWeighId());


        deviceManageEditBean.setaDeviceInterval(temp.getIntervalTime());
//
        deviceManageEditBean.setRangeMin(temp.getWeighMinData());
        deviceManageEditBean.setRangeMax(temp.getWeighMaxData());
    ...
    public void deterMine(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("weighName", getTextString(keyArray.get("deviceName")));
        formBody.put("weighAddress", getSpinnerString(spinnerMap.get("deviceAddress")));
        formBody.put("weighMaxData", getTextString(keyArray.get("rangeMax")));
        formBody.put("weighMinData", getTextString(keyArray.get("rangeMin")));
        ...
        OkHttpUtil.getInstance().sendRowOkHttp("add", LainNewApi.getInstance().getRootPath() + LainNewApi.weightInsert, MyGson.getInstance().toJson(formBody), this);
    ...
    public void deterChange(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("weighMinData", getTextString(keyArray.get("rangeMin")));
        formBody.put("intervalTime", getTextString(keyArray.get("interval")));
        formBody.put("weighId", getTextString(keyArray.get("actionID")));
    ...
    @Override
    public void updateRealView(TextView textView, WeightRealBean data, int position) {
        //设置列表的数据
        textView.setText(data.getWeighName());
    }
}
