...
import environment.bean.TransformerRealBean;
import environment.bean.TransformerAlertBean;
import environment.bean.TransformerHistoryBean;
import http.MyGson;
...
/**
 * @ClassName: TransformerFragment
 * @Description: 变压器温控器
 * @Author: YIN LUO FEI
 * @Date: 2020/3/28 13:43
 */
public class TransformerFragment extends Device8060Fragment<TransformerRealBean, TransformerAlertBean,
        TransformerHistoryBean, TransformerRealBean> implements BaseRecyclerViewAdapter.OnItemClickListener, I_DeviceDetailManageLink<Device_Detail_AddBean>, I_HistoryAdapter<TransformerHistoryBean>, I_RealAdapterListen<TransformerRealBean>, I_UpdateRealView<TransformerRealBean> {

    private UltimateRecyclerView waterPressureRealRecycler;

    List<TransformerRealBean> waterPressureRealBeans = new ArrayList<>();


    //-------------------------------------------------
    private BaseDeviceDetailManageAdapter<Device_Detail_AddBean, TransformerRealBean> manageAdapter;
...
//    public Temperature_Add deviceDetail_AddAdapter;
    ...
    String deleteUrl = "";
    String changeUrl = "";
    NewHistoryAdapter<TransformerHistoryBean> newHistoryAdapter;
    List<TransformerHistoryBean> lineDataList = new ArrayList<>();
    //设备管理的数据
    private ArrayList<Device_Detail_AddBean> addBeans = new ArrayList<>();
    private static LayoutInflater layoutInflater;

    //设备实时数据工具类
    public RealDataUtil<TransformerRealBean> realDataUtil;
     ...
     */
    public TransformerFragment(String realLink, String alertLink, String historyLink, String manageLink) {
        super(realLink, alertLink, historyLink, manageLink);
    }

    public TransformerFragment(String realLink, String alertLink, String historyLink, String manageLink, String deleteUrl, String changeUrl) {
        super(realLink, alertLink, historyLink, manageLink);
...
//        changeUrl = LainNewApi.tempChange;
        ...
        //查询实时数据
        device8060Present.queryRealData(TransformerRealBean.class);
//        startRealUpdate();
        //设置历史数据列表
    ...
    public void queryHistoryData() {
        try {
            ...
            //默认查询第一个设备
            TransformerRealBean temperatureBean = waterPressureRealBeans.get(0);
            ///获取设备的ID
            
            int temEhmId = temperatureBean.getTransformerId();
            //结束时间
            String endTime = DateUtil.getInstance().getCurrentDate();
            //开始时间
            String startTime = DateUtil.getInstance().getStartTime();
            //开始查询
            device8060Present.queryHistoryData(TransformerHistoryBean.class, temEhmId, startTime, endTime);
    ...
    protected void querySelectDeviceHistory(int ehmID, String startTime, String endTime) {
        device8060Present.queryHistoryData(TransformerHistoryBean.class, ehmID, startTime, endTime);
    }
    ...
    public void requestReal() {
        ...
        try {
            ...
            if (alertDeviceBeanList.isEmpty()) {
                //每次查询都保存第一个设备ID
                ehmIDFirst = waterPressureRealBeans.get(0).getTransformerId();
                //更新报警查询的设备
                for (TransformerRealBean b :
                        waterPressureRealBeans) {
                    AlertDeviceBean alertB = new AlertDeviceBean();

                    alertB.setName(b.getTransformerDeviceName());
    ...
    public void setCurrentData(int position) {
        try {
            //点击切换上面的圆形进度条的数据
            //设置相关的属性值
            TransformerRealBean b = waterPressureRealBeans.get(position);

            if (b.getTransformerHum() == currentTempValue && b.getTransformerTem() == currentHumValue)
                return;
    ...
    public void requestAlert() {
        ...
        List<TransformerAlertBean> air = getAlertData();
        alertBeans.clear();

        for (int i = 0; i < air.size(); i++) {
            TransformerAlertBean alertBean = air.get(i);
            AlertBeans alertBeansItem = new AlertBeans();
            alertBeansItem.setEhaInfo(alertBean.getTransformerAlarmInfo());
            alertBeansItem.setEhaTime(alertBean.getTransformerAlarmTime());
            alertBeansItem.setEcmDeviceName(alertBean.getTransformerName());
...
//        先清空原有的设备数据
        ...
        List<TransformerRealBean> manageData = getManageData();
...
//        适配添加设备的数据
        for (TransformerRealBean b :
            ...
            bean.setDeviceName(b.getTransformerDeviceName());

            bean.setEhmId(String.valueOf(b.getTransformerId()));
            //湿度
            bean.setAlertValue(b.getTransformerMinTem() + " ~ " + b.getTransformerMaxTem());

            //位置
            bean.setPosition(String.valueOf(b.getTransformerDeviceAddress()));
            //更新时间
...
//        设置 设备管理 中的 添加设备 列表
    ...
    public void queryDeviceManage() {
        //只执行一次
        if (addBeans.size() == 0)
            device8060Present.queryDeviceManage(TransformerRealBean.class);
    }

    @Override
    public void querySelectDeviceAlert(int ehmID, String startTime, String endTime) {
        device8060Present.queryAlertData(TransformerAlertBean.class, ehmID, startTime, endTime);
    }
    @Override
    public void onItemClickListener(View v, int position) {
        ...
        try {
            ...
            if (v.findViewById(com.example.pool.R.id.btn_panel_device) != null) {
                //先保存数据，点击确定后在进行查询
                setEhmID(waterPressureRealBeans.get(position).getTransformerId());
                return;
            }
            //历史数据点击查看 曲线图
            if (v.findViewById(com.example.pool.R.id.new_history_name) != null) {
                ...
                for (TransformerHistoryBean temp :
                        lineDataList) {
                  
                    temps.add(temp.getTransformerHum() + "");
                    hums.add(temp.getTransformerTem() + "");
                    times.add(temp.getTransformerHistoryTime() + "");
                }
                ...
                intent.putExtra("markUnit",  waterPressureRealBeans.get(position).getTransformerDeviceName());
                ...
                intent.putExtra("title", waterPressureRealBeans.get(position).getTransformerDeviceName());
            ...
            TransformerRealBean b = waterPressureRealBeans.get(position);
            //设置数据
            realDataUtil.setInnerMax(String.valueOf(b.getTransformerMaxHum()));
            realDataUtil.setInnerData(String.valueOf(b.getTransformerHum()));
            //设置报警值，必须先设置最大值，再设置当前值，才不会出现问题
            realDataUtil.setOuterMax(String.valueOf(b.getTransformerMaxTem()));
            realDataUtil.setOuterData(String.valueOf(b.getTransformerTem()));
            //设置文案
     ...
     */
    @Override
    public Class<TransformerRealBean> getParseClass() {
        return TransformerRealBean.class;
    }
    @Override
    public void bindHistoryData(BaseViewHolder holder, TransformerHistoryBean data, int position) {
        ...
        historyTemp.setText("水流量" + data.getTransformerHum());
        historyTime.setText("时间" + data.getTransformerHistoryTime());
    ...
    public void deleteDevice(int position, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        String ehmId = String.valueOf(waterPressureRealBeans.get(position).getTransformerId());
     ...
     */
    @Override
    public void realItemAdapter(BaseViewHolder holder, TransformerRealBean data, int position) {

        TextView item_name = holder.getView(com.example.pool.R.id.dissolved_item_name);
        item_name.setText(data.getTransformerDeviceName());
    ...
    public void changeDevice(String flag, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        ...
        TransformerRealBean temp = waterPressureRealBeans.get(position);
        ...
        deviceManageEditBean.setaDeviceName(temp.getTransformerDeviceName());
        deviceManageEditBean.setActionID(temp.getTransformerId());


        deviceManageEditBean.setaDeviceInterval(temp.getIntervalTime());
//
        deviceManageEditBean.setTempRangeMax(temp.getTransformerMaxHum());
        deviceManageEditBean.setTempRangeMin(temp.getTransformerMinTem());

        deviceManageEditBean.setHumRangeMax(temp.getTransformerMaxHum());
        deviceManageEditBean.setHumRangeMin(temp.getTransformerMinHum());
    ...
    public void deterMine(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("transformerDeviceName", getTextString(keyArray.get("deviceName")));
        formBody.put("transformerDeviceAddress", getSpinnerString(spinnerMap.get("deviceAddress")));
        formBody.put("transformerDeviceFunction", getSpinnerString(spinnerMap.get("functionCode")));
        formBody.put("transformerMaxTem", getTextString(keyArray.get("tempMax")));
        formBody.put("transformerMinTem", getTextString(keyArray.get("tempMin")));
        formBody.put("transformerMaxHum", getTextString(keyArray.get("humMax")));
        formBody.put("transformerMinHum", getTextString(keyArray.get("humMin")));
        ...
        OkHttpUtil.getInstance().sendRowOkHttp("add", LainNewApi.getInstance().getRootPath() + LainNewApi.transformerInsert, MyGson.getInstance().toJson(formBody), this);
    ...
    public void deterChange(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("transformerMinHum", getTextString(keyArray.get("humMin")));
        formBody.put("intervalTime", getTextString(keyArray.get("interval")));
        formBody.put("transformerId", getTextString(keyArray.get("actionID")));
    ...
    @Override
    public void updateRealView(TextView textView, TransformerRealBean data, int position) {
        //设置列表的数据
        textView.setText(data.getTransformerDeviceName());
    }
}
