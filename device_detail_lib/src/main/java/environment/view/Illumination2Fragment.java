...
import environment.bean.Illumination2AlertBean;
import environment.bean.Illumination2HistoryBean;
import environment.bean.Illumination2RealBean;
import http.MyGson;
...
/**
 * @ClassName: Illumination2
 * @Description: 照度传感器
 * @Author: YIN LUO FEI
 * @Date: 2020/3/28 12:53
 */
public class Illumination2Fragment extends Device8060Fragment<Illumination2RealBean, Illumination2AlertBean,
        Illumination2HistoryBean, Illumination2RealBean> implements BaseRecyclerViewAdapter.OnItemClickListener, I_DeviceDetailManageLink<Device_Detail_AddBean>, I_HistoryAdapter<Illumination2HistoryBean>, I_RealAdapterListen<Illumination2RealBean>, I_UpdateRealView<Illumination2RealBean> {

    private UltimateRecyclerView waterPressureRealRecycler;

    List<Illumination2RealBean> waterPressureRealBeans = new ArrayList<>();

    //-------------------------------------------------
    private BaseDeviceDetailManageAdapter<Device_Detail_AddBean, Illumination2RealBean> manageAdapter;
...
//    public Temperature_Add deviceDetail_AddAdapter;
    ...
    String deleteUrl = "";
    String changeUrl = "";
    NewHistoryAdapter<Illumination2HistoryBean> newHistoryAdapter;
    List<Illumination2HistoryBean> lineDataList = new ArrayList<>();
    //设备管理的数据
    private ArrayList<Device_Detail_AddBean> addBeans = new ArrayList<>();
    private static LayoutInflater layoutInflater;

    //设备实时数据工具类
    public RealDataUtil<Illumination2RealBean> realDataUtil;
     ...
     */
    public Illumination2Fragment(String realLink, String alertLink, String historyLink, String manageLink) {
        super(realLink, alertLink, historyLink, manageLink);
    }

    public Illumination2Fragment(String realLink, String alertLink, String historyLink, String manageLink, String deleteUrl, String changeUrl) {
        super(realLink, alertLink, historyLink, manageLink);
...
//        changeUrl = LainNewApi.tempChange;
        ...
        //查询实时数据
        device8060Present.queryRealData(Illumination2RealBean.class);
//        startRealUpdate();
        //设置历史数据列表
    ...
    public void queryHistoryData() {
        try {
            ...
            //默认查询第一个设备
            Illumination2RealBean temperatureBean = waterPressureRealBeans.get(0);
            ///获取设备的ID
            ...
            //开始查询
            device8060Present.queryHistoryData(Illumination2HistoryBean.class, temEhmId, startTime, endTime);
    ...
    protected void querySelectDeviceHistory(int ehmID, String startTime, String endTime) {
        device8060Present.queryHistoryData(Illumination2HistoryBean.class, ehmID, startTime, endTime);
    }
    ...
    public void setCurrentData(int position) {
        try {
            //点击切换上面的圆形进度条的数据
            //设置相关的属性值
            Illumination2RealBean b = waterPressureRealBeans.get(position);
    ...
    public void requestAlert() {
        ...
        List<Illumination2AlertBean> air = getAlertData();
        alertBeans.clear();

        for (int i = 0; i < air.size(); i++) {
            Illumination2AlertBean alertBean = air.get(i);
...
//        先清空原有的设备数据
        ...
        List<Illumination2RealBean> manageData = getManageData();
...
//        设置 设备管理 中的 添加设备 列表
    ...
    public void queryDeviceManage() {
        //只执行一次
        if (addBeans.size() == 0)
            device8060Present.queryDeviceManage(Illumination2RealBean.class);
    }

    @Override
    public void querySelectDeviceAlert(int ehmID, String startTime, String endTime) {
        device8060Present.queryAlertData(Illumination2AlertBean.class, ehmID, startTime, endTime);
    }
    ...
    public void onItemClickListener(View v, int position) {
        ...
        try {
            ...
            if (v.findViewById(com.example.pool.R.id.new_history_name) != null) {
                ...
                for (Illumination2HistoryBean temp :
                        lineDataList) {
     ...
     */
    @Override
    public Class<Illumination2RealBean> getParseClass() {
        return Illumination2RealBean.class;
    }

    @Override
    public void bindHistoryData(BaseViewHolder holder, Illumination2HistoryBean data, int position) {
     ...
     */
    @Override
    public void realItemAdapter(BaseViewHolder holder, Illumination2RealBean data, int position) {
    ...
    public void changeDevice(String flag, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        ...
        Illumination2RealBean temp = waterPressureRealBeans.get(position);
...
//
    ...
    public void deterMine(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        OkHttpUtil.getInstance().sendRowOkHttp("add", LainNewApi.getInstance().getRootPath() + LainNewApi.illumination2Insert, MyGson.getInstance().toJson(formBody), this);
    ...
    @Override
    public void updateRealView(TextView textView, Illumination2RealBean data, int position) {
        //设置列表的数据
