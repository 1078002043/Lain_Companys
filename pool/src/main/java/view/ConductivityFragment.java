...
import bean.ConductivityAlertBean;
import bean.ConductivityHistoryBean;
import bean.ConductivityRealBean;
import bean.Device_Detail_AddBean;
import http.MyGson;
import http.OkHttpUtil;
import present.ConductivityPresent;
...
public class ConductivityFragment extends Device8060Fragment<ConductivityRealBean, ConductivityAlertBean,
        ConductivityHistoryBean, ConductivityRealBean> implements BaseRecyclerViewAdapter.OnItemClickListener, I_DeviceDetailManageLink<Device_Detail_AddBean>, I_HistoryAdapter<ConductivityHistoryBean>, I_Conductivity, I_UpdateRealView<ConductivityRealBean> {

    //初始化 present
    private ConductivityPresent conductivityPresent;
    //设备列表数据
    private UltimateRecyclerView conductivityRecycler;

    //实时数据
    private List<ConductivityRealBean> conductivityRealBeans = new ArrayList<>();

    //-------------------------------------------------
    private BaseDeviceDetailManageAdapter<Device_Detail_AddBean, ConductivityRealBean> manageAdapter;
...
//    public Temperature_Add deviceDetail_AddAdapter;
    ...
    //设备实时数据工具类
    public RealDataUtil<ConductivityRealBean> realDataUtil;

    //删除URL
    String deleteUrl = "";
    String changeUrl = "";
    NewHistoryAdapter<ConductivityHistoryBean> newHistoryAdapter;
    List<ConductivityHistoryBean> lineDataList = new ArrayList<>();
    //设备管理的数据
     ...
     */
    public ConductivityFragment(String realLink, String alertLink, String historyLink, String manageLink) {
        super(realLink, alertLink, historyLink, manageLink);
    }

    public ConductivityFragment(String realLink, String alertLink, String historyLink, String manageLink, String deleteUrl, String changeUrl) {
        super(realLink, alertLink, historyLink, manageLink);
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        //Present
        conductivityPresent = new ConductivityPresent(this);

        //请求数据
        conductivityPresent.dealRealData(Pool_API.Pool_IP + Pool_API.conductivity, "conductivity_real");
...
//        changeUrl = LainNewApi.tempChange;
        manageAdapter = new BaseDeviceDetailManageAdapter<>(addBeans, conductivityRealBeans, getActivity(), R.layout.split_air_manage_swiper, deleteUrl, changeUrl, this, "溶氧检测");
        ...
        //查询实时数据
        device8060Present.queryRealData(ConductivityRealBean.class);
//        startRealUpdate();
        //设置历史数据列表
    ...
    private void initRealDataUtil() {
        ...
        realDataUtil.initView(viewList.get(0), getActivity(), conductivityRealBeans, this, this);
    }
    ...
    public void queryHistoryData() {
        try {
            //默认查询第一个设备
            ConductivityRealBean temperatureBean = conductivityRealBeans.get(0);
            ///获取设备的ID
            int temEhmId = temperatureBean.getConId();
            //结束时间
            String endTime = DateUtil.getInstance().getCurrentDate();
            //开始时间
            String startTime = DateUtil.getInstance().getStartTime();
            //开始查询
            device8060Present.queryHistoryData(ConductivityHistoryBean.class, temEhmId, startTime, endTime);
    ...
    protected void querySelectDeviceHistory(int ehmID, String startTime, String endTime) {
        device8060Present.queryHistoryData(ConductivityHistoryBean.class, ehmID, startTime, endTime);
    }
    ...
    public void requestReal() {
        ...
        try {
            //先清空
            conductivityRealBeans.clear();
            //更新所有的数据，必须调用 getRealData()，才能获取到数据，必须使用 addAll 才能更新列表
            conductivityRealBeans.addAll(getRealData());
            ...
            if (alertDeviceBeanList.isEmpty()) {
                //每次查询都保存第一个设备ID
                ehmIDFirst = conductivityRealBeans.get(0).getConId();
                //更新报警查询的设备
                for (ConductivityRealBean b :
                        conductivityRealBeans) {
                    AlertDeviceBean alertB = new AlertDeviceBean();
                    alertB.setName(b.getConName());
    ...
    public void setCurrentData(int position) {
        try {
            //点击切换上面的圆形进度条的数据
            //设置相关的属性值
            ConductivityRealBean b = conductivityRealBeans.get(position);

            if (b.getConData() == currentTempValue)
                return;
    ...
    public void requestAlert() {
        List<ConductivityAlertBean> air = getAlertData();
        alertBeans.clear();

        for (int i = 0; i < air.size(); i++) {
            ConductivityAlertBean alertBean = air.get(i);
            AlertBeans alertBeansItem = new AlertBeans();
            alertBeansItem.setEhaInfo(alertBean.getInfo());
            alertBeansItem.setEhaTime(alertBean.getConAlarmTime());
            alertBeansItem.setEcmDeviceName(alertBean.getConName());
...
//        先清空原有的设备数据
        ...
        List<ConductivityRealBean> manageData = getManageData();
...
//        适配添加设备的数据
        for (ConductivityRealBean b :
            ...
            bean.setDeviceName(b.getConName());

            bean.setEhmId(String.valueOf(b.getConId()));
            //湿度
            bean.setAlertValue(b.getConMinData() + " ~ " + b.getConMaxData());

            //位置
            bean.setPosition(String.valueOf(b.getConAddress()));
            //更新时间
...
//        设置 设备管理 中的 添加设备 列表
    ...
    public void queryDeviceManage() {
        //只执行一次
        if (addBeans.size() == 0)
            device8060Present.queryDeviceManage(ConductivityRealBean.class);
    }

    @Override
    public void querySelectDeviceAlert(int ehmID, String startTime, String endTime) {
        device8060Present.queryAlertData(ConductivityAlertBean.class, ehmID, startTime, endTime);
    }
    ...
    public void onItemClickListener(View v, int position) {
        ...
        try {
            ...
            if (v.findViewById(R.id.btn_panel_device) != null) {
                //先保存数据，点击确定后在进行查询
                setEhmID(conductivityRealBeans.get(position).getConId());
                return;
            }
            //历史数据点击查看 曲线图
            if (v.findViewById(com.example.pool.R.id.new_history_name) != null) {
                ...
                for (ConductivityHistoryBean temp :
                        lineDataList) {
                    temps.add(temp.getConData() + "");
                    hums.add(temp.getConHistoryTime() + "");
                }
                ...
                intent.putExtra("markUnit", "电导");
                intent.putExtra("linesText", "电导");
                ...
                intent.putExtra("title", conductivityRealBeans.get(position).getConName());
            ...
            ConductivityRealBean b = conductivityRealBeans.get(position);
//设置数据
            realDataUtil.setInnerMax(String.valueOf(b.getConMaxData()));
            realDataUtil.setInnerData(String.valueOf(b.getConData()));
            //设置报警值，必须先设置最大值，再设置当前值，才不会出现问题
            realDataUtil.setOuterMax(String.valueOf(b.getConMaxData()));
            realDataUtil.setOuterData(String.valueOf(b.getConMaxData()));
            //设置文案
     ...
     */
    @Override
    public Class<ConductivityRealBean> getParseClass() {
        return ConductivityRealBean.class;
    }

    @Override
    public void bindHistoryData(BaseViewHolder holder, ConductivityHistoryBean data, int position) {
        ...
        historyTemp.setText("电导" + data.getConData());
        historyTime.setText("时间" + data.getConHistoryTime());
    ...
    public void deleteDevice(int position, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        String ehmId = String.valueOf(conductivityRealBeans.get(position).getConId());
    ...
    @Override
    public void conductivityRealComplete(List<ConductivityRealBean> conductivityRealBeans) {
    ...
    public void changeDevice(String flag, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        ...
        ConductivityRealBean temp = conductivityRealBeans.get(position);
        ...
        deviceManageEditBean.setaDeviceName(temp.getConName());
        deviceManageEditBean.setActionID(temp.getConId());


        deviceManageEditBean.setaDeviceInterval(temp.getIntervalTime());
//
        deviceManageEditBean.setRangeMin(temp.getConMinData());
        deviceManageEditBean.setRangeMax(temp.getConMaxData());
    ...
    public void deterMine(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("conName", getTextString(keyArray.get("deviceName")));
        formBody.put("conAddress", getSpinnerString(spinnerMap.get("deviceAddress")));
        formBody.put("conMaxData", getTextString(keyArray.get("rangeMax")));
        formBody.put("conMinData", getTextString(keyArray.get("rangeMin")));
        ...
        OkHttpUtil.getInstance().sendRowOkHttp("add", LainNewApi.getInstance().getRootPath() + LainNewApi.conductivityInsert, MyGson.getInstance().toJson(formBody), this);
    ...
    public void deterChange(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("conMinData", getTextString(keyArray.get("rangeMin")));
        formBody.put("intervalTime", getTextString(keyArray.get("interval")));
        formBody.put("conId", getTextString(keyArray.get("actionID")));
    ...
    @Override
    public void updateRealView(TextView textView, ConductivityRealBean data, int position) {
        //设置列表的数据
        textView.setText(data.getConName());
    }
}
