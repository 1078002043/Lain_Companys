...
import bean.ChlorophyllAlertBean;
import bean.ChlorophyllHistoryBean;
import bean.ChlorophyllRealBean;
...
/**
 * @ClassName: ChlorophyllFragment
 * @Description: 叶绿素
 * @Author: YIN LUO FEI
 * @Date: 2020/3/27 22:42
 */
public class ChlorophyllFragment extends Device8060Fragment<ChlorophyllRealBean, ChlorophyllAlertBean,
        ChlorophyllHistoryBean, ChlorophyllRealBean> implements BaseRecyclerViewAdapter.OnItemClickListener, I_DeviceDetailManageLink<Device_Detail_AddBean>, I_HistoryAdapter<ChlorophyllHistoryBean>, I_RealAdapterListen<ChlorophyllRealBean>, I_UpdateRealView<ChlorophyllRealBean> {

    private UltimateRecyclerView waterPressureRealRecycler;

    List<ChlorophyllRealBean> waterPressureRealBeans = new ArrayList<>();

    //-------------------------------------------------
    private BaseDeviceDetailManageAdapter<Device_Detail_AddBean, ChlorophyllRealBean> manageAdapter;
...
//    public Temperature_Add deviceDetail_AddAdapter;
    ...
    String deleteUrl = "";
    String changeUrl = "";
    NewHistoryAdapter<ChlorophyllHistoryBean> newHistoryAdapter;
    List<ChlorophyllHistoryBean> lineDataList = new ArrayList<>();
    //设备管理的数据
    ...
    //设备实时数据工具类
    public RealDataUtil<ChlorophyllRealBean> realDataUtil;
     ...
     */
    public ChlorophyllFragment(String realLink, String alertLink, String historyLink, String manageLink) {
        super(realLink, alertLink, historyLink, manageLink);
    }

    public ChlorophyllFragment(String realLink, String alertLink, String historyLink, String manageLink, String deleteUrl, String changeUrl) {
        super(realLink, alertLink, historyLink, manageLink);
...
//        changeUrl = LainNewApi.tempChange;
        ...
        //查询实时数据
        device8060Present.queryRealData(ChlorophyllRealBean.class);
//        startRealUpdate();
        //设置历史数据列表
    ...
    public void queryHistoryData() {
        try {
            ...
            //默认查询第一个设备
            ChlorophyllRealBean temperatureBean = waterPressureRealBeans.get(0);
            ///获取设备的ID

            int temEhmId = temperatureBean.getChlorophyllId();
            //结束时间
            String endTime = DateUtil.getInstance().getCurrentDate();
            //开始时间
            String startTime = DateUtil.getInstance().getStartTime();
            //开始查询
            device8060Present.queryHistoryData(ChlorophyllHistoryBean.class, temEhmId, startTime, endTime);
    ...
    protected void querySelectDeviceHistory(int ehmID, String startTime, String endTime) {
        device8060Present.queryHistoryData(ChlorophyllHistoryBean.class, ehmID, startTime, endTime);
    }
    ...
    public void requestReal() {
        ...
        try {
            ...
            if (alertDeviceBeanList.isEmpty()) {
                //每次查询都保存第一个设备ID
                ehmIDFirst = waterPressureRealBeans.get(0).getChlorophyllId();
                //更新报警查询的设备
                for (ChlorophyllRealBean b :
                        waterPressureRealBeans) {
                    AlertDeviceBean alertB = new AlertDeviceBean();
                    alertB.setName(b.getChlorophyllName());
    ...
    public void setCurrentData(int position) {
        try {
            //点击切换上面的圆形进度条的数据
            //设置相关的属性值
            ChlorophyllRealBean b = waterPressureRealBeans.get(position);

            if (b.getChlorophyllData() == currentTempValue && b.getTemp() == currentHumValue)
                return;
    ...
    public void requestAlert() {
        ...
        List<ChlorophyllAlertBean> air = getAlertData();
        alertBeans.clear();

        for (int i = 0; i < air.size(); i++) {
            ChlorophyllAlertBean alertBean = air.get(i);
            AlertBeans alertBeansItem = new AlertBeans();
            alertBeansItem.setEhaInfo(alertBean.getInfo());
            alertBeansItem.setEhaTime(alertBean.getChlorophyllAlarmTime());
            alertBeansItem.setEcmDeviceName(alertBean.getChlorophyllName());
...
//        先清空原有的设备数据
        ...
        List<ChlorophyllRealBean> manageData = getManageData();
...
//        适配添加设备的数据
        for (ChlorophyllRealBean b :
            ...
            bean.setDeviceName(b.getChlorophyllName());

            bean.setEhmId(String.valueOf(b.getChlorophyllId()));
            //湿度
            bean.setHumidityMin(String.valueOf(b.getChlorophyllMinData()));
            bean.setHumidityMax(String.valueOf(b.getChlorophyllMaxData()));
            //温度
            bean.setTemperatureMax(String.valueOf(b.getMinTemp()));
            bean.setTemperatureMin(String.valueOf(b.getMaxTemp()));
            //位置
            bean.setPosition(String.valueOf(b.getChlorophyllAddress()));
            //更新时间
...
//        设置 设备管理 中的 添加设备 列表
    ...
    public void queryDeviceManage() {
        //只执行一次
        if (addBeans.size() == 0)
            device8060Present.queryDeviceManage(ChlorophyllRealBean.class);
    }

    @Override
    public void querySelectDeviceAlert(int ehmID, String startTime, String endTime) {
        device8060Present.queryAlertData(ChlorophyllAlertBean.class, ehmID, startTime, endTime);
    }
    ...
    public void onItemClickListener(View v, int position) {
        ...
        try {
            ...
            if (v.findViewById(R.id.btn_panel_device) != null) {
                //先保存数据，点击确定后在进行查询
                setEhmID(waterPressureRealBeans.get(position).getChlorophyllId());
                return;
            }
            //历史数据点击查看 曲线图
            if (v.findViewById(com.example.pool.R.id.new_history_name) != null) {
                ...
                for (ChlorophyllHistoryBean temp :
                        lineDataList) {
                    temps.add(temp.getChlorophyllData() + "");
                    hums.add(temp.getTemp() + "");
                    times.add(temp.getChlorophyllHistoryTime() + "");
                }
                ...
                intent.putExtra("markUnit", waterPressureRealBeans.get(position).getChlorophyllName());
                ...
                intent.putExtra("title", waterPressureRealBeans.get(position).getChlorophyllName());
            ...
            ChlorophyllRealBean b = waterPressureRealBeans.get(position);
            //设置数据
            realDataUtil.setInnerMax(String.valueOf(b.getChlorophyllMaxData()));
            realDataUtil.setInnerData(String.valueOf(b.getChlorophyllData()));
     ...
     */
    @Override
    public Class<ChlorophyllRealBean> getParseClass() {
        return ChlorophyllRealBean.class;
    }

    @Override
    public void bindHistoryData(BaseViewHolder holder, ChlorophyllHistoryBean data, int position) {
        ...
        historyTemp.setText("水流量" + data.getChlorophyllData());
        historyTime.setText("时间" + data.getChlorophyllHistoryTime());
    ...
    public void deleteDevice(int position, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        String ehmId = String.valueOf(waterPressureRealBeans.get(position).getChlorophyllId());
     ...
     */
    @Override
    public void realItemAdapter(BaseViewHolder holder, ChlorophyllRealBean data, int position) {

        TextView item_name = holder.getView(R.id.dissolved_item_name);
        item_name.setText(data.getChlorophyllName());
    ...
    public void changeDevice(String flag, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        ...
        ChlorophyllRealBean temp = waterPressureRealBeans.get(position);
        ...
        deviceManageEditBean.setaDeviceName(temp.getChlorophyllName());
        deviceManageEditBean.setActionID(temp.getChlorophyllId());
        ...
        //浓度
        deviceManageEditBean.setConcentrationRangeMin(temp.getChlorophyllMinData());
        deviceManageEditBean.setConcentrationRangeMax(temp.getChlorophyllMaxData());
    ...
    public void deterMine(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("chlorophyllAddress", getSpinnerString(spinnerMap.get("deviceAddress")));
        //温度
        formBody.put("chlorophyllMaxData", getTextString(keyArray.get("concentrationMax")));
        formBody.put("chlorophyllMinData", getTextString(keyArray.get("concentrationMin")));
        //浓度
        ...
        OkHttpUtil.getInstance().sendRowOkHttp("add", LainNewApi.getInstance().getRootPath() + LainNewApi.chlorophyllInsert, MyGson.getInstance().toJson(formBody), this);
    ...
    @Override
    public void updateRealView(TextView textView, ChlorophyllRealBean data, int position) {
        //设置列表的数据
        textView.setText(data.getChlorophyllName());
    }
}
