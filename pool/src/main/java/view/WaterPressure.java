...
import bean.WaterPressureAlertBean;
import bean.WaterPressureHistoryBean;
...
public class WaterPressure extends Device8060Fragment<WaterPressureRealBean, WaterPressureAlertBean,
        WaterPressureHistoryBean, WaterPressureRealBean> implements BaseRecyclerViewAdapter.OnItemClickListener, I_DeviceDetailManageLink<Device_Detail_AddBean>, I_HistoryAdapter<WaterPressureHistoryBean>, I_RealAdapterListen<WaterPressureRealBean>, I_UpdateRealView<WaterPressureRealBean> {

    private UltimateRecyclerView waterPressureRealRecycler;

    List<WaterPressureRealBean> waterPressureRealBeans = new ArrayList<>();


    //设备实时数据工具类
    public RealDataUtil<WaterPressureRealBean> realDataUtil;

    //-------------------------------------------------
    private BaseDeviceDetailManageAdapter<Device_Detail_AddBean, WaterPressureRealBean> manageAdapter;
...
//    public Temperature_Add deviceDetail_AddAdapter;
    ...
    NewHistoryAdapter<WaterPressureHistoryBean> newHistoryAdapter;
    List<WaterPressureHistoryBean> lineDataList = new ArrayList<>();
    //设备管理的数据
     ...
     */
    public WaterPressure(String realLink, String alertLink, String historyLink, String manageLink) {
        super(realLink, alertLink, historyLink, manageLink);
    }

    public WaterPressure(String realLink, String alertLink, String historyLink, String manageLink, String deleteUrl, String changeUrl) {
        super(realLink, alertLink, historyLink, manageLink);
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        //设备列表
        waterPressureRealRecycler = viewList.get(0).findViewById(R.id.dissolved_real_recycler);
...
//        changeUrl = LainNewApi.tempChange;
        ...
        //查询实时数据
        device8060Present.queryRealData(WaterPressureRealBean.class);
//        startRealUpdate();
        //设置历史数据列表
    ...
    public void queryHistoryData() {
        try {
            ...
            //默认查询第一个设备
            WaterPressureRealBean temperatureBean = waterPressureRealBeans.get(0);
            ///获取设备的ID
            int temEhmId = temperatureBean.getWaterPressureId();
            //结束时间
            String endTime = DateUtil.getInstance().getCurrentDate();
            //开始时间
            String startTime = DateUtil.getInstance().getStartTime();
            //开始查询
            device8060Present.queryHistoryData(WaterPressureHistoryBean.class, temEhmId, startTime, endTime);
    ...
    protected void querySelectDeviceHistory(int ehmID, String startTime, String endTime) {
        device8060Present.queryHistoryData(WaterPressureHistoryBean.class, ehmID, startTime, endTime);
    }
    ...
    public void requestReal() {
        ...
        try {
            ...
            if (alertDeviceBeanList.isEmpty()) {
                //每次查询都保存第一个设备ID
                ehmIDFirst = waterPressureRealBeans.get(0).getWaterPressureId();
                //更新报警查询的设备
                for (WaterPressureRealBean b :
                        waterPressureRealBeans) {
                    AlertDeviceBean alertB = new AlertDeviceBean();
                    alertB.setName(b.getWaterPressureName());
    ...
    public void setCurrentData(int position) {
        try {
            //点击切换上面的圆形进度条的数据
            //设置相关的属性值
            WaterPressureRealBean b = waterPressureRealBeans.get(position);

            if (b.getWaterPressureData() == currentTempValue)
                return;
    ...
    public void requestAlert() {
        ...
        List<WaterPressureAlertBean> air = getAlertData();
        alertBeans.clear();

        for (int i = 0; i < air.size(); i++) {
            WaterPressureAlertBean alertBean = air.get(i);
            AlertBeans alertBeansItem = new AlertBeans();
            alertBeansItem.setEhaInfo(alertBean.getInfo());
            alertBeansItem.setEhaTime(alertBean.getWaterPressureAlarmTime());
            alertBeansItem.setEcmDeviceName(alertBean.getWaterPressureName());
...
//        先清空原有的设备数据
        ...
        List<WaterPressureRealBean> manageData = getManageData();
...
//        适配添加设备的数据
        for (WaterPressureRealBean b :
            ...
            bean.setDeviceName(b.getWaterPressureName());

            bean.setEhmId(String.valueOf(b.getWaterPressureId()));
            //湿度
            bean.setAlertValue(b.getWaterPressureMinData() + " ~ " + b.getWaterPressureMaxData());

            //位置
            bean.setPosition(String.valueOf(b.getWaterPressureAddress()));
            //更新时间
...
//        设置 设备管理 中的 添加设备 列表
    ...
    public void queryDeviceManage() {
        //只执行一次
        if (addBeans.size() == 0)
            device8060Present.queryDeviceManage(WaterPressureRealBean.class);
    }

    @Override
    public void querySelectDeviceAlert(int ehmID, String startTime, String endTime) {
        device8060Present.queryAlertData(WaterPressureAlertBean.class, ehmID, startTime, endTime);
    }
    ...
    public void onItemClickListener(View v, int position) {
        ...
        try {
            ...
            if (v.findViewById(R.id.btn_panel_device) != null) {
                //先保存数据，点击确定后在进行查询
                setEhmID(waterPressureRealBeans.get(position).getWaterPressureId());
                return;
            }
            //历史数据点击查看 曲线图
            if (v.findViewById(com.example.pool.R.id.new_history_name) != null) {
                ...
                for (WaterPressureHistoryBean temp :
                        lineDataList) {
                    temps.add(temp.getWaterPressureData() + "");
                    hums.add(temp.getWaterPressureHistoryTime() + "");
                }
                ...
                intent.putExtra("markUnit", waterPressureRealBeans.get(position).getWaterPressureName());
                ...
                intent.putExtra("title", waterPressureRealBeans.get(position).getWaterPressureName());
            ...
            WaterPressureRealBean b = waterPressureRealBeans.get(position);
            //设置数据
            realDataUtil.setInnerMax(String.valueOf(b.getWaterPressureMaxData()));
            realDataUtil.setInnerData(String.valueOf(b.getWaterPressureData()));
            //设置报警值，必须先设置最大值，再设置当前值，才不会出现问题
            realDataUtil.setOuterMax(String.valueOf(b.getWaterPressureMaxData()));
            realDataUtil.setOuterData(String.valueOf(b.getWaterPressureMaxData()));
            //设置文案
            realDataUtil.setInnerTitle("压力 kPa");
     ...
     */
    @Override
    public Class<WaterPressureRealBean> getParseClass() {
        return WaterPressureRealBean.class;
    }

    @Override
    public void bindHistoryData(BaseViewHolder holder, WaterPressureHistoryBean data, int position) {
        ...
        historyTemp.setText("水流量" + data.getWaterPressureData());
        historyTime.setText("时间" + data.getWaterPressureHistoryTime());
    ...
    public void deleteDevice(int position, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        String ehmId = String.valueOf(waterPressureRealBeans.get(position).getWaterPressureId());
     ...
     */
    @Override
    public void realItemAdapter(BaseViewHolder holder, WaterPressureRealBean data, int position) {

        TextView item_name = holder.getView(R.id.dissolved_item_name);

        item_name.setText(data.getWaterPressureName());
    ...
    public void changeDevice(String flag, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        ...
        WaterPressureRealBean temp = waterPressureRealBeans.get(position);
        ...
        deviceManageEditBean.setaDeviceName(temp.getWaterPressureName());
        deviceManageEditBean.setActionID(temp.getWaterPressureId());


        deviceManageEditBean.setaDeviceInterval(temp.getIntervalTime());
//
        deviceManageEditBean.setRangeMin(temp.getWaterPressureMinData());
        deviceManageEditBean.setRangeMax(temp.getWaterPressureMaxData());
    ...
    public void deterMine(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("waterPressureName", getTextString(keyArray.get("deviceName")));
        formBody.put("waterPressureAddress", getSpinnerString(spinnerMap.get("deviceAddress")));
        formBody.put("waterPressureMaxData", getTextString(keyArray.get("rangeMax")));
        formBody.put("waterPressureMinData", getTextString(keyArray.get("rangeMin")));
        ...
        OkHttpUtil.getInstance().sendRowOkHttp("add", LainNewApi.getInstance().getRootPath() + LainNewApi.waterPressureInsert, MyGson.getInstance().toJson(formBody), this);
    ...
    public void deterChange(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("waterPressureMinData", getTextString(keyArray.get("rangeMin")));
        formBody.put("intervalTime", getTextString(keyArray.get("interval")));
        formBody.put("waterPressureId", getTextString(keyArray.get("actionID")));
    ...
    @Override
    public void updateRealView(TextView textView, WaterPressureRealBean data, int position) {
        //设置列表的数据
        textView.setText(data.getWaterPressureName());
    }
}
