...
import bean.SalinityAlertBean;
import bean.SalinityHistoryBean;
import bean.SalinityRealBean;
import http.MyGson;
import http.OkHttpUtil;
import present.SalinityPresent;
 ...
 * 盐度
 */
public class SalinityFragment extends Device8060Fragment<SalinityRealBean, SalinityAlertBean,
        SalinityHistoryBean, SalinityRealBean> implements BaseRecyclerViewAdapter.OnItemClickListener, I_DeviceDetailManageLink<Device_Detail_AddBean>, I_HistoryAdapter<SalinityHistoryBean>, I_Salinity, I_UpdateRealView<SalinityRealBean> {

    //初始化 present
    private SalinityPresent salinityPresent;
    //设备列表数据
    private UltimateRecyclerView salinityRecycler;

    //实时数据
    private List<SalinityRealBean> salinityRealBeans = new ArrayList<>();


    //设备实时数据工具类
    public RealDataUtil<SalinityRealBean> realDataUtil;

    //-------------------------------------------------
    private BaseDeviceDetailManageAdapter<Device_Detail_AddBean, SalinityRealBean> manageAdapter;
...
//    public Temperature_Add deviceDetail_AddAdapter;
    ...
    NewHistoryAdapter<SalinityHistoryBean> newHistoryAdapter;
    List<SalinityHistoryBean> lineDataList = new ArrayList<>();
    //设备管理的数据
     ...
     */
    public SalinityFragment(String realLink, String alertLink, String historyLink, String manageLink) {
        super(realLink, alertLink, historyLink, manageLink);
    }

    public SalinityFragment(String realLink, String alertLink, String historyLink, String manageLink, String deleteUrl, String changeUrl) {
        super(realLink, alertLink, historyLink, manageLink);
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        //Present
        salinityPresent = new SalinityPresent(this);

        //请求数据
        salinityPresent.dealRealData(Pool_API.Pool_IP + Pool_API.salinity, "salinity_real");
...
//        changeUrl = LainNewApi.tempChange;
        manageAdapter = new BaseDeviceDetailManageAdapter<>(addBeans, salinityRealBeans, getActivity(), R.layout.split_air_manage_swiper, deleteUrl, changeUrl, this, "溶氧检测");
        ...
        //查询实时数据
        device8060Present.queryRealData(SalinityRealBean.class);
//        startRealUpdate();
        //设置历史数据列表
    ...
    private void initRealDataUtil() {
        ...
        realDataUtil.initView(viewList.get(0), getActivity(), salinityRealBeans, this, this);
    }
    ...
    public void queryHistoryData() {
        try {
            ...
            //默认查询第一个设备
            SalinityRealBean temperatureBean = salinityRealBeans.get(0);
            ///获取设备的ID
            int temEhmId = temperatureBean.getSalinityId();
            //结束时间
            String endTime = DateUtil.getInstance().getCurrentDate();
            //开始时间
            String startTime = DateUtil.getInstance().getStartTime();
            //开始查询
            device8060Present.queryHistoryData(SalinityHistoryBean.class, temEhmId, startTime, endTime);
    ...
    protected void querySelectDeviceHistory(int ehmID, String startTime, String endTime) {
        device8060Present.queryHistoryData(SalinityHistoryBean.class, ehmID, startTime, endTime);
    }
    ...
    public void requestReal() {
        ...
        try {
            //先清空
            salinityRealBeans.clear();
            //更新所有的数据，必须调用 getRealData()，才能获取到数据，必须使用 addAll 才能更新列表
            salinityRealBeans.addAll(getRealData());
            ...
            if (alertDeviceBeanList.isEmpty()) {
                //每次查询都保存第一个设备ID
                ehmIDFirst = salinityRealBeans.get(0).getSalinityId();
                //更新报警查询的设备
                for (SalinityRealBean b :
                        salinityRealBeans) {
                    AlertDeviceBean alertB = new AlertDeviceBean();
                    alertB.setName(b.getSalinityName());
    ...
    public void setCurrentData(int position) {
        try {
            //点击切换上面的圆形进度条的数据
            //设置相关的属性值
            SalinityRealBean b = salinityRealBeans.get(position);

            if (b.getSalinityData() == currentTempValue)
                return;
    ...
    public void requestAlert() {
        ...
        List<SalinityAlertBean> air = getAlertData();
        alertBeans.clear();

        for (int i = 0; i < air.size(); i++) {
            SalinityAlertBean alertBean = air.get(i);
            AlertBeans alertBeansItem = new AlertBeans();
            alertBeansItem.setEhaInfo(alertBean.getInfo());
            alertBeansItem.setEhaTime(alertBean.getSalinityAlarmTime());
            alertBeansItem.setEcmDeviceName(alertBean.getSalinityName());
...
//        先清空原有的设备数据
        ...
        List<SalinityRealBean> manageData = getManageData();
...
//        适配添加设备的数据
        for (SalinityRealBean b :
            ...
            bean.setDeviceName(b.getSalinityName());

            bean.setEhmId(String.valueOf(b.getSalinityId()));
            //湿度
            bean.setAlertValue(b.getSalinityMinData() + " ~ " + b.getSalinityMaxData());

            //位置
            bean.setPosition(String.valueOf(b.getSalinityAddress()));
            //更新时间
...
//        设置 设备管理 中的 添加设备 列表
    ...
    public void queryDeviceManage() {
        //只执行一次
        if (addBeans.size() == 0)
            device8060Present.queryDeviceManage(SalinityRealBean.class);
    }

    @Override
    public void querySelectDeviceAlert(int ehmID, String startTime, String endTime) {
        device8060Present.queryAlertData(SalinityAlertBean.class, ehmID, startTime, endTime);
    }
    ...
    public void onItemClickListener(View v, int position) {
        ...
        try {
            ...
            if (v.findViewById(R.id.btn_panel_device) != null) {
                //先保存数据，点击确定后在进行查询
                setEhmID(salinityRealBeans.get(position).getSalinityId());
                return;
            }
            //历史数据点击查看 曲线图
            if (v.findViewById(com.example.pool.R.id.new_history_name) != null) {
                ...
                for (SalinityHistoryBean temp :
                        lineDataList) {
                    temps.add(temp.getSalinityData() + "");
                    hums.add(temp.getSalinityHistoryTime() + "");
                }
                ...
                intent.putExtra("markUnit", salinityRealBeans.get(position).getSalinityName());
                intent.putExtra("linesText", "盐度");
                ...
                intent.putExtra("title", salinityRealBeans.get(position).getSalinityName());
            ...
            SalinityRealBean b = salinityRealBeans.get(position);
            //设置数据
            realDataUtil.setInnerMax(String.valueOf(b.getSalinityMaxData()));
            realDataUtil.setInnerData(String.valueOf(b.getSalinityData()));
            //设置报警值，必须先设置最大值，再设置当前值，才不会出现问题
            realDataUtil.setOuterMax(String.valueOf(b.getSalinityMaxData()));
            realDataUtil.setOuterData(String.valueOf(b.getSalinityMaxData()));
            //设置文案
            realDataUtil.setInnerTitle("盐度 PPT");
     ...
     */
    @Override
    public Class<SalinityRealBean> getParseClass() {
        return SalinityRealBean.class;
    }

    @Override
    public void bindHistoryData(BaseViewHolder holder, SalinityHistoryBean data, int position) {
        ...
        historyTemp.setText("盐度" + data.getSalinityData());
        historyTime.setText("时间" + data.getSalinityHistoryTime());
    ...
    public void deleteDevice(int position, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        String ehmId = String.valueOf(salinityRealBeans.get(position).getSalinityId());
    ...
    @Override
    public void salinityRealComplete(List<SalinityRealBean> salinityRealBeans) {
    ...
    public void changeDevice(String flag, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        ...
        SalinityRealBean temp = salinityRealBeans.get(position);
        ...
        deviceManageEditBean.setaDeviceName(temp.getSalinityName());
        deviceManageEditBean.setActionID(temp.getSalinityId());


        deviceManageEditBean.setaDeviceInterval(temp.getIntervalTime());
//
        deviceManageEditBean.setRangeMin(temp.getSalinityMinData());
        deviceManageEditBean.setRangeMax(temp.getSalinityMaxData());
    ...
    public void deterMine(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("salinityName", getTextString(keyArray.get("deviceName")));
        formBody.put("salinityAddress", getSpinnerString(spinnerMap.get("deviceAddress")));
        formBody.put("salinityMaxData", getTextString(keyArray.get("rangeMax")));
        formBody.put("salinityMinData", getTextString(keyArray.get("rangeMin")));
        ...
        OkHttpUtil.getInstance().sendRowOkHttp("add", LainNewApi.getInstance().getRootPath() + LainNewApi.salinityInsert, MyGson.getInstance().toJson(formBody), this);
    ...
    public void deterChange(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("salinityMinData", getTextString(keyArray.get("rangeMin")));
        formBody.put("intervalTime", getTextString(keyArray.get("interval")));
        formBody.put("salinityId", getTextString(keyArray.get("actionID")));
    ...
    @Override
    public void updateRealView(TextView textView, SalinityRealBean data, int position) {
        //设置列表的数据
        textView.setText(data.getSalinityName());
    }
}
