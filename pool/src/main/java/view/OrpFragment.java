...
import bean.OrpAlertBean;
import bean.OrpHistoryBean;
import bean.OrpRealBean;
import http.MyGson;
...
/**
 * @ClassName: OrpFragment
 * @Description: ORP变送器
 * @Author: YIN LUO FEI
 * @Date: 2020/3/28 0:39
 */
public class OrpFragment extends Device8060Fragment<OrpRealBean, OrpAlertBean,
        OrpHistoryBean, OrpRealBean> implements BaseRecyclerViewAdapter.OnItemClickListener, I_DeviceDetailManageLink<Device_Detail_AddBean>, I_HistoryAdapter<OrpHistoryBean>, I_RealAdapterListen<OrpRealBean> , I_UpdateRealView<OrpRealBean> {

    private UltimateRecyclerView waterPressureRealRecycler;

    List<OrpRealBean> waterPressureRealBeans = new ArrayList<>();


    //-------------------------------------------------
    private BaseDeviceDetailManageAdapter<Device_Detail_AddBean, OrpRealBean> manageAdapter;
...
//    public Temperature_Add deviceDetail_AddAdapter;
    ...
    String deleteUrl = "";
    String changeUrl = "";
    NewHistoryAdapter<OrpHistoryBean> newHistoryAdapter;
    List<OrpHistoryBean> lineDataList = new ArrayList<>();
    //设备管理的数据
    private ArrayList<Device_Detail_AddBean> addBeans = new ArrayList<>();
    private static LayoutInflater layoutInflater;

    //设备实时数据工具类
    public RealDataUtil<OrpRealBean> realDataUtil;
     ...
     */
    public OrpFragment(String realLink, String alertLink, String historyLink, String manageLink) {
        super(realLink, alertLink, historyLink, manageLink);
    }

    public OrpFragment(String realLink, String alertLink, String historyLink, String manageLink, String deleteUrl, String changeUrl) {
        super(realLink, alertLink, historyLink, manageLink);
...
//        changeUrl = LainNewApi.tempChange;
        ...
        //查询实时数据
        device8060Present.queryRealData(OrpRealBean.class);
//        startRealUpdate();
        //设置历史数据列表
    ...
    public void queryHistoryData() {
        try {
            ...
            //默认查询第一个设备
            OrpRealBean temperatureBean = waterPressureRealBeans.get(0);
            ///获取设备的ID

            int temEhmId = temperatureBean.getOrpMeterId();
            //结束时间
            String endTime = DateUtil.getInstance().getCurrentDate();
            //开始时间
            String startTime = DateUtil.getInstance().getStartTime();
            //开始查询
            device8060Present.queryHistoryData(OrpHistoryBean.class, temEhmId, startTime, endTime);
    ...
    protected void querySelectDeviceHistory(int ehmID, String startTime, String endTime) {
        device8060Present.queryHistoryData(OrpHistoryBean.class, ehmID, startTime, endTime);
    }
    ...
    public void requestReal() {
        ...
        try {
            ...
            if (alertDeviceBeanList.isEmpty()) {
                //每次查询都保存第一个设备ID
                ehmIDFirst = waterPressureRealBeans.get(0).getOrpMeterId();
                //更新报警查询的设备
                for (OrpRealBean b :
                        waterPressureRealBeans) {
                    AlertDeviceBean alertB = new AlertDeviceBean();

                    alertB.setName(b.getOrpMeterName());
    ...
    public void setCurrentData(int position) {
        try {
            //点击切换上面的圆形进度条的数据
            //设置相关的属性值
            OrpRealBean b = waterPressureRealBeans.get(position);

            if (b.getOrpMeterData() == currentTempValue)
                return;
    ...
    public void requestAlert() {
        ...
        List<OrpAlertBean> air = getAlertData();
        alertBeans.clear();

        for (int i = 0; i < air.size(); i++) {
            OrpAlertBean alertBean = air.get(i);
            AlertBeans alertBeansItem = new AlertBeans();
            alertBeansItem.setEhaInfo(alertBean.getInfo());
            alertBeansItem.setEhaTime(alertBean.getOrpMeterAlarmTime());
            alertBeansItem.setEcmDeviceName(alertBean.getOrpMeterName());
...
//        先清空原有的设备数据
        ...
        List<OrpRealBean> manageData = getManageData();
...
//        适配添加设备的数据
        for (OrpRealBean b :
            ...
            bean.setDeviceName(b.getOrpMeterName());

            bean.setEhmId(String.valueOf(b.getOrpMeterId()));
            //湿度
            bean.setAlertValue(b.getOrpMeterMinData() + " ~ " + b.getOrpMeterMaxData());

            //位置
            bean.setPosition(String.valueOf(b.getOrpMeterAddress()));
            //更新时间
...
//        设置 设备管理 中的 添加设备 列表
    ...
    public void queryDeviceManage() {
        //只执行一次
        if (addBeans.size() == 0)
            device8060Present.queryDeviceManage(OrpRealBean.class);
    }

    @Override
    public void querySelectDeviceAlert(int ehmID, String startTime, String endTime) {
        device8060Present.queryAlertData(OrpAlertBean.class, ehmID, startTime, endTime);
    }
    ...
    public void onItemClickListener(View v, int position) {
        ...
        try {
            ...
            if (v.findViewById(R.id.btn_panel_device) != null) {
                //先保存数据，点击确定后在进行查询
                setEhmID(waterPressureRealBeans.get(position).getOrpMeterId());
                return;
            }
            //历史数据点击查看 曲线图
            if (v.findViewById(com.example.pool.R.id.new_history_name) != null) {
                ...
                for (OrpHistoryBean temp :
                        lineDataList) {
                    temps.add(temp.getOrpMeterData() + "");
                    hums.add(temp.getOrpMeterHistoryTime() + "");
                }
                ...
                intent.putExtra("markUnit", waterPressureRealBeans.get(position).getOrpMeterName());
                ...
                intent.putExtra("title", waterPressureRealBeans.get(position).getOrpMeterName());
            ...
            OrpRealBean b = waterPressureRealBeans.get(position);
//设置数据
            realDataUtil.setInnerMax(String.valueOf(b.getOrpMeterMaxData()));
            realDataUtil.setInnerData(String.valueOf(b.getOrpMeterData()));
            //设置报警值，必须先设置最大值，再设置当前值，才不会出现问题
            realDataUtil.setOuterMax(String.valueOf(b.getOrpMeterMaxData()));
            realDataUtil.setOuterData(String.valueOf(b.getOrpMeterMaxData()));
            //设置文案
            realDataUtil.setInnerTitle("量程 mV");
     ...
     */
    @Override
    public Class<OrpRealBean> getParseClass() {
        return OrpRealBean.class;
    }

    @Override
    public void bindHistoryData(BaseViewHolder holder, OrpHistoryBean data, int position) {
        ...
        historyTemp.setText("水流量" + data.getOrpMeterData());
        historyTime.setText("时间" + data.getOrpMeterHistoryTime());
    ...
    public void deleteDevice(int position, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        String ehmId = String.valueOf(waterPressureRealBeans.get(position).getOrpMeterId());
     ...
     */
    @Override
    public void realItemAdapter(BaseViewHolder holder, OrpRealBean data, int position) {

        TextView item_name = holder.getView(R.id.dissolved_item_name);
        item_name.setText(data.getOrpMeterName());
    ...
    public void changeDevice(String flag, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        ...
        OrpRealBean temp = waterPressureRealBeans.get(position);
        ...
        deviceManageEditBean.setaDeviceName(temp.getOrpMeterName());
        deviceManageEditBean.setActionID(temp.getOrpMeterId());


        deviceManageEditBean.setaDeviceInterval(temp.getIntervalTime());
//
        deviceManageEditBean.setRangeMin(temp.getOrpMeterMinData());
        deviceManageEditBean.setRangeMax(temp.getOrpMeterMaxData());
    ...
    public void deterMine(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("orpMeterName", getTextString(keyArray.get("deviceName")));
        formBody.put("orpMeterAddress", getSpinnerString(spinnerMap.get("deviceAddress")));
        formBody.put("orpMeterMaxData", getTextString(keyArray.get("rangeMax")));
        formBody.put("orpMeterMinData", getTextString(keyArray.get("rangeMin")));
        ...
        OkHttpUtil.getInstance().sendRowOkHttp("add", LainNewApi.getInstance().getRootPath() + LainNewApi.orpInsert, MyGson.getInstance().toJson(formBody), this);
    ...
    public void deterChange(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("orpMeterMinData", getTextString(keyArray.get("rangeMin")));
        formBody.put("intervalTime", getTextString(keyArray.get("interval")));
        formBody.put("orpMeterId", getTextString(keyArray.get("actionID")));
    ...
    @Override
    public void updateRealView(TextView textView, OrpRealBean data, int position) {
        //设置列表的数据
        textView.setText(data.getOrpMeterName());
    }
}
