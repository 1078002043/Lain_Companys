...
import bean.WaterLevelAlertBean;
import bean.WaterLevelHistoryBean;
import http.MyGson;
import http.OkHttpUtil;
import present.WaterLevelPresent;
 ...
 * 水位检测
 */
public class WaterLevelFragment extends Device8060Fragment<WaterLevelRealBean, WaterLevelAlertBean,
        WaterLevelHistoryBean, WaterLevelRealBean> implements BaseRecyclerViewAdapter.OnItemClickListener, I_DeviceDetailManageLink<Device_Detail_AddBean>, I_HistoryAdapter<WaterLevelHistoryBean>, I_WaterLevel, I_UpdateRealView<WaterLevelRealBean> {

    //初始化 present
    private WaterLevelPresent waterLevelPresent;
    //设备列表数据
    private UltimateRecyclerView waterLevelRecycler;

    //实时数据
    private List<WaterLevelRealBean> waterLevelRealBeans = new ArrayList<>();

    //设备实时数据工具类
    public RealDataUtil<WaterLevelRealBean> realDataUtil;
    //-------------------------------------------------
    private BaseDeviceDetailManageAdapter<Device_Detail_AddBean, WaterLevelRealBean> manageAdapter;
...
//    public Temperature_Add deviceDetail_AddAdapter;
    ...
    NewHistoryAdapter<WaterLevelHistoryBean> newHistoryAdapter;
    List<WaterLevelHistoryBean> lineDataList = new ArrayList<>();
    //设备管理的数据
     ...
     */
    public WaterLevelFragment(String realLink, String alertLink, String historyLink, String manageLink) {
        super(realLink, alertLink, historyLink, manageLink);
    }

    public WaterLevelFragment(String realLink, String alertLink, String historyLink, String manageLink, String deleteUrl, String changeUrl) {
        super(realLink, alertLink, historyLink, manageLink);
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        //Present
        waterLevelPresent = new WaterLevelPresent(this);

        //请求数据
        waterLevelPresent.dealRealData(Pool_API.Pool_IP + Pool_API.waterLevel, "water_level_real");
...
//        changeUrl = LainNewApi.tempChange;
        manageAdapter = new BaseDeviceDetailManageAdapter<>(addBeans, waterLevelRealBeans, getActivity(), R.layout.split_air_manage_swiper, deleteUrl, changeUrl, this, "溶氧检测");
        ...
        //查询实时数据
        device8060Present.queryRealData(WaterLevelRealBean.class);
//        startRealUpdate();
        //设置历史数据列表
    ...
    private void initRealDataUtil() {
        ...
        realDataUtil.initView(viewList.get(0), getActivity(), waterLevelRealBeans, this, this);
    }
    ...
    public void queryHistoryData() {
        try {
            ...
            //默认查询第一个设备
            WaterLevelRealBean temperatureBean = waterLevelRealBeans.get(0);
            ///获取设备的ID
            int temEhmId = temperatureBean.getWaterLevelId();
            //结束时间
            String endTime = DateUtil.getInstance().getCurrentDate();
            //开始时间
            String startTime = DateUtil.getInstance().getStartTime();
            //开始查询
            device8060Present.queryHistoryData(WaterLevelHistoryBean.class, temEhmId, startTime, endTime);
    ...
    protected void querySelectDeviceHistory(int ehmID, String startTime, String endTime) {
        device8060Present.queryHistoryData(WaterLevelHistoryBean.class, ehmID, startTime, endTime);
    }
    ...
    public void requestReal() {
        ...
        try {
            //先清空
            waterLevelRealBeans.clear();
            //更新所有的数据，必须调用 getRealData()，才能获取到数据，必须使用 addAll 才能更新列表
            waterLevelRealBeans.addAll(getRealData());
            ...
            if (alertDeviceBeanList.isEmpty()) {
                //每次查询都保存第一个设备ID
                ehmIDFirst = waterLevelRealBeans.get(0).getWaterLevelId();
                //更新报警查询的设备
                for (WaterLevelRealBean b :
                        waterLevelRealBeans) {
                    AlertDeviceBean alertB = new AlertDeviceBean();
                    alertB.setName(b.getWaterLevelName());
    ...
    public void setCurrentData(int position) {
        try {
            //点击切换上面的圆形进度条的数据
            //设置相关的属性值
            WaterLevelRealBean b = waterLevelRealBeans.get(position);

            if (b.getWaterLevelData() == currentTempValue)
                return;
    ...
    public void requestAlert() {
        ...
        List<WaterLevelAlertBean> air = getAlertData();
        alertBeans.clear();

        for (int i = 0; i < air.size(); i++) {
            WaterLevelAlertBean alertBean = air.get(i);
            AlertBeans alertBeansItem = new AlertBeans();
            alertBeansItem.setEhaInfo(alertBean.getInfo());
            alertBeansItem.setEhaTime(alertBean.getWaterLevelAlarmTime());
            alertBeansItem.setEcmDeviceName(alertBean.getWaterLevelName());
...
//        先清空原有的设备数据
        ...
        List<WaterLevelRealBean> manageData = getManageData();
...
//        适配添加设备的数据
        for (WaterLevelRealBean b :
            ...
            bean.setDeviceName(b.getWaterLevelName());

            bean.setEhmId(String.valueOf(b.getWaterLevelId()));
            //湿度
            bean.setAlertValue(b.getWaterLevelMinData() + " ~ " + b.getWaterLevelMaxData());

            //位置
            bean.setPosition(String.valueOf(b.getWaterLevelAddress()));
            //更新时间
...
//        设置 设备管理 中的 添加设备 列表
    ...
    public void queryDeviceManage() {
        //只执行一次
        if (addBeans.size() == 0)
            device8060Present.queryDeviceManage(WaterLevelRealBean.class);
    }

    @Override
    public void querySelectDeviceAlert(int ehmID, String startTime, String endTime) {
        device8060Present.queryAlertData(WaterLevelAlertBean.class, ehmID, startTime, endTime);
    }
    ...
    public void onItemClickListener(View v, int position) {
        ...
        try {
            ...
            if (v.findViewById(R.id.btn_panel_device) != null) {
                //先保存数据，点击确定后在进行查询
                setEhmID(waterLevelRealBeans.get(position).getWaterLevelId());
                return;
            }
            //历史数据点击查看 曲线图
            if (v.findViewById(com.example.pool.R.id.new_history_name) != null) {
                ...
                for (WaterLevelHistoryBean temp :
                        lineDataList) {
                    temps.add(temp.getWaterLevelData() + "");
                    hums.add(temp.getWaterLevelHistoryTime() + "");
                }
                ...
                intent.putExtra("markUnit", waterLevelRealBeans.get(position).getWaterLevelName());
                ...
                intent.putExtra("title", waterLevelRealBeans.get(position).getWaterLevelName());
            ...
            WaterLevelRealBean b = waterLevelRealBeans.get(position);
            //设置数据
            realDataUtil.setInnerMax(String.valueOf(b.getWaterLevelMaxData()));
            realDataUtil.setInnerData(String.valueOf(b.getWaterLevelData()));
            //设置报警值，必须先设置最大值，再设置当前值，才不会出现问题
            realDataUtil.setOuterMax(String.valueOf(b.getWaterLevelMaxData()));
            realDataUtil.setOuterData(String.valueOf(b.getWaterLevelMaxData()));
            //设置文案
            realDataUtil.setInnerTitle("量程 CM");
     ...
     */
    @Override
    public Class<WaterLevelRealBean> getParseClass() {
        return WaterLevelRealBean.class;
    }

    @Override
    public void bindHistoryData(BaseViewHolder holder, WaterLevelHistoryBean data, int position) {
        ...
        historyTemp.setText("水流量" + data.getWaterLevelData());
        historyTime.setText("时间" + data.getWaterLevelHistoryTime());
    ...
    public void deleteDevice(int position, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        String ehmId = String.valueOf(waterLevelRealBeans.get(position).getWaterLevelId());
    ...
    @Override
    public void waterLevelRealComplete(List<WaterLevelRealBean> waterLevelRealBeans) {
    ...
    public void changeDevice(String flag, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        ...
        WaterLevelRealBean temp = waterLevelRealBeans.get(position);
        ...
        deviceManageEditBean.setaDeviceName(temp.getWaterLevelName());
        deviceManageEditBean.setActionID(temp.getWaterLevelId());


        deviceManageEditBean.setaDeviceInterval(temp.getIntervalTime());
//
        deviceManageEditBean.setRangeMin(temp.getWaterLevelMinData());
        deviceManageEditBean.setRangeMax(temp.getWaterLevelMaxData());
    ...
    public void deterMine(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("waterLevelName", getTextString(keyArray.get("deviceName")));
        formBody.put("waterLevelAddress", getSpinnerString(spinnerMap.get("deviceAddress")));
        formBody.put("waterLevelMaxData", getTextString(keyArray.get("rangeMax")));
        formBody.put("waterLevelMinData", getTextString(keyArray.get("rangeMin")));
        ...
        OkHttpUtil.getInstance().sendRowOkHttp("add", LainNewApi.getInstance().getRootPath() + LainNewApi.waterLevelInsert, MyGson.getInstance().toJson(formBody), this);
    ...
    public void deterChange(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("waterLevelMinData", getTextString(keyArray.get("rangeMin")));
        formBody.put("intervalTime", getTextString(keyArray.get("interval")));
        formBody.put("waterLevelId", getTextString(keyArray.get("actionID")));
    ...
    @Override
    public void updateRealView(TextView textView, WaterLevelRealBean data, int position) {
        //设置列表的数据
        textView.setText(data.getWaterLevelName());
    }
}
