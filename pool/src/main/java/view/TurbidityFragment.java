...
import bean.TurbidityRealBean;
import bean.Device_Detail_AddBean;
import bean.TurbidityAlertBean;
import bean.TurbidityHistoryBean;
import http.MyGson;
...
/**
 * @ClassName: TurbidityFragment
 * @Description: 浑浊度
 * @Author: YIN LUO FEI
 * @Date: 2020/3/27 22:07
 */
public class TurbidityFragment extends Device8060Fragment<TurbidityRealBean, TurbidityAlertBean,
        TurbidityHistoryBean, TurbidityRealBean> implements BaseRecyclerViewAdapter.OnItemClickListener, I_DeviceDetailManageLink<Device_Detail_AddBean>, I_HistoryAdapter<TurbidityHistoryBean>, I_RealAdapterListen<TurbidityRealBean>, I_UpdateRealView<TurbidityRealBean> {

    private UltimateRecyclerView waterPressureRealRecycler;

    List<TurbidityRealBean> waterPressureRealBeans = new ArrayList<>();


    //-------------------------------------------------
    private BaseDeviceDetailManageAdapter<Device_Detail_AddBean, TurbidityRealBean> manageAdapter;
...
//    public Temperature_Add deviceDetail_AddAdapter;
    ...
    //删除URL
    NewHistoryAdapter<TurbidityHistoryBean> newHistoryAdapter;
    List<TurbidityHistoryBean> lineDataList = new ArrayList<>();
    //设备管理的数据
    ...
    //设备实时数据工具类
    public RealDataUtil<TurbidityRealBean> realDataUtil;
     ...
     */
    public TurbidityFragment(String realLink, String alertLink, String historyLink, String manageLink) {
        super(realLink, alertLink, historyLink, manageLink);
    }

    public TurbidityFragment(String realLink, String alertLink, String historyLink, String manageLink, String deleteUrl, String changeUrl) {
        super(realLink, alertLink, historyLink, manageLink);
...
//        changeUrl = LainNewApi.tempChange;
        ...
        //查询实时数据
        device8060Present.queryRealData(TurbidityRealBean.class);
//        startRealUpdate();
        //设置历史数据列表
    ...
    public void queryHistoryData() {
        try {
            ...
            //默认查询第一个设备
            TurbidityRealBean temperatureBean = waterPressureRealBeans.get(0);
            ///获取设备的ID

            int temEhmId = temperatureBean.getMuddyId();
            //结束时间
            String endTime = DateUtil.getInstance().getCurrentDate();
            //开始时间
            String startTime = DateUtil.getInstance().getStartTime();
            //开始查询
            device8060Present.queryHistoryData(TurbidityHistoryBean.class, temEhmId, startTime, endTime);
    ...
    protected void querySelectDeviceHistory(int ehmID, String startTime, String endTime) {
        device8060Present.queryHistoryData(TurbidityHistoryBean.class, ehmID, startTime, endTime);
    }
    ...
    public void requestReal() {
        ...
        try {
            ...
            if (alertDeviceBeanList.isEmpty()) {
                //每次查询都保存第一个设备ID
                ehmIDFirst = waterPressureRealBeans.get(0).getMuddyId();
                //更新报警查询的设备
                for (TurbidityRealBean b :
                        waterPressureRealBeans) {
                    AlertDeviceBean alertB = new AlertDeviceBean();
                    alertB.setName(b.getMuddyName());
    ...
    public void setCurrentData(int position) {
        try {
            //点击切换上面的圆形进度条的数据
            //设置相关的属性值
            TurbidityRealBean b = waterPressureRealBeans.get(position);

            if (b.getMuddyData() == currentTempValue)
                return;
    ...
    public void requestAlert() {
        ...
        List<TurbidityAlertBean> air = getAlertData();
        alertBeans.clear();

        for (int i = 0; i < air.size(); i++) {
            TurbidityAlertBean alertBean = air.get(i);
            AlertBeans alertBeansItem = new AlertBeans();
            alertBeansItem.setEhaInfo(alertBean.getInfo());
            alertBeansItem.setEhaTime(alertBean.getMuddyAlarmTime());
            alertBeansItem.setEcmDeviceName(alertBean.getMuddyName());
...
//        先清空原有的设备数据
        ...
        List<TurbidityRealBean> manageData = getManageData();
...
//        适配添加设备的数据
        for (TurbidityRealBean b :
            ...
            bean.setDeviceName(b.getMuddyName());

            bean.setEhmId(String.valueOf(b.getMuddyId()));
            //湿度
            bean.setAlertValue(b.getMuddyMinData() + " ~ " + b.getMuddyMaxData());

            //位置
            bean.setPosition(String.valueOf(b.getMuddyAddress()));
            //更新时间
...
//        设置 设备管理 中的 添加设备 列表
    ...
    public void queryDeviceManage() {
        //只执行一次
        if (addBeans.size() == 0)
            device8060Present.queryDeviceManage(TurbidityRealBean.class);
    }

    @Override
    public void querySelectDeviceAlert(int ehmID, String startTime, String endTime) {
        device8060Present.queryAlertData(TurbidityAlertBean.class, ehmID, startTime, endTime);
    }
    ...
    public void onItemClickListener(View v, int position) {
        ...
        try {
            ...
            if (v.findViewById(R.id.btn_panel_device) != null) {
                //先保存数据，点击确定后在进行查询
                setEhmID(waterPressureRealBeans.get(position).getMuddyId());
                return;
            }
            //历史数据点击查看 曲线图
            if (v.findViewById(com.example.pool.R.id.new_history_name) != null) {
                ...
                for (TurbidityHistoryBean temp :
                        lineDataList) {
                    temps.add(temp.getMuddyData() + "");
                    hums.add(temp.getMuddyHistoryTime() + "");
                }
                ...
                intent.putExtra("markUnit", waterPressureRealBeans.get(position).getMuddyName());
                ...
                intent.putExtra("title", waterPressureRealBeans.get(position).getMuddyName());
            ...
            TurbidityRealBean b = waterPressureRealBeans.get(position);
            //设置数据
            realDataUtil.setInnerMax(String.valueOf(b.getMuddyMaxData()));
            realDataUtil.setInnerData(String.valueOf(b.getMuddyData()));
            //设置报警值，必须先设置最大值，再设置当前值，才不会出现问题
            realDataUtil.setOuterMax(String.valueOf(b.getMuddyMaxData()));
            realDataUtil.setOuterData(String.valueOf(b.getMuddyMaxData()));
            //设置文案
            realDataUtil.setInnerTitle("浊度 NTU");
     ...
     */
    @Override
    public Class<TurbidityRealBean> getParseClass() {
        return TurbidityRealBean.class;
    }

    @Override
    public void bindHistoryData(BaseViewHolder holder, TurbidityHistoryBean data, int position) {
        ...
        historyTemp.setText("水流量" + data.getMuddyData());
        historyTime.setText("时间" + data.getMuddyHistoryTime());
    ...
    public void deleteDevice(int position, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        String ehmId = String.valueOf(waterPressureRealBeans.get(position).getMuddyId());
     ...
     */
    @Override
    public void realItemAdapter(BaseViewHolder holder, TurbidityRealBean data, int position) {

        TextView item_name = holder.getView(R.id.dissolved_item_name);
        item_name.setText(data.getMuddyName());
    ...
    public void changeDevice(String flag, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        ...
        TurbidityRealBean temp = waterPressureRealBeans.get(position);
        ...
        deviceManageEditBean.setaDeviceName(temp.getMuddyName());
        deviceManageEditBean.setActionID(temp.getMuddyId());


        deviceManageEditBean.setaDeviceInterval(temp.getIntervalTime());
//
        deviceManageEditBean.setRangeMin(temp.getMuddyMinData());
        deviceManageEditBean.setRangeMax(temp.getMuddyMaxData());
    ...
    public void deterMine(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("muddyName", getTextString(keyArray.get("deviceName")));
        formBody.put("muddyAddress", getSpinnerString(spinnerMap.get("deviceAddress")));
        formBody.put("muddyMaxData", getTextString(keyArray.get("rangeMax")));
        formBody.put("muddyMinData", getTextString(keyArray.get("rangeMin")));
        ...
        OkHttpUtil.getInstance().sendRowOkHttp("add", LainNewApi.getInstance().getRootPath() + LainNewApi.turbidityInsert, MyGson.getInstance().toJson(formBody), this);
    ...
    public void deterChange(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("muddyMinData", getTextString(keyArray.get("rangeMin")));
        formBody.put("intervalTime", getTextString(keyArray.get("interval")));
        formBody.put("muddyId", getTextString(keyArray.get("actionID")));
    ...
    @Override
    public void updateRealView(TextView textView, TurbidityRealBean data, int position) {
        //设置列表的数据
        textView.setText(data.getMuddyName());
    }
}
