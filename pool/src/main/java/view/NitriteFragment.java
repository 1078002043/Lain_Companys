...
import bean.NitriteAlertBean;
import bean.NitriteHistoryBean;
 ...
 * 亚硝酸盐
 */
public class NitriteFragment extends Device8060Fragment<NitriteRealBean, NitriteAlertBean,
        NitriteHistoryBean, NitriteRealBean> implements BaseRecyclerViewAdapter.OnItemClickListener, I_DeviceDetailManageLink<Device_Detail_AddBean>, I_HistoryAdapter<NitriteHistoryBean>, I_Nitrite , I_UpdateRealView<NitriteRealBean> {

    //初始化 present
    private NitritePresent nitritePresent;
    //设备列表数据
    private UltimateRecyclerView nitriteRecycler;

    //实时数据
    private List<NitriteRealBean> nitriteRealBeans = new ArrayList<>();

    //-------------------
    private BaseDeviceDetailManageAdapter<Device_Detail_AddBean, NitriteRealBean> manageAdapter;
...
//    public Temperature_Add deviceDetail_AddAdapter;
    ...
    String deleteUrl = "";
    String changeUrl = "";
    NewHistoryAdapter<NitriteHistoryBean> newHistoryAdapter;
    List<NitriteHistoryBean> lineDataList = new ArrayList<>();
    //设备管理的数据
    private ArrayList<Device_Detail_AddBean> addBeans = new ArrayList<>();
    private static LayoutInflater layoutInflater;

    //设备实时数据工具类
    public RealDataUtil<NitriteRealBean> realDataUtil;
     ...
     */
    public NitriteFragment(String realLink, String alertLink, String historyLink, String manageLink) {
        super(realLink, alertLink, historyLink, manageLink);
    }

    public NitriteFragment(String realLink, String alertLink, String historyLink, String manageLink, String deleteUrl, String changeUrl) {
        super(realLink, alertLink, historyLink, manageLink);
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        //Present
        nitritePresent = new NitritePresent(this);

        //请求数据
        nitritePresent.dealRealData(Pool_API.Pool_IP + Pool_API.nitrite, "nitrite_real");
...
//        changeUrl = LainNewApi.tempChange;
        ...
        manageAdapter = new BaseDeviceDetailManageAdapter<>(addBeans, nitriteRealBeans, getActivity(), R.layout.split_air_manage_swiper, deleteUrl, changeUrl, this, "溶氧检测");
        ...
        //查询实时数据
        device8060Present.queryRealData(NitriteRealBean.class);
//        startRealUpdate();
        //设置历史数据列表
...
//初始化实时数据工具类
    ...
    private void initRealDataUtil() {
        ...
        realDataUtil.initView(viewList.get(0), getActivity(), nitriteRealBeans, this, this);
    }
    ...
    public void queryHistoryData() {
        try {
            //默认查询第一个设备
            NitriteRealBean temperatureBean = nitriteRealBeans.get(0);
            ///获取设备的ID
            int temEhmId = temperatureBean.getNitriteId();
            //结束时间
            String endTime = DateUtil.getInstance().getCurrentDate();
            //开始时间
            String startTime = DateUtil.getInstance().getStartTime();
            //开始查询
            device8060Present.queryHistoryData(NitriteHistoryBean.class, temEhmId, startTime, endTime);
    ...
    protected void querySelectDeviceHistory(int ehmID, String startTime, String endTime) {
        device8060Present.queryHistoryData(NitriteHistoryBean.class, ehmID, startTime, endTime);
    }
    ...
    public void requestReal() {
        ...
        try {
            //先清空
            nitriteRealBeans.clear();
            //更新所有的数据，必须调用 getRealData()，才能获取到数据，必须使用 addAll 才能更新列表
            nitriteRealBeans.addAll(getRealData());
            ...
            if (alertDeviceBeanList.isEmpty()) {
                //每次查询都保存第一个设备ID
                ehmIDFirst = nitriteRealBeans.get(0).getNitriteId();
                //更新报警查询的设备
                for (NitriteRealBean b :
                        nitriteRealBeans) {
                    AlertDeviceBean alertB = new AlertDeviceBean();
                    alertB.setName(b.getNitriteName());
    ...
    public void setCurrentData(int position) {
        try {
            //点击切换上面的圆形进度条的数据
            //设置相关的属性值
            NitriteRealBean b = nitriteRealBeans.get(position);

            if (b.getNitriteData() == currentTempValue)
                return;
    ...
    public void requestAlert() {
        List<NitriteAlertBean> air = getAlertData();
        alertBeans.clear();

        for (int i = 0; i < air.size(); i++) {
            NitriteAlertBean alertBean = air.get(i);
            AlertBeans alertBeansItem = new AlertBeans();
            alertBeansItem.setEhaInfo(alertBean.getInfo());
            alertBeansItem.setEhaTime(alertBean.getNitriteAlarmTime());
            alertBeansItem.setEcmDeviceName(alertBean.getNitriteName());
...
//        先清空原有的设备数据
        ...
        List<NitriteRealBean> manageData = getManageData();
...
//        适配添加设备的数据
        for (NitriteRealBean b :
            ...
            bean.setDeviceName(b.getNitriteName());

            bean.setEhmId(String.valueOf(b.getNitriteId()));
            //湿度
            bean.setAlertValue(b.getNitriteMinData() + " ~ " + b.getNitriteMaxData());

            //位置
            bean.setPosition(String.valueOf(b.getNitriteAddress()));
            //更新时间
...
//        设置 设备管理 中的 添加设备 列表
    ...
    public void querySelectDeviceAlert(int ehmID, String startTime, String endTime) {
        device8060Present.queryAlertData(NitriteAlertBean.class, ehmID, startTime, endTime);
    }
    ...
    public void onItemClickListener(View v, int position) {
        ...
        try {
            ...
            if (v.findViewById(R.id.btn_panel_device) != null) {
                //先保存数据，点击确定后在进行查询
                setEhmID(nitriteRealBeans.get(position).getNitriteId());
                return;
            }
            //历史数据点击查看 曲线图
            if (v.findViewById(com.example.pool.R.id.new_history_name) != null) {
                ...
                for (NitriteHistoryBean temp :
                        lineDataList) {
                    temps.add(temp.getNitriteData() + "");
                    hums.add(temp.getNitriteHistoryTime() + "");
                }
                ...
                intent.putExtra("title", nitriteRealBeans.get(position).getNitriteName());
            ...
            NitriteRealBean b = nitriteRealBeans.get(position);
            //设置数据
            realDataUtil.setInnerMax(String.valueOf(b.getNitriteMaxData()));
            realDataUtil.setInnerData(String.valueOf(b.getNitriteData()));
            //设置报警值，必须先设置最大值，再设置当前值，才不会出现问题
            realDataUtil.setOuterMax(String.valueOf(b.getNitriteMaxData()));
            realDataUtil.setOuterData(String.valueOf(b.getNitriteMaxData()));
            //设置文案
            realDataUtil.setInnerTitle("重量 mg/kg");
     ...
     */
    @Override
    public Class<NitriteRealBean> getParseClass() {
        return NitriteRealBean.class;
    }

    @Override
    public void bindHistoryData(BaseViewHolder holder, NitriteHistoryBean data, int position) {
        ...
        historyTemp.setText("温度" + data.getNitriteData());
        historyTime.setText("时间" + data.getNitriteHistoryTime());
    ...
    public void deleteDevice(int position, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        String ehmId = String.valueOf(nitriteRealBeans.get(position).getNitriteId());
    ...
    public void queryDeviceManage() {
        //只执行一次
        if (addBeans.size() == 0)
            device8060Present.queryDeviceManage(NitriteRealBean.class);
    }
    ...
    @Override
    public void nitriteRealComplete(List<NitriteRealBean> nitriteRealBeans) {
    ...
    public void changeDevice(String flag, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        ...
        NitriteRealBean temp = nitriteRealBeans.get(position);
        ...
        deviceManageEditBean.setaDeviceName(temp.getNitriteName());
        deviceManageEditBean.setActionID(temp.getNitriteId());


        deviceManageEditBean.setaDeviceInterval(temp.getIntervalTime());
//
        deviceManageEditBean.setRangeMin(temp.getNitriteMinData());
        deviceManageEditBean.setRangeMax(temp.getNitriteMaxData());
    ...
    public void deterMine(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("nitriteName", getTextString(keyArray.get("deviceName")));
        formBody.put("nitriteAddress", getSpinnerString(spinnerMap.get("deviceAddress")));
        formBody.put("nitriteMaxData", getTextString(keyArray.get("rangeMax")));
        formBody.put("nitriteMinData", getTextString(keyArray.get("rangeMin")));
        ...
        OkHttpUtil.getInstance().sendRowOkHttp("add", LainNewApi.getInstance().getRootPath() + LainNewApi.nitriteInsert, MyGson.getInstance().toJson(formBody), this);
    ...
    public void deterChange(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("nitriteMinData", getTextString(keyArray.get("rangeMin")));
        formBody.put("intervalTime", getTextString(keyArray.get("interval")));
        formBody.put("nitriteId", getTextString(keyArray.get("actionID")));
    ...
    @Override
    public void updateRealView(TextView textView, NitriteRealBean data, int position) {
        //设置列表的数据
        textView.setText(data.getNitriteName());
    }
}
