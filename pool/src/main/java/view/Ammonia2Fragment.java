...
import bean.Ammonia2AlertBean;
import bean.Ammonia2HistoryBean;
import bean.Ammonia2RealBean;
 ...
 * 氨气
 */
public class Ammonia2Fragment extends Device8060Fragment<Ammonia2RealBean, Ammonia2AlertBean,
        Ammonia2HistoryBean, Ammonia2RealBean> implements BaseRecyclerViewAdapter.OnItemClickListener, I_DeviceDetailManageLink<Device_Detail_AddBean>, I_HistoryAdapter<Ammonia2HistoryBean>, I_RealAdapterListen<Ammonia2RealBean>, I_UpdateRealView<Ammonia2RealBean> {


    private UltimateRecyclerView waterPressureRealRecycler;

    List<Ammonia2RealBean> waterPressureRealBeans = new ArrayList<>();

    //-------------------------------------------------
    private BaseDeviceDetailManageAdapter<Device_Detail_AddBean, Ammonia2RealBean> manageAdapter;
...
//    public Temperature_Add deviceDetail_AddAdapter;
    ...
    String deleteUrl = "";
    String changeUrl = "";
    NewHistoryAdapter<Ammonia2HistoryBean> newHistoryAdapter;
    List<Ammonia2HistoryBean> lineDataList = new ArrayList<>();
    //设备管理的数据
    ...
    //设备实时数据工具类
    public RealDataUtil<Ammonia2RealBean> realDataUtil;
     ...
     */
    public Ammonia2Fragment(String realLink, String alertLink, String historyLink, String manageLink) {
        super(realLink, alertLink, historyLink, manageLink);
    }

    public Ammonia2Fragment(String realLink, String alertLink, String historyLink, String manageLink, String deleteUrl, String changeUrl) {
        super(realLink, alertLink, historyLink, manageLink);
...
//        changeUrl = LainNewApi.tempChange;
        ...
        //查询实时数据
        device8060Present.queryRealData(Ammonia2RealBean.class);
//        startRealUpdate();
        //设置历史数据列表
    ...
    public void queryHistoryData() {
        try {
            ...
            //默认查询第一个设备
            Ammonia2RealBean temperatureBean = waterPressureRealBeans.get(0);
            ///获取设备的ID

            int temEhmId = temperatureBean.getAmmoniaId();
            //结束时间
            String endTime = DateUtil.getInstance().getCurrentDate();
            //开始时间
            String startTime = DateUtil.getInstance().getStartTime();
            //开始查询
            device8060Present.queryHistoryData(Ammonia2HistoryBean.class, temEhmId, startTime, endTime);
    ...
    protected void querySelectDeviceHistory(int ehmID, String startTime, String endTime) {
        device8060Present.queryHistoryData(Ammonia2HistoryBean.class, ehmID, startTime, endTime);
    }
    ...
    public void requestReal() {
        ...
        try {
            ...
            if (alertDeviceBeanList.isEmpty()) {
                //每次查询都保存第一个设备ID
                ehmIDFirst = waterPressureRealBeans.get(0).getAmmoniaId();
                //更新报警查询的设备
                for (Ammonia2RealBean b :
                        waterPressureRealBeans) {
                    AlertDeviceBean alertB = new AlertDeviceBean();

                    alertB.setName(b.getAmmoniaName());
    ...
    public void setCurrentData(int position) {
        try {
            //点击切换上面的圆形进度条的数据
            //设置相关的属性值
            Ammonia2RealBean b = waterPressureRealBeans.get(position);

            if (b.getAmmoniaData() == currentTempValue)
                return;
    ...
    public void requestAlert() {
        ...
        List<Ammonia2AlertBean> air = getAlertData();
        alertBeans.clear();

        for (int i = 0; i < air.size(); i++) {
            Ammonia2AlertBean alertBean = air.get(i);
            AlertBeans alertBeansItem = new AlertBeans();
            alertBeansItem.setEhaInfo(alertBean.getInfo());
            alertBeansItem.setEhaTime(alertBean.getAmmoniaAlarmTime());
            alertBeansItem.setEcmDeviceName(alertBean.getAmmoniaName());
...
//        先清空原有的设备数据
        ...
        List<Ammonia2RealBean> manageData = getManageData();
...
//        适配添加设备的数据
        for (Ammonia2RealBean b :
            ...
            bean.setDeviceName(b.getAmmoniaName());

            bean.setEhmId(String.valueOf(b.getAmmoniaId()));
            //湿度
            bean.setAlertValue(b.getAmmoniaMinData() + " ~ " + b.getAmmoniaMaxData());

            //位置
            bean.setPosition(String.valueOf(b.getAmmoniaAddress()));
            //更新时间
...
//        设置 设备管理 中的 添加设备 列表
    ...
    public void queryDeviceManage() {
        //只执行一次
        if (addBeans.size() == 0)
            device8060Present.queryDeviceManage(Ammonia2RealBean.class);
    }

    @Override
    public void querySelectDeviceAlert(int ehmID, String startTime, String endTime) {
        device8060Present.queryAlertData(Ammonia2AlertBean.class, ehmID, startTime, endTime);
    }
    ...
    public void onItemClickListener(View v, int position) {
        ...
        try {
            ...
            if (v.findViewById(R.id.btn_panel_device) != null) {
                //先保存数据，点击确定后在进行查询
                setEhmID(waterPressureRealBeans.get(position).getAmmoniaId());
                return;
            }
            //历史数据点击查看 曲线图
            if (v.findViewById(com.example.pool.R.id.new_history_name) != null) {
                ...
                for (Ammonia2HistoryBean temp :
                        lineDataList) {
                    temps.add(temp.getAmmoniaData() + "");
                    hums.add(temp.getAmmoniaAlarmTime() + "");
                }
                ...
                intent.putExtra("markUnit", waterPressureRealBeans.get(position).getAmmoniaName());
                ...
                intent.putExtra("title", waterPressureRealBeans.get(position).getAmmoniaName());
            ...
            realDataUtil.setOuterMax(String.valueOf(b.getAmmoniaMaxData()));
            realDataUtil.setOuterData(String.valueOf(b.getAmmoniaMaxData()));
            //设置文案
            realDataUtil.setInnerTitle("氨气 PPM");
     ...
     */
    @Override
    public Class<Ammonia2RealBean> getParseClass() {
        return Ammonia2RealBean.class;
    }

    @Override
    public void bindHistoryData(BaseViewHolder holder, Ammonia2HistoryBean data, int position) {
        ...
        historyTemp.setText("水流量" + data.getAmmoniaData());
        historyTime.setText("时间" + data.getAmmoniaAlarmTime());
    ...
    public void deleteDevice(int position, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        String ehmId = String.valueOf(waterPressureRealBeans.get(position).getAmmoniaId());
     ...
     */
    @Override
    public void realItemAdapter(BaseViewHolder holder, Ammonia2RealBean data, int position) {

        TextView item_name = holder.getView(R.id.dissolved_item_name);

        item_name.setText(data.getAmmoniaName());
    ...
    public void changeDevice(String flag, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        ...
        Ammonia2RealBean temp = waterPressureRealBeans.get(position);
        ...
        deviceManageEditBean.setaDeviceName(temp.getAmmoniaName());
        deviceManageEditBean.setActionID(temp.getAmmoniaId());


        deviceManageEditBean.setaDeviceInterval(temp.getIntervalTime());
//
        deviceManageEditBean.setRangeMin(temp.getAmmoniaMinData());
        deviceManageEditBean.setRangeMax(temp.getAmmoniaMaxData());
    ...
    public void deterMine(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("ammoniaName", getTextString(keyArray.get("deviceName")));
        formBody.put("ammoniaAddress", getSpinnerString(spinnerMap.get("deviceAddress")));
        formBody.put("ammoniaMaxData", getTextString(keyArray.get("rangeMax")));
        formBody.put("ammoniaMinData", getTextString(keyArray.get("rangeMin")));
        ...
        OkHttpUtil.getInstance().sendRowOkHttp("add", LainNewApi.getInstance().getRootPath() + LainNewApi.ammonia2Insert, MyGson.getInstance().toJson(formBody), this);
    ...
    public void deterChange(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("ammoniaMinData", getTextString(keyArray.get("rangeMin")));
        formBody.put("intervalTime", getTextString(keyArray.get("interval")));
        formBody.put("ammoniaId", getTextString(keyArray.get("actionID")));
    ...
    @Override
    public void updateRealView(TextView textView, Ammonia2RealBean data, int position) {
        //设置列表的数据
        textView.setText(data.getAmmoniaName());
    }
}
