...
import bean.TotalphosphorusAlertBean;
import bean.TotalphosphorusHistoryBean;
import bean.TotalphosphorusRealBean;
import http.MyGson;
...
/**
 * @ClassName: TotalphosphorusFragment
 * @Description:
 * @Author: YIN LUO FEI
 * @Date: 2020/3/27 23:35
 */
public class TotalphosphorusFragment extends Device8060Fragment<TotalphosphorusRealBean, TotalphosphorusAlertBean,
        TotalphosphorusHistoryBean, TotalphosphorusRealBean> implements BaseRecyclerViewAdapter.OnItemClickListener, I_DeviceDetailManageLink<Device_Detail_AddBean>, I_HistoryAdapter<TotalphosphorusHistoryBean>, I_RealAdapterListen<TotalphosphorusRealBean> , I_UpdateRealView<TotalphosphorusRealBean> {

    private UltimateRecyclerView waterPressureRealRecycler;

    List<TotalphosphorusRealBean> waterPressureRealBeans = new ArrayList<>();

    //设备实时数据工具类
    public RealDataUtil<TotalphosphorusRealBean> realDataUtil;

    //-------------------------------------------------
    private BaseDeviceDetailManageAdapter<Device_Detail_AddBean, TotalphosphorusRealBean> manageAdapter;
...
//    public Temperature_Add deviceDetail_AddAdapter;
    ...
    String deleteUrl = "";
    String changeUrl = "";
    NewHistoryAdapter<TotalphosphorusHistoryBean> newHistoryAdapter;
    List<TotalphosphorusHistoryBean> lineDataList = new ArrayList<>();
    //设备管理的数据
     ...
     */
    public TotalphosphorusFragment(String realLink, String alertLink, String historyLink, String manageLink) {
        super(realLink, alertLink, historyLink, manageLink);
    }

    public TotalphosphorusFragment(String realLink, String alertLink, String historyLink, String manageLink, String deleteUrl, String changeUrl) {
        super(realLink, alertLink, historyLink, manageLink);
...
//        changeUrl = LainNewApi.tempChange;
        ...
        //查询实时数据
        device8060Present.queryRealData(TotalphosphorusRealBean.class);
//        startRealUpdate();
        //设置历史数据列表
    ...
    public void queryHistoryData() {
        try {
            ...
            //默认查询第一个设备
            TotalphosphorusRealBean temperatureBean = waterPressureRealBeans.get(0);
            ///获取设备的ID

            int temEhmId = temperatureBean.getPhosphorusId();
            //结束时间
            String endTime = DateUtil.getInstance().getCurrentDate();
            //开始时间
            String startTime = DateUtil.getInstance().getStartTime();
            //开始查询
            device8060Present.queryHistoryData(TotalphosphorusHistoryBean.class, temEhmId, startTime, endTime);
    ...
    protected void querySelectDeviceHistory(int ehmID, String startTime, String endTime) {
        device8060Present.queryHistoryData(TotalphosphorusHistoryBean.class, ehmID, startTime, endTime);
    }
    ...
    public void requestReal() {
        ...
        try {
            ...
            if (alertDeviceBeanList.isEmpty()) {
                //每次查询都保存第一个设备ID
                ehmIDFirst = waterPressureRealBeans.get(0).getPhosphorusId();
                //更新报警查询的设备
                for (TotalphosphorusRealBean b :
                        waterPressureRealBeans) {
                    AlertDeviceBean alertB = new AlertDeviceBean();

                    alertB.setName(b.getPhosphorusName());
    ...
    public void setCurrentData(int position) {
        try {
            //点击切换上面的圆形进度条的数据
            //设置相关的属性值
            TotalphosphorusRealBean b = waterPressureRealBeans.get(position);

            if (b.getPhosphorusData() == currentTempValue)
                return;
    ...
    public void requestAlert() {
        ...
        List<TotalphosphorusAlertBean> air = getAlertData();
        alertBeans.clear();

        for (int i = 0; i < air.size(); i++) {
            TotalphosphorusAlertBean alertBean = air.get(i);
            AlertBeans alertBeansItem = new AlertBeans();
            alertBeansItem.setEhaInfo(alertBean.getInfo());
            alertBeansItem.setEhaTime(alertBean.getPhosphorusAlarmTime());
            alertBeansItem.setEcmDeviceName(alertBean.getPhosphorusName());
...
//        先清空原有的设备数据
        ...
        List<TotalphosphorusRealBean> manageData = getManageData();
...
//        适配添加设备的数据
        for (TotalphosphorusRealBean b :
            ...
            bean.setDeviceName(b.getPhosphorusName());

            bean.setEhmId(String.valueOf(b.getPhosphorusId()));
            //湿度
            bean.setAlertValue(b.getPhosphorusMinData() + " ~ " + b.getPhosphorusMaxData());

            //位置
            bean.setPosition(String.valueOf(b.getPhosphorusAddress()));
            //更新时间
...
//        设置 设备管理 中的 添加设备 列表
    ...
    public void queryDeviceManage() {
        //只执行一次
        if (addBeans.size() == 0)
            device8060Present.queryDeviceManage(TotalphosphorusRealBean.class);
    }

    @Override
    public void querySelectDeviceAlert(int ehmID, String startTime, String endTime) {
        device8060Present.queryAlertData(TotalphosphorusAlertBean.class, ehmID, startTime, endTime);
    }
    ...
    public void onItemClickListener(View v, int position) {
        ...
        try {
            ...
            if (v.findViewById(R.id.btn_panel_device) != null) {
                //先保存数据，点击确定后在进行查询
                setEhmID(waterPressureRealBeans.get(position).getPhosphorusId());
                return;
            }
            //历史数据点击查看 曲线图
            if (v.findViewById(com.example.pool.R.id.new_history_name) != null) {
                ...
                for (TotalphosphorusHistoryBean temp :
                        lineDataList) {
                    temps.add(temp.getPhosphorusData() + "");
                    hums.add(temp.getPhosphorusHistoryTime() + "");
                }
                ...
                intent.putExtra("markUnit", waterPressureRealBeans.get(position).getPhosphorusName());
                ...
                intent.putExtra("title", waterPressureRealBeans.get(position).getPhosphorusName());
            ...
            TotalphosphorusRealBean b = waterPressureRealBeans.get(position);
//设置数据
            realDataUtil.setInnerMax(String.valueOf(b.getPhosphorusMaxData()));
            realDataUtil.setInnerData(String.valueOf(b.getPhosphorusData()));
            //设置报警值，必须先设置最大值，再设置当前值，才不会出现问题
            realDataUtil.setOuterMax(String.valueOf(b.getPhosphorusMaxData()));
            realDataUtil.setOuterData(String.valueOf(b.getPhosphorusMaxData()));
            //设置文案
     ...
     */
    @Override
    public Class<TotalphosphorusRealBean> getParseClass() {
        return TotalphosphorusRealBean.class;
    }

    @Override
    public void bindHistoryData(BaseViewHolder holder, TotalphosphorusHistoryBean data, int position) {
        ...
        historyTemp.setText("水流量" + data.getPhosphorusData());
        historyTime.setText("时间" + data.getPhosphorusHistoryTime());
    ...
    public void deleteDevice(int position, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        String ehmId = String.valueOf(waterPressureRealBeans.get(position).getPhosphorusId());
     ...
     */
    @Override
    public void realItemAdapter(BaseViewHolder holder, TotalphosphorusRealBean data, int position) {

        TextView item_name = holder.getView(R.id.dissolved_item_name);
        item_name.setText(data.getPhosphorusName());
    ...
    public void changeDevice(String flag, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        ...
        TotalphosphorusRealBean temp = waterPressureRealBeans.get(position);
        ...
        deviceManageEditBean.setaDeviceName(temp.getPhosphorusName());
        deviceManageEditBean.setActionID(temp.getPhosphorusId());


        deviceManageEditBean.setaDeviceInterval(temp.getIntervalTime());
//
        deviceManageEditBean.setRangeMin(temp.getPhosphorusMinData());
        deviceManageEditBean.setRangeMax(temp.getPhosphorusMaxData());
    ...
    public void deterMine(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("phosphorusName", getTextString(keyArray.get("deviceName")));
        formBody.put("phosphorusAddress", getSpinnerString(spinnerMap.get("deviceAddress")));
        formBody.put("phosphorusMaxData", getTextString(keyArray.get("rangeMax")));
        formBody.put("phosphorusMinData", getTextString(keyArray.get("rangeMin")));
        ...
        OkHttpUtil.getInstance().sendRowOkHttp("add", LainNewApi.getInstance().getRootPath() + LainNewApi.phosphorusInsert, MyGson.getInstance().toJson(formBody), this);
    ...
    public void deterChange(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("phosphorusMinData", getTextString(keyArray.get("rangeMin")));
        formBody.put("intervalTime", getTextString(keyArray.get("interval")));
        formBody.put("phosphorusId", getTextString(keyArray.get("actionID")));
    ...
    @Override
    public void updateRealView(TextView textView, TotalphosphorusRealBean data, int position) {
        //设置列表的数据
        textView.setText(data.getPhosphorusName());
    }
}
