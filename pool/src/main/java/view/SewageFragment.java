...
import bean.SewageAlertBean;
import bean.SewageHistoryBean;
import bean.SewageRealBean;
import http.MyGson;
...
/**
 * @ClassName: SewageFragment
 * @Description: 污水流量
 * @Author: YIN LUO FEI
 * @Date: 2020/3/27 22:25
 */
public class SewageFragment extends Device8060Fragment<SewageRealBean, SewageAlertBean,
        SewageHistoryBean, SewageRealBean> implements BaseRecyclerViewAdapter.OnItemClickListener, I_DeviceDetailManageLink<Device_Detail_AddBean>, I_HistoryAdapter<SewageHistoryBean>, I_RealAdapterListen<SewageRealBean>, I_UpdateRealView<SewageRealBean> {

    private UltimateRecyclerView waterPressureRealRecycler;

    List<SewageRealBean> waterPressureRealBeans = new ArrayList<>();

    //-------------------------------------------------
    private BaseDeviceDetailManageAdapter<Device_Detail_AddBean, SewageRealBean> manageAdapter;
...
//    public Temperature_Add deviceDetail_AddAdapter;
    ...
    NewHistoryAdapter<SewageHistoryBean> newHistoryAdapter;
    List<SewageHistoryBean> lineDataList = new ArrayList<>();
    //设备管理的数据
    ...
    //设备实时数据工具类
    public RealDataUtil<SewageRealBean> realDataUtil;
     ...
     */
    public SewageFragment(String realLink, String alertLink, String historyLink, String manageLink) {
        super(realLink, alertLink, historyLink, manageLink);
    }

    public SewageFragment(String realLink, String alertLink, String historyLink, String manageLink, String deleteUrl, String changeUrl) {
        super(realLink, alertLink, historyLink, manageLink);
...
//        changeUrl = LainNewApi.tempChange;
        ...
        //查询实时数据
        device8060Present.queryRealData(SewageRealBean.class);
//        startRealUpdate();
        //设置历史数据列表
    ...
    public void queryHistoryData() {
        try {
            ...
            //默认查询第一个设备
            SewageRealBean temperatureBean = waterPressureRealBeans.get(0);
            ///获取设备的ID

            int temEhmId = temperatureBean.getSewageDischargeId();
            //结束时间
            String endTime = DateUtil.getInstance().getCurrentDate();
            //开始时间
            String startTime = DateUtil.getInstance().getStartTime();
            //开始查询
            device8060Present.queryHistoryData(SewageHistoryBean.class, temEhmId, startTime, endTime);
    ...
    protected void querySelectDeviceHistory(int ehmID, String startTime, String endTime) {
        device8060Present.queryHistoryData(SewageHistoryBean.class, ehmID, startTime, endTime);
    }
    ...
    public void requestReal() {
        ...
        try {
            ...
            if (alertDeviceBeanList.isEmpty()) {
                //每次查询都保存第一个设备ID
                ehmIDFirst = waterPressureRealBeans.get(0).getSewageDischargeId();
                //更新报警查询的设备
                for (SewageRealBean b :
                        waterPressureRealBeans) {
                    AlertDeviceBean alertB = new AlertDeviceBean();
                    alertB.setName(b.getSewageDischargeName());
    ...
    public void setCurrentData(int position) {
        try {
            //点击切换上面的圆形进度条的数据
            //设置相关的属性值
            SewageRealBean b = waterPressureRealBeans.get(position);

            if (b.getSewageDischargeData() == currentTempValue)
                return;
    ...
    public void requestAlert() {
        ...
        List<SewageAlertBean> air = getAlertData();
        alertBeans.clear();

        for (int i = 0; i < air.size(); i++) {
            SewageAlertBean alertBean = air.get(i);
            AlertBeans alertBeansItem = new AlertBeans();
            alertBeansItem.setEhaInfo(alertBean.getInfo());
            alertBeansItem.setEhaTime(alertBean.getSewageDischargeAlarmTime());
            alertBeansItem.setEcmDeviceName(alertBean.getSewageDischargeName());
...
//        先清空原有的设备数据
        ...
        List<SewageRealBean> manageData = getManageData();
...
//        适配添加设备的数据
        for (SewageRealBean b :
            ...
            bean.setDeviceName(b.getSewageDischargeName());

            bean.setEhmId(String.valueOf(b.getSewageDischargeId()));
            //湿度
            bean.setAlertValue(b.getSewageDischargeMinData() + " ~ " + b.getSewageDischargeMaxData());

            //位置
            bean.setPosition(String.valueOf(b.getSewageDischargeAddress()));
            //更新时间
...
//        设置 设备管理 中的 添加设备 列表
    ...
    public void queryDeviceManage() {
        //只执行一次
        if (addBeans.size() == 0)
            device8060Present.queryDeviceManage(SewageRealBean.class);
    }

    @Override
    public void querySelectDeviceAlert(int ehmID, String startTime, String endTime) {
        device8060Present.queryAlertData(SewageAlertBean.class, ehmID, startTime, endTime);
    }
    ...
    public void onItemClickListener(View v, int position) {
        ...
        try {
            ...
            if (v.findViewById(R.id.btn_panel_device) != null) {
                //先保存数据，点击确定后在进行查询
                setEhmID(waterPressureRealBeans.get(position).getSewageDischargeId());
                return;
            }
            //历史数据点击查看 曲线图
            if (v.findViewById(com.example.pool.R.id.new_history_name) != null) {
                ...
                for (SewageHistoryBean temp :
                        lineDataList) {
                    temps.add(temp.getSewageDischargeData() + "");
                    hums.add(temp.getSewageDischargeHistoryTime() + "");
                }
                ...
                intent.putExtra("markUnit", waterPressureRealBeans.get(position).getSewageDischargeName());
                ...
                intent.putExtra("title", waterPressureRealBeans.get(position).getSewageDischargeName());
            ...
            SewageRealBean b = waterPressureRealBeans.get(position);
            //设置数据
            realDataUtil.setInnerMax(String.valueOf(b.getSewageDischargeMaxData()));
            realDataUtil.setInnerData(String.valueOf(b.getSewageDischargeData()));
            //设置报警值，必须先设置最大值，再设置当前值，才不会出现问题
            realDataUtil.setOuterMax(String.valueOf(b.getSewageDischargeMaxData()));
            realDataUtil.setOuterData(String.valueOf(b.getSewageDischargeMaxData()));
            //设置文案
     ...
     */
    @Override
    public Class<SewageRealBean> getParseClass() {
        return SewageRealBean.class;
    }

    @Override
    public void bindHistoryData(BaseViewHolder holder, SewageHistoryBean data, int position) {
        ...
        historyTemp.setText("水流量" + data.getSewageDischargeData());
        historyTime.setText("时间" + data.getSewageDischargeHistoryTime());
    ...
    public void deleteDevice(int position, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        String ehmId = String.valueOf(waterPressureRealBeans.get(position).getSewageDischargeId());
     ...
     */
    @Override
    public void realItemAdapter(BaseViewHolder holder, SewageRealBean data, int position) {

        TextView item_name = holder.getView(R.id.dissolved_item_name);
        item_name.setText(data.getSewageDischargeName());
    ...
    public void changeDevice(String flag, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        ...
        SewageRealBean temp = waterPressureRealBeans.get(position);
        ...
        deviceManageEditBean.setaDeviceName(temp.getSewageDischargeName());
        deviceManageEditBean.setActionID(temp.getSewageDischargeId());


        deviceManageEditBean.setaDeviceInterval(temp.getIntervalTime());
//
        deviceManageEditBean.setRangeMin(temp.getSewageDischargeMinData());
        deviceManageEditBean.setRangeMax(temp.getSewageDischargeMaxData());
    ...
    public void deterMine(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("sewageDischargeName", getTextString(keyArray.get("deviceName")));
        formBody.put("sewageDischargeAddress", getSpinnerString(spinnerMap.get("deviceAddress")));
        formBody.put("sewageDischargeMaxData", getTextString(keyArray.get("rangeMax")));
        formBody.put("sewageDischargeMinData", getTextString(keyArray.get("rangeMin")));
        ...
        OkHttpUtil.getInstance().sendRowOkHttp("add", LainNewApi.getInstance().getRootPath() + LainNewApi.sewageInsert, MyGson.getInstance().toJson(formBody), this);
    ...
    public void deterChange(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("sewageDischargeMinData", getTextString(keyArray.get("rangeMin")));
        formBody.put("intervalTime", getTextString(keyArray.get("interval")));
        formBody.put("sewageDischargeId", getTextString(keyArray.get("actionID")));
    ...
    @Override
    public void updateRealView(TextView textView, SewageRealBean data, int position) {
        //设置列表的数据
        textView.setText(data.getSewageDischargeName());
    }
}
