...
import bean.UreaAlertBean;
import bean.UreaHistoryBean;
import bean.UreaRealBean;
...
/**
 * @ClassName: UreaFragment
 * @Description:
 * @Author: YIN LUO FEI
 * @Date: 2020/3/28 1:37
 */
public class UreaFragment extends Device8060Fragment<UreaRealBean, UreaAlertBean,
        UreaHistoryBean, UreaRealBean> implements BaseRecyclerViewAdapter.OnItemClickListener, I_DeviceDetailManageLink<Device_Detail_AddBean>, I_HistoryAdapter<UreaHistoryBean>, I_RealAdapterListen<UreaRealBean>, I_UpdateRealView<UreaRealBean> {

    private UltimateRecyclerView waterPressureRealRecycler;

    List<UreaRealBean> waterPressureRealBeans = new ArrayList<>();


    //-------------------------------------------------
    private BaseDeviceDetailManageAdapter<Device_Detail_AddBean, UreaRealBean> manageAdapter;
...
//    public Temperature_Add deviceDetail_AddAdapter;
    ...
    String deleteUrl = "";
    String changeUrl = "";
    NewHistoryAdapter<UreaHistoryBean> newHistoryAdapter;
    List<UreaHistoryBean> lineDataList = new ArrayList<>();
    //设备管理的数据
    ...
    //设备实时数据工具类
    public RealDataUtil<UreaRealBean> realDataUtil;
     ...
     */
    public UreaFragment(String realLink, String alertLink, String historyLink, String manageLink) {
        super(realLink, alertLink, historyLink, manageLink);
    }

    public UreaFragment(String realLink, String alertLink, String historyLink, String manageLink, String deleteUrl, String changeUrl) {
        super(realLink, alertLink, historyLink, manageLink);
...
//        changeUrl = LainNewApi.tempChange;
        ...
        //查询实时数据
        device8060Present.queryRealData(UreaRealBean.class);
//        startRealUpdate();
        //设置历史数据列表
    ...
    public void queryHistoryData() {
        try {
            ...
            //默认查询第一个设备
            UreaRealBean temperatureBean = waterPressureRealBeans.get(0);
            ///获取设备的ID

            int temEhmId = temperatureBean.getUreaId();
            //结束时间
            String endTime = DateUtil.getInstance().getCurrentDate();
            //开始时间
            String startTime = DateUtil.getInstance().getStartTime();
            //开始查询
            device8060Present.queryHistoryData(UreaHistoryBean.class, temEhmId, startTime, endTime);
    ...
    protected void querySelectDeviceHistory(int ehmID, String startTime, String endTime) {
        device8060Present.queryHistoryData(UreaHistoryBean.class, ehmID, startTime, endTime);
    }
    ...
    public void requestReal() {
        ...
        try {
            ...
            if (alertDeviceBeanList.isEmpty()) {
                //每次查询都保存第一个设备ID
                ehmIDFirst = waterPressureRealBeans.get(0).getUreaId();
                //更新报警查询的设备
                for (UreaRealBean b :
                        waterPressureRealBeans) {
                    AlertDeviceBean alertB = new AlertDeviceBean();

                    alertB.setName(b.getUreaName());
    ...
    public void setCurrentData(int position) {
        try {
            //点击切换上面的圆形进度条的数据
            //设置相关的属性值
            UreaRealBean b = waterPressureRealBeans.get(position);

            if (b.getUreaData() == currentTempValue)
                return;
    ...
    public void requestAlert() {
        ...
        List<UreaAlertBean> air = getAlertData();
        alertBeans.clear();

        for (int i = 0; i < air.size(); i++) {
            UreaAlertBean alertBean = air.get(i);
            AlertBeans alertBeansItem = new AlertBeans();
            alertBeansItem.setEhaInfo(alertBean.getInfo());
            alertBeansItem.setEhaTime(alertBean.getUreaAlarmTime());
            alertBeansItem.setEcmDeviceName(alertBean.getUreaName());
...
//        先清空原有的设备数据
        ...
        List<UreaRealBean> manageData = getManageData();
...
//        适配添加设备的数据
        for (UreaRealBean b :
            ...
            bean.setDeviceName(b.getUreaName());

            bean.setEhmId(String.valueOf(b.getUreaId()));
            //湿度
            bean.setAlertValue(b.getUreaMinData() + " ~ " + b.getUreaMaxData());

            //位置
            bean.setPosition(String.valueOf(b.getUreaAddress()));
            //更新时间
...
//        设置 设备管理 中的 添加设备 列表
    ...
    public void queryDeviceManage() {
        //只执行一次
        if (addBeans.size() == 0)
            device8060Present.queryDeviceManage(UreaRealBean.class);
    }

    @Override
    public void querySelectDeviceAlert(int ehmID, String startTime, String endTime) {
        device8060Present.queryAlertData(UreaAlertBean.class, ehmID, startTime, endTime);
    }
    ...
    public void onItemClickListener(View v, int position) {
        ...
        try {
            ...
            if (v.findViewById(R.id.btn_panel_device) != null) {
                //先保存数据，点击确定后在进行查询
                setEhmID(waterPressureRealBeans.get(position).getUreaId());
                return;
            }
            //历史数据点击查看 曲线图
            if (v.findViewById(com.example.pool.R.id.new_history_name) != null) {
                ...
                for (UreaHistoryBean temp :
                        lineDataList) {
                    temps.add(temp.getUreaData() + "");
                    hums.add(temp.getUreaHistoryTime() + "");
                }
                ...
                intent.putExtra("markUnit", waterPressureRealBeans.get(position).getUreaName());
                ...
                intent.putExtra("title", waterPressureRealBeans.get(position).getUreaName());
            ...
            //设置相关的属性值
            UreaRealBean b = waterPressureRealBeans.get(position);
            realDataUtil.setInnerMax(String.valueOf(b.getUreaMaxData()));
            realDataUtil.setInnerData(String.valueOf(b.getUreaData()));
            //设置报警值，必须先设置最大值，再设置当前值，才不会出现问题
            realDataUtil.setOuterMax(String.valueOf(b.getUreaMaxData()));
            realDataUtil.setOuterData(String.valueOf(b.getUreaMaxData()));
            //设置文案
     ...
     */
    @Override
    public Class<UreaRealBean> getParseClass() {
        return UreaRealBean.class;
    }

    @Override
    public void bindHistoryData(BaseViewHolder holder, UreaHistoryBean data, int position) {
        ...
        historyTemp.setText("水流量" + data.getUreaData());
        historyTime.setText("时间" + data.getUreaHistoryTime());
    ...
    public void deleteDevice(int position, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        String ehmId = String.valueOf(waterPressureRealBeans.get(position).getUreaId());
     ...
     */
    @Override
    public void realItemAdapter(BaseViewHolder holder, UreaRealBean data, int position) {

        TextView item_name = holder.getView(R.id.dissolved_item_name);
        item_name.setText(data.getUreaName());
    ...
    public void changeDevice(String flag, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        ...
        UreaRealBean temp = waterPressureRealBeans.get(position);
        ...
        deviceManageEditBean.setaDeviceName(temp.getUreaName());
        deviceManageEditBean.setActionID(temp.getUreaId());


        deviceManageEditBean.setaDeviceInterval(temp.getIntervalTime());
//
        deviceManageEditBean.setRangeMin(temp.getUreaMinData());
        deviceManageEditBean.setRangeMax(temp.getUreaMaxData());
    ...
    public void deterMine(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("ureaName", getTextString(keyArray.get("deviceName")));
        formBody.put("ureaAddress", getSpinnerString(spinnerMap.get("deviceAddress")));
        formBody.put("ureaMaxData", getTextString(keyArray.get("rangeMax")));
        formBody.put("ureaMinData", getTextString(keyArray.get("rangeMin")));
        ...
        OkHttpUtil.getInstance().sendRowOkHttp("add", LainNewApi.getInstance().getRootPath() + LainNewApi.ureaIP, MyGson.getInstance().toJson(formBody), this);
    ...
    public void deterChange(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("ureaMinData", getTextString(keyArray.get("rangeMin")));
        formBody.put("intervalTime", getTextString(keyArray.get("interval")));
        formBody.put("ureaId", getTextString(keyArray.get("actionID")));
    ...
    @Override
    public void updateRealView(TextView textView, UreaRealBean data, int position) {
        //设置列表的数据
        textView.setText(data.getUreaName());
    }
}
