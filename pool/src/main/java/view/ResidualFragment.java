...
import bean.ResidualRealBean;
import bean.Device_Detail_AddBean;
import bean.ResidualAlertBean;
import bean.ResidualHistoryBean;
import http.MyGson;
...
/**
 * @ClassName: ResidualFragment
 * @Description: 余氯
 * @Author: YIN LUO FEI
 * @Date: 2020/3/27 20:12
 */

public class ResidualFragment extends Device8060Fragment<ResidualRealBean, ResidualAlertBean,
        ResidualHistoryBean, ResidualRealBean> implements BaseRecyclerViewAdapter.OnItemClickListener, I_DeviceDetailManageLink<Device_Detail_AddBean>, I_HistoryAdapter<ResidualHistoryBean>, I_RealAdapterListen<ResidualRealBean> , I_UpdateRealView<ResidualRealBean> {

    private UltimateRecyclerView waterPressureRealRecycler;

    List<ResidualRealBean> waterPressureRealBeans = new ArrayList<>();

    //-------------------------------------------------
    private BaseDeviceDetailManageAdapter<Device_Detail_AddBean, ResidualRealBean> manageAdapter;
...
//    public Temperature_Add deviceDetail_AddAdapter;
    ...
    String deleteUrl = "";
    String changeUrl = "";
    NewHistoryAdapter<ResidualHistoryBean> newHistoryAdapter;
    List<ResidualHistoryBean> lineDataList = new ArrayList<>();
    //设备管理的数据
    ...
    //设备实时数据工具类
    public RealDataUtil<ResidualRealBean> realDataUtil;
     ...
     */
    public ResidualFragment(String realLink, String alertLink, String historyLink, String manageLink) {
        super(realLink, alertLink, historyLink, manageLink);
    }

    public ResidualFragment(String realLink, String alertLink, String historyLink, String manageLink, String deleteUrl, String changeUrl) {
        super(realLink, alertLink, historyLink, manageLink);
...
//        changeUrl = LainNewApi.tempChange;
        ...
        //查询实时数据
        device8060Present.queryRealData(ResidualRealBean.class);
//        startRealUpdate();
        //设置历史数据列表
    ...
    public void queryHistoryData() {
        try {
            ...
            //默认查询第一个设备
            ResidualRealBean temperatureBean = waterPressureRealBeans.get(0);
            ///获取设备的ID

            int temEhmId = temperatureBean.getResidualChlorineId();
            //结束时间
            String endTime = DateUtil.getInstance().getCurrentDate();
            //开始时间
            String startTime = DateUtil.getInstance().getStartTime();
            //开始查询
            device8060Present.queryHistoryData(ResidualHistoryBean.class, temEhmId, startTime, endTime);
    ...
    protected void querySelectDeviceHistory(int ehmID, String startTime, String endTime) {
        device8060Present.queryHistoryData(ResidualHistoryBean.class, ehmID, startTime, endTime);
    }
    ...
    public void requestReal() {
        ...
        try {
            ...
            if (alertDeviceBeanList.isEmpty()) {
                //每次查询都保存第一个设备ID
                ehmIDFirst = waterPressureRealBeans.get(0).getResidualChlorineId();
                //更新报警查询的设备
                for (ResidualRealBean b :
                        waterPressureRealBeans) {
                    AlertDeviceBean alertB = new AlertDeviceBean();
                    alertB.setName(b.getResidualChlorineName());
    ...
    public void setCurrentData(int position) {
        try {
            //点击切换上面的圆形进度条的数据
            //设置相关的属性值
            ResidualRealBean b = waterPressureRealBeans.get(position);

            if (b.getResidualChlorineData() == currentTempValue)
                return;
    ...
    public void requestAlert() {
        ...
        List<ResidualAlertBean> air = getAlertData();
        alertBeans.clear();

        for (int i = 0; i < air.size(); i++) {
            ResidualAlertBean alertBean = air.get(i);
            AlertBeans alertBeansItem = new AlertBeans();
            alertBeansItem.setEhaInfo(alertBean.getInfo());
            alertBeansItem.setEhaTime(alertBean.getResidualChlorineAlarmTime());
            alertBeansItem.setEcmDeviceName(alertBean.getResidualChlorineName());
...
//        先清空原有的设备数据
        ...
        List<ResidualRealBean> manageData = getManageData();
...
//        适配添加设备的数据
        for (ResidualRealBean b :
            ...
            bean.setDeviceName(b.getResidualChlorineName());

            bean.setEhmId(String.valueOf(b.getResidualChlorineId()));
            //湿度
            bean.setAlertValue(b.getResidualChlorineMinData() + " ~ " + b.getResidualChlorineMaxData());

            //位置
            bean.setPosition(String.valueOf(b.getResidualChlorineAddress()));
            //更新时间
...
//        设置 设备管理 中的 添加设备 列表
    ...
    public void queryDeviceManage() {
        //只执行一次
        if (addBeans.size() == 0)
            device8060Present.queryDeviceManage(ResidualRealBean.class);
    }

    @Override
    public void querySelectDeviceAlert(int ehmID, String startTime, String endTime) {
        device8060Present.queryAlertData(ResidualAlertBean.class, ehmID, startTime, endTime);
    }
    ...
    public void onItemClickListener(View v, int position) {
        ...
        try {
            ...
            if (v.findViewById(R.id.btn_panel_device) != null) {
                //先保存数据，点击确定后在进行查询
                setEhmID(waterPressureRealBeans.get(position).getResidualChlorineId());
                return;
            }
            //历史数据点击查看 曲线图
            if (v.findViewById(com.example.pool.R.id.new_history_name) != null) {
                ...
                for (ResidualHistoryBean temp :
                        lineDataList) {
                    temps.add(temp.getResidualChlorineData() + "");
                    hums.add(temp.getResidualChlorineHistoryTime() + "");
                }
                ...
                intent.putExtra("markUnit", waterPressureRealBeans.get(position).getResidualChlorineName());
                ...
                intent.putExtra("title", waterPressureRealBeans.get(position).getResidualChlorineName());
            ...
            ResidualRealBean b = waterPressureRealBeans.get(position);
            //设置数据
            realDataUtil.setInnerMax(String.valueOf(b.getResidualChlorineMaxData()));
            realDataUtil.setInnerData(String.valueOf(b.getResidualChlorineData()));
            //设置报警值，必须先设置最大值，再设置当前值，才不会出现问题
            realDataUtil.setOuterMax(String.valueOf(b.getResidualChlorineMaxData()));
            realDataUtil.setOuterData(String.valueOf(b.getResidualChlorineMaxData()));
            //设置文案
     ...
     */
    @Override
    public Class<ResidualRealBean> getParseClass() {
        return ResidualRealBean.class;
    }

    @Override
    public void bindHistoryData(BaseViewHolder holder, ResidualHistoryBean data, int position) {
        ...
        historyTemp.setText("水流量" + data.getResidualChlorineData());
        historyTime.setText("时间" + data.getResidualChlorineHistoryTime());
    ...
    public void deleteDevice(int position, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        String ehmId = String.valueOf(waterPressureRealBeans.get(position).getResidualChlorineId());
     ...
     */
    @Override
    public void realItemAdapter(BaseViewHolder holder, ResidualRealBean data, int position) {

        TextView item_name = holder.getView(R.id.dissolved_item_name);

        item_name.setText(data.getResidualChlorineName());
    ...
    public void changeDevice(String flag, BaseDeviceDetailManageAdapter.SVHolder svHolder) {
        ...
        ResidualRealBean temp = waterPressureRealBeans.get(position);
        ...
        deviceManageEditBean.setaDeviceName(temp.getResidualChlorineName());
        deviceManageEditBean.setActionID(temp.getResidualChlorineId());


        deviceManageEditBean.setaDeviceInterval(temp.getIntervalTime());
//
        deviceManageEditBean.setRangeMin(temp.getResidualChlorineMinData());
        deviceManageEditBean.setRangeMax(temp.getResidualChlorineMaxData());
    ...
    public void deterMine(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("residualChlorineName", getTextString(keyArray.get("deviceName")));
        formBody.put("residualChlorineAddress", getSpinnerString(spinnerMap.get("deviceAddress")));
        formBody.put("residualChlorineMaxData", getTextString(keyArray.get("rangeMax")));
        formBody.put("residualChlorineMinData", getTextString(keyArray.get("rangeMin")));
        ...
        OkHttpUtil.getInstance().sendRowOkHttp("add", LainNewApi.getInstance().getRootPath() + LainNewApi.residualInsert, MyGson.getInstance().toJson(formBody), this);
    ...
    public void deterChange(Map<String, EditText> keyArray, Map<String, Spinner> spinnerMap) {
        ...
        formBody.put("residualChlorineMinData", getTextString(keyArray.get("rangeMin")));
        formBody.put("intervalTime", getTextString(keyArray.get("interval")));
        formBody.put("residualChlorineId", getTextString(keyArray.get("actionID")));
    ...
    @Override
    public void updateRealView(TextView textView, ResidualRealBean data, int position) {
        //设置列表的数据
        textView.setText(data.getResidualChlorineName());
    }
}
